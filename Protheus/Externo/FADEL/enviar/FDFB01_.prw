...
User Function fCnabFA()
    ...
    If Pergunte(cPerg, .T.)
	    ...
	    if nHandle = -1
            ...
            if ProcN == 'FINA740'
                FWMsgRun(, {|| lTemDados := fLoadSql('1', @_cAlias ) },'Por Favor Aguarde...' , 'Processando Banco de Dados')
            else 
                FWMsgRun(, {|| lTemDados := fLoadSql('2', @_cAlias ) },'Por Favor Aguarde...' , 'Processando Banco de Dados')
            ENDIF
            ...
            If lTemDados
                ...
                (_cAlias)->(DbGoTop())
                if ProcN == 'FINA740'
                    nICtDeb := AllTrim((_cAlias)->E1_CONTA)
                    nICtCre := AllTrim((_cAlias)->E1_AGEDEP)
                    nIMoned := AllTrim(Str((_cAlias)->E1_MOEDA))
                    nImTotr := StrTran( Subs( AllTrim(str((_cAlias)->E1_VALOR)),1, len(AllTrim(Str((_cAlias)->E1_VALOR)))),".","")
                    nInRodo := StrZero(Val(AllTrim((_cAlias)->A1_CGC)),12,0)
                    nIfChCr := AllTrim((_cAlias)->E1_VENCTO)
                ELSE 
                ENDIF
                    while !(_cAlias) -> (EOF())
                        cTxt := "D"
                       ...
                       // cTxt += RIGHT("0000000000"+AllTrim((_cAlias)->E1_CONTA),10)
                        ...
                        cTxt += nInRodo //LEFT("000000000000"+AllTrim((_cAlias)->A1_CGC), 12)
                        //cTxt += StrZero(Val(AllTrim((_cAlias)->A1_CGC)),12,0) //LEFT("000000000000"+AllTrim((_cAlias)->A1_CGC), 12)
                    // IF POsicione('SA2',1,FWxFilial("SA2")+(_cAlias)->E1_CLIENTE, "A2_COD") != ''  
                        cTxt += "0"
...
Return nil
Static Function fLoadSql(_cTipo, _cAlias)
    Local _cQry 		:= ""
    if _cTipo == '1'
        ...
        _cQry += "	AND E1_NUMBOR BETWEEN '"+MV_PAR01+"' AND '"+MV_PAR02+"'" + CRLF
