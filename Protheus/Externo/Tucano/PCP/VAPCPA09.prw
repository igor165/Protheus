...
For nI := 1 to Len(aDadTl)
	...
	aAdd(aDatC1,{aDadTl[nI][1]})
	
	For nX := 2 To Len(aDadTl[nI])
		if (Len(aDadTl[nI][nX]) * 30) > nwidth 
			
			lQuebra := .T.
			aAdd(aDatC1[nI],{aDadTl[nI][nX][1]})
			...
			For nY := 2 to Len(aDadTl[nI][nX])
				...
				aAdd(aDatC1[nI][Len(aDatC1[nI])],aDadTl[nI][nX][nY])
			Next nY

		else
			aAdd(aDatC1[nI],aDadTl[nI][nX])
		endif 
	Next nX
Next nI 

if Len(aDatC1) > 0
	aDadTl := aClone(aDatC1)
endif
...
//					
//					                         			//linha inicial, coluna inicial                                       //tamanho coluna, tamanho linha
//					AAdd(aLinCnf[nChvCnf], TPanel():New(nLinLin, 015,, aPnlCnf[nChvCnf], oTFntGr, .T.,, aCorTl[4], aCorTl[2], (065 * (Len(aDadTl[nCntAll][nCntLin]) - 1)) + 005, 090)) //painel da linha
//					AAdd(aCurLin[nChvCnf], {})
				EndIf
