...
Static Function LoadSB8()
    ...
    _cQry += " 	   FROM SB8010 (NOLOCK) SB8  " + CRLF
    _cQry += " 	   JOIN SB1010 (NOLOCK) SB1 ON  " + CRLF
    _cQry += " 	        B1_COD = B8_PRODUTO  " + CRLF
    _cQry += " 		AND SB1.D_E_L_E_T_ = ' '  " + CRLF
    _cQry += " 	  WHERE B8_LOTECTL NOT IN (SELECT Z0O_LOTE FROM Z0O010 WHERE Z0O_FILIAL = B8_FILIAL AND Z0O_LOTE = B8_LOTECTL AND D_E_L_E_T_ =' ' ) " + CRLF
    _cQry += " 	    AND B8_LOTECTL IN (SELECT DISTINCT Z0F_LOTE FROM Z0F010 WHERE Z0F_FILIAL = B8_FILIAL AND Z0F_LOTE = B8_LOTECTL AND D_E_L_E_T_ =' ' ) " + CRLF
