            ...
            if !LockByName("CriaTrat" + DToS(Z0R->Z0R_DATA), .T., .T.)
                ...
                if !Empty(aIMS)
                    ...
                    cSql := " with Lotes as (" + CRLF +;
                            ...
                            "            , cast(convert(datetime, '" + DToS(Z0R->Z0R_DATA) + "', 103) - convert(datetime, min(SB8.B8_XDATACO), 103) as numeric) DIAS_NO_CURRAL -- Dias Cocho" + CRLF +; // +1 DIA
        ...
        if !Empty(aIMS)
            ...
            cSql := " with Lotes as (" + CRLF +;
                              " select Z08_FILIAL" + CRLF +;
                                   ...
                                   " , sum(SB8.B8_SALDO) B8_SALDO" + CRLF +;
                                   " , min(SB8.B8_XDATACO) DT_ENTRADA" + CRLF +;
                                   " , cast(convert(datetime, '" + DToS(Z0R->Z0R_DATA) + "', 103) - convert(datetime, min(SB8.B8_XDATACO), 103) as numeric) DIAS_NO_CURRAL -- Dias Cocho" + CRLF +; //+1
                                " from " + RetSqlName("Z08") + " Z08" + CRLF +;
                                " join " + RetSqlName("SB8") + " SB8" + CRLF +;
                                  " on SB8.B8_FILIAL  = '" + FWxFilial("SB8") + "'" + CRLF +;
                                 " and SB8.B8_X_CURRA = Z08.Z08_CODIGO" + CRLF +;
            ...
            cSql +=           " select Lotes.Z08_CONFNA" + CRLF +; //", Lotes.Z08_LINHA" 
                                    ...
                                    ", DIAS_NO_CURRAL DIAS_COCHO" + CRLF +; 
                                   " , case " + CRLF +;
                                          " when cast(UltDiaPlanNut.MAIORDIAPL as numeric) <= cast(convert(datetime, '" + DToS(Z0R->Z0R_DATA) + "', 103) - convert(datetime, DiasPlano.Z0O_DATAIN, 103) as numeric) + cast(Z0O.Z0O_DIAIN as numeric)-1" + CRLF +; // O primeiro dia é o dia 1 e não o dia 0
                                          " then right('000' + UltDiaPlanNut.MAIORDIAPL, 3)" + CRLF +;
                                          " else right('000' + cast((cast(convert(datetime, '" + DToS(Z0R->Z0R_DATA) + "', 103) - convert(datetime, DiasPlano.Z0O_DATAIN, 103) as numeric) + cast(Z0O.Z0O_DIAIN as numeric)-1) as varchar(3)), 3)" + CRLF +;
                                      " end DIA_PLNUTRI" + CRLF +;
                                   " , right('000' + cast((cast(convert(datetime, '" + DToS(Z0R->Z0R_DATA) + "', 103) - convert(datetime, DiasPlano.Z0O_DATAIN, 103) as numeric) + cast(Z0O.Z0O_DIAIN as numeric)) as varchar(3)), 3) DIAS_NO_PLANO" + CRLF +;
                                   " , Z0O.Z0O_GMD" + CRLF +;
                                   " , Z0O.Z0O_DCESP" + CRLF +;
    ...
    if Empty(LOTES->Z0O_CODPLA)
        ...
        if !Empty(aChave := MaxVerTrat(LOTES->B8_LOTECTL, Z0R->Z0R_DATA))
            ...
            if !TMPZ05->(Eof())
                ...
                cNroTrato := ""
                cDieta    := ""
                nPCMSAnt := (If(TMPZ05->Z05_CMSPN == 0, TMPZ05->Z05_KGMSDI / TMPZ05->Z05_PESMAT,TMPZ05->Z05_CMSPN) / 100) / 0.022
                ...
                DbUseArea(.T., "TOPCONN", TCGenQry(,,;
                    ...
                    */ 
                    ...
                    ElseIf GetMV("VA_AJUDAN") == "P" // Se Ajuste for em Percentual (Z0G_PERAJU)
                         //Peso Projetado = LOTES->B8_XPESOCO + LOTES->DIAS_COCHO * LOTES->Z0O_GMD
                        If !(TMPZ05->Z05_KGMSDI == 0)
                            nQtdTrato := NoRound(( (LOTES->B8_XPESOCO + (LOTES->DIAS_COCHO * LOTES->Z0O_GMD)) * (nPCMSAnt + (Z0G->Z0G_PERAJU/100)) * 0.022 ) /   TMPZ06->QTD, TamSX3("Z06_KGMSTR")[2])
                        Else
                                    nQtdTrato := 0
                        EndIf
                        //nQtdTrato := NoRound(( ( (LOTES->B8_XPESOCO + LOTES->DIAS_COCHO * LOTES->Z0O_GMD) / TMPZ05->Z05_PESMAT )  *  ( (LOTES->B8_XPESOCO + LOTES->DIAS_COCHO * LOTES->Z0O_GMD) * ( (TMPZ05->Z05_CMSPN / 100)) * If(Z0G->Z0G_PERAJU > 0, (Z0G->Z0G_PERAJU / 100), 1) ) ) ) /   TMPZ06->QTD
                    ...
                    while !TMPZ06->(Eof())
                        if (++i == TMPZ06->QTD)
                            ...
                            ElseIf GetMV("VA_AJUDAN") == "P" // Se Ajuste for em Percentual (Z0G_PERAJU)
                                //nQtdTrato := (((LOTES->B8_XPESOCO + LOTES->DIAS_COCHO * LOTES->Z0O_GMD) * (TMPZ05->Z05_CMSPN/100)) * If(Z0G->Z0G_PERAJU > 0, (Z0G->Z0G_PERAJU/100), 1 ) /TMPZ06->QTD)- nTotTrtClc
                                If !(TMPZ05->Z05_KGMSDI == 0)
                                    if TMPZ06->Z06_TRATO $ "1" .and. Z0G->Z0G_DESCONT == 'S' 
                                        nQtdTrato := (((LOTES->B8_XPESOCO + (LOTES->DIAS_COCHO * LOTES->Z0O_GMD)) *  (nPCMSAnt + (Z0G->Z0G_PERAJU/100)) * 0.022 ) ) / TMPZ06->QTD * (Z0G->Z0G_PERDES/100)
                                        nDesPerc  := nQtdTrato//((((LOTES->B8_XPESOCO + (LOTES->DIAS_COCHO * LOTES->Z0O_GMD)) *  (nPCMSAnt + (Z0G->Z0G_PERAJU/100)) * 0.022 ) ) - nQtdTrato )/ (TMPZ06->QTD - 1)
                                    Else
                                        //nQtdTrato := (((LOTES->B8_XPESOCO + (LOTES->DIAS_COCHO * LOTES->Z0O_GMD))  * (nPCMSAnt + (Z0G->Z0G_PERAJU/100)) * 0.022 ) ) - nTotTrtClc
                                        if Z0G->Z0G_DESCONT == 'S' 
                                            nQtdTrato := (((LOTES->B8_XPESOCO + (LOTES->DIAS_COCHO * LOTES->Z0O_GMD)) *  (nPCMSAnt + (Z0G->Z0G_PERAJU/100)) * 0.022 )  - nDesPerc) / (TMPZ06->QTD - 1)
                                        else 
                                            nQtdTrato := (((LOTES->B8_XPESOCO + (LOTES->DIAS_COCHO * LOTES->Z0O_GMD)) *  (nPCMSAnt + (Z0G->Z0G_PERAJU/100)) * 0.022 ) ) / (TMPZ06->QTD)
                                        EndIf
                                    EndIf
                                Else
                                    nQtdTrato := 0
        ...
        if LOTES->DIAS_NO_PLANO <= LOTES->MAIORDIAPL
            ...
            if GetMV("VA_CRTRDAN",,.F.) .and. !Empty(aChave := MaxVerTrat(LOTES->B8_LOTECTL, Z0R->Z0R_DATA))
                ...
                DbUseArea(.T., "TOPCONN", TCGenQry(,,;
                                        ...
                                        " )" +;
                                        " select Z0O.*, Z0M.*, QTDTRAT.QTD, Z05.Z05_KGMSDI QTDTRATO, Z05.Z05_PESMAT, Z05.Z05_CMSPN CMSPNANT" +;
                                          " from " + RetSqlName("Z0O") + " Z0O" +;
                                          " join " + RetSqlName("Z0M") + " Z0M" +;
                                            " on Z0M.Z0M_FILIAL = '" + FWxFilial("Z0M") + "'" +;
                                           " and Z0M.Z0M_CODIGO = Z0O.Z0O_CODPLA" +;
                    ...
                    nPCMSAnt := (If(TMPZ0M->CMSPNANT == 0, TMPZ0M->QTDTRATO/TMPZ0M->Z05_PESMAT,TMPZ0M->CMSPNANT)/100) / 0.022
                    
                    If GetMV("VA_AJUDAN") == "K" // Ajuste da nota de Cocho em KG (Z0G_AJSTKG)
                        nQtdTrato := Noround((TMPZ0M->QTDTRATO+Z0G->Z0G_AJSTKG)/TMPZ0M->QTD, TamSX3("Z06_KGMSTR")[2])
                    ElseIf GetMV("VA_AJUDAN") == "P" // Se Ajuste for em Percentual (Z0G_PERAJU)
                        //TODO ARTHUR TOSHIO VERIFICAR TMPZ0M->QTDTRATO PARA PEGAR DO DIA ANTERIOR NAZ05                        
                        //nQtdTrato := NoRound(( ( (LOTES->B8_XPESOCO + LOTES->DIAS_COCHO * LOTES->Z0O_GMD) / TMPZ0M->Z05_PESMAT )  *  ( (LOTES->B8_XPESOCO + LOTES->DIAS_COCHO * LOTES->Z0O_GMD) * ( (TMPZ0M->CMSPNANT / 100)) * If(Z0G->Z0G_PERAJU > 0, (1+Z0G->Z0G_PERAJU / 100), 1) ) ) ) /   TMPZ0M->QTD
                        //nQtdTrato := NoRound((LOTES->B8_XPESOCO + ( LOTES->DIAS_COCHO ) * LOTES->Z0O_GMD) *  (TMPZ0M->CMSPNANT/100) + ((TMPZ0M->CMSPNANT/100) * (Z0G->Z0G_PERAJU/100))) / TMPZ0M->QTD
                        If !(TMPZ0M->CMSPNANT == 0)
                            nQtdTrato := NoRound(((LOTES->B8_XPESOCO + (LOTES->DIAS_COCHO * LOTES->Z0O_GMD)) * (nPCMSAnt + (Z0G->Z0G_PERAJU/100)) * 0.022 ) / TMPZ0M->QTD)
                            
                        Else 
                            nQtdTrato := 0
                        EndIf
                        //nQtdTrato := NoRound(((LOTES->B8_XPESOCO + ( LOTES->DIAS_COCHO ) * LOTES->Z0O_GMD) *  ((TMPZ0M->CMSPNANT/100)) * (1+(Z0G->Z0G_PERAJU/100)))/nPerMS) /TMPZ0M->QTD
                    ...
                    while !TMPZ0M->(Eof())
                        ...
                        if (++i == TMPZ0M->QTD)
                            ...
                            ElseIf GetMV("VA_AJUDAN") == "P" // Se Ajuste for em Percentual (Z0G_PERAJU)
                                //nQtdTrato := NoRound((LOTES->B8_XPESOCO + (LOTES->DIAS_COCHO +1 )* LOTES->Z0O_GMD) / (LOTES->B8_XPESOCO + (LOTES->DIAS_COCHO +1 )* LOTES->Z0O_GMD) * ((TMPZ0M->CMSPNANT/100) + ((TMPZ0M->CMSPNANT/100)*(Z0G->Z0G_PERAJU/100 )))) - nTotTrtClc
                                nQtdTrato := ((LOTES->B8_XPESOCO + (LOTES->DIAS_COCHO * LOTES->Z0O_GMD)) *  (nPCMSAnt + (Z0G->Z0G_PERAJU/100)) * 0.022 ) - nTotTrtClc
            ...
            nPcmsIni := SuperGetMV("MV_CMSIN",,1.45)
                ...
                DbUseArea(.T., "TOPCONN", TCGenQry(,, _cQry), "TMPZ0M", .F., .F.)
                    ...
                    nTotMCal := 0
                    nTotMN := 0
                    //nPCMSAnt := (If(TMPZ0M->CMSPNANT==0, TMPZ0M->Z05_KGMSDI/TMPZ0M->Z05_PESMAT,TMPZ0M->CMSPNANT)/100) / 0.022
                    If GetMV("VA_AJUDAN") == "K" // Ajuste da nota de Cocho em KG (Z0G_AJSTKG)
                        nQtdTrato := NoRound((TMPZ0M->Z0M_QUANT+Z0G->Z0G_AJSTKG)/TMPZ0M->QTD, TamSX3("Z06_KGMSTR")[2])
                    ElseIf GetMV("VA_AJUDAN") == "P" // Se Ajuste for em Percentual (Z0G_PERAJU)
                            
                            nQtdTrato := (LOTES->B8_XPESOCO * (nPcmsIni / 100)) / TMPZ0M->QTD
            ...
            if !Empty(aChave := MaxVerTrat(LOTES->B8_LOTECTL, Z0R->Z0R_DATA))
                ...
                if !TMPZ05->(Eof())
                    ...
                    DbUseArea(.T., "TOPCONN", TCGenQry(,,;
                        ...
                        cNroTrato := ""
                        cDieta := ""
                        nPCMSAnt := (TMPZ05->CMSPNANT/100) / 0.022
                        ...
                        ElseIf GetMV("VA_AJUDAN") == "P" // Se Ajuste for em Percentual (Z0G_PERAJU)
                            //nQtdTrato := NoRound((TMPZ05->Z05_KGMSDI+((TMPZ05->Z05_KGMSDI*Z0G->Z0G_PERAJU)/100))/TMPZ06->QTD, TamSX3("Z06_KGMSTR")[2])
                            If !(TMPZ05->CMSPNANT == 0)
                                nQtdTrato := NoRound(((LOTES->B8_XPESOCO + (LOTES->DIAS_COCHO * LOTES->Z0O_GMD)) * (nPCMSAnt + (Z0G->Z0G_PERAJU/100)) * 0.022 ) / TMPZ06->QTD)
                            Else
                                nQtdTrato := 0
                            EndIf
                        //nQtdT
                    ...
                    " select MS.Z06_LOTE" + CRLF +;
                      ...
                      " join (" + CRLF +;
                           " select PVT.Z06_LOTE, PVT.[1] DI1, PVT.[2] DI2, PVT.[3] DI3, PVT.[4] DI4" + CRLF +;
                             ...
                             " from (" + CRLF +;
                                  " select Z06.Z06_LOTE, Z06.Z06_TRATO, SB1.B1_XDESC Z06_DIETA" + CRLF +;
                                    " from " + RetSqlName("Z06") + " Z06" + CRLF +;
                                    " join " + RetSqlName("SB1") + " SB1 ON "  + CRLF +;
                                         " SB1.B1_COD = Z06.Z06_DIETA " + CRLF +;
...
DbUseArea(.T., "TOPCONN", TCGenQry(,,;
                          " select Z06.Z06_TRATO" +;
                                ...
                                ", Z06.Z06_MEGCAL" +;
                                ", Z06.Z06_KGMNT" +;
                                ", SB1.B1_XDESC Z06_DESC" +;
                                ", Z06.R_E_C_N_O_ Z06_RECNO" +;
                            " from " + RetSqlName("Z06") + " Z06" +;
                            " join " + RetSqlName("SB1") + " SB1 on" +;
                                 " SB1.B1_COD = Z06.Z06_DIETA" +;
                             " and SB1.D_E_L_E_T_ = ' ' " +;
                           " where Z06.Z06_FILIAL = '" + FWxFilial("Z06") + "'" +;
...
next
                    ...
                    cSql:= "with PRODU AS (" +;
                          ...
                          " )" +;
                                " select PERIODO.DIA" +;
                                     " , Z05_DIETA " +;
                                     " , Z05_CMSPN " +;
                                     " , ISNULL((SELECT STRING_AGG(RTRIM(B1_XDESC), '; ') FROM " + RetSqlName("SB1") + " WHERE B1_COD IN (SELECT DISTINCT Z0W.Z0W_RECEIT FROM " + RetSqlName("Z0W") + " Z0W WHERE Z0W.Z0W_FILIAL = Z0R.Z0R_FILIAL AND Z0W.Z0W_LOTE = Z05.Z05_LOTE AND Z0W.Z0W_DATA = Z05.Z05_DATA AND Z0W.D_E_L_E_T_ = ' ' GROUP BY Z0W.Z0W_FILIAL, Z0W.Z0W_DATA, Z0W.Z0W_LOTE, Z0W.Z0W_RECEIT )), ' ' ) AS  Z05_DESC " +;
                                     " , Z05.Z05_CABECA" +;
                                     " , Z05.Z05_KGMSDI" +;
                                     " , Z05.Z05_KGMNDI" +;
                                     " , Z05.Z05_MEGCAL" +;
    ...
    MemoWrite( "C:\totvs_relatorios\VAPCPA05ANT.sql", cSql )
            ...
            if Empty(cVersao)
                DbUseArea(.T., "TOPCONN", TCGenQry(,,;
                                          ...
                                          " where ZG1_FILIAL = '" + FWxFilial("ZG1") + "'" +;
                                            " and ZG1_COD = '" + cDieta + "'" +; 
                                            " and ZG1_DTALT <= '" +DtOS(Z0R->Z0R_DATA)+ "' " +;
                                            " and D_E_L_E_T_ = ' '";
                                                     ), "TMPZG1", .F., .F.)
...
default dData    := dDataBase
        ...
        DbUseArea(.T., "TOPCONN", TcGenQry(,,;
                              " select min(SB8.B8_XDATACO) B8_XDATACO" +;
                                " from " + RetSqlName("SB8") + " SB8" +;
                               " where SB8.B8_FILIAL  = '" + FWxFilial("SB8") + "'" +;
                                 " and SB8.B8_LOTECTL = '" + cLote + "'" +;
                                 " and SB8.B8_SALDO   <> 0" +;
