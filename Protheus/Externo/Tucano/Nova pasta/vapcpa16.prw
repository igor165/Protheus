...
Private oFntSay17N := TFont():New('Courier New',,-26,,.t.)
private lCheck := .t.   
...
if !Empty(dData)
    ...
    if Z0I->(DbSeek(FWxFilial("Z0I")+DToS(dData)))
        ...
        Do Case
            Case aBrwNotas[1][15] > 100
                //oDifMeta := nil
                oDifMeta := TSay():New(040, aPos2[16], {|| Transform (0, "@E 999.99")}, oWndNotas,, oFntSay17N, , , , .t.,CLR_GREEN/* anClrText */)
                oDifMeta :SetText(Transform(aBrwNotas[1][15], "@E 999.99"))
                MsgAlert("Atingiu a meta!","Atenção")
            Case aBrwNotas[1][15] > 90 .and. aBrwNotas[1][15] < 100
                //oDifMeta := nil
                oDifMeta := TSay():New(040, aPos2[16], {|| Transform (0, "@E 999.99")}, oWndNotas,, oFntSay17N, , , , .t.,CLR_MAGENTA/* anClrText */)
                oDifMeta :SetText(Transform(aBrwNotas[1][15], "@E 999.99"))
                //MsgAlert("Não Atingiu a meta 90-100 !","Atenção")
            Case aBrwNotas[1][15] > 80 .and. aBrwNotas[1][15] < 90
                //oDifMeta := nil
...
Local cMsg   := ""
    ...
    DbUseArea(.t., "TOPCONN", TCGenQry(,,;
           _cQry := " select TMPTBL.DIA_NOTA" + CRLF +;
                 ...
                 ",   CASE WHEN (sum(case Z0W.Z0W_PESDIG when 0 then Z0W.Z0W_QTDREA else Z0W.Z0W_PESDIG end) >0) THEN ((SELECT (sum(case Z0W.Z0W_PESDIG when 0 then Z0W.Z0W_QTDREA else Z0W.Z0W_PESDIG end)*Z05_MEGCAL)/ SUM(Z0W_QTDPRE)) / ISNULL(( (SELECT (Z05_PESMAT+Z0O_GMD) * ISNULL((Z0O_CMSPRE/100),0.022) " + CRLF+;
                               "FROM " + RetSqlName("Z0O") + " Z0O" + CRLF+;
			                  ...
			                  "(SELECT AVG(G1_ENERG) " + CRLF+;
			                    ...
			                    "WHERE G1_FILIAL = '" + FWxFilial("Z0I") + "' AND D_E_L_E_T_ = ' ' AND G1_COD = '060601022' ) ),0) ) ELSE 0 END SITUA" + CRLF+;
