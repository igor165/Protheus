...
//	Local bPosZ6		:= {|oModelGrid, nLine, cAction, cField| FSZ6LPos(oModelGrid, nLine, cAction, cField) }
...
/* 	aAdd(aGatilhos, FWStruTriggger( "CBC_LOTEVIF",;                             //Campo Origem
                                    "CBC_LOTEVIF",;                             //Campo Destino
                                    PadR('', TamSX3('Z6_LOTEVIF')[1]),;         //Regra de Preenchimento
                                    .F.,;                                       //Irá Posicionar?
	...
	oModel:AddGrid('FORMGridM', 'FORMCabec', oStrGridM,bPreZ6/* ,bPosZ6 */)
    ...
    //GRID ABA02
	oModel:AddGrid('FORMRG2', 'FORMGridM' , oStrRLt,bPreZ7/* ,bPosZ7 */)
...
Static Function ViewDef()
	...
	Local oView     := Nil
	Local bBloco := {|/* oView,cId,cField,xValue */|MinhaAcao(/* oView,cId,cField,xValue */)}
	...
	oView:SetViewProperty('VIEW_GridM', "ENABLENEWGRID")

	//cCss := GridCss()

//	oView:SetViewProperty('VIEW_GridM', 'SETCSS',  GridCss(oView))
	oView:SetViewProperty('VIEW_GridM', 'SETCSS', { "QTableView { selection-background-color: #1C9DBD; } "+ CRLF +;
													"QLineEdit { background: #F235DF;}" })
...
Static Function FSZ7LPre(oModelGrid, nLine, cAction, cField)
	...
	//oView:Refresh()
RETURN lRet 
/* Static Function FSZ6LPos(oModelGrid, nLine, cAction, cField)
	Local oModel 		:= FWModelActive()
	Local oModelGM 		:= oModel:GetModel("FORMGridM")
	Local oModelGR 		:= oModel:GetModel("FORMRG2")
	Local lRet			:= .T.	

	if cField == 'Z6G_QTDLIB2'
			//oModelGM:LoadValue("Z6G_QTDLIB2"   , &(ReadVar()))	
		if U_VLDLIB2()
			U_FATBGRV()
		endif
	elseif cField == 'Z6G_QTDLIB'
		//oModelGM:LoadValue("Z6G_QTDLIB"   , &(ReadVar()))
		if U_VLDLIB()
			U_FATBGRV()
		endif
	elseif cField == 'Z6G_OMAVOL'
 	...
 	if cAction == 'SETVALUE'
		if cField $ 'Z6G_QTDLIB2|Z6G_QTDLIB|Z6G_OMAVOL'
			...
			elseif SUBSTR(oModelGM:GetValue("Z5G_CLI"),1,6)+SUBSTR(oModelGM:GetValue("Z5G_CLI"),8,2)  == ALLTRIM(cCliCFGru)
				...
				lRet := .F.
			elseif cField == 'Z6G_QTDLIB2'
				if oModelGM:SetValue("Z6G_QTDLIB2"   , &(ReadVar()))	
					U_FATBGRV()
				endif 
				//if U_VLDLIB2()
				//	U_FATBGRV()
				//endif
			elseif cField == 'Z6G_QTDLIB'
				if oModelGM:SetValue("Z6G_QTDLIB"   , &(ReadVar()))
					if U_VLDLIB()
						U_FATBGRV()
					endif
				endif
			elseif cField == 'Z6G_OMAVOL'
				//oModelGM:SetValue("Z6G_OMAVOL"   , &(ReadVar()))
				if oModelGM:SetValue("Z6G_OMAVOL"   , &(ReadVar()))	
					U_FATBGRV()
				endif 
				//if U_VLDVOL1()
				//	U_FATBGRV()
...
Static Function GetModelCabec(oModel,oStrCabec)
	...
	oStrCabec:AddField('Manter Foco?'			, 'Manter Foco?'		, 'CBC_FOCO'	, 'C',1							,0						,FwBuildFeature( STRUCT_FEATURE_VALID,"Pertence('SN')")						,,{'S=Sim', 'N=Não'}																																					,.F.,FwBuildFeature( STRUCT_FEATURE_INIPAD, "'N'" )																					,.F.,.F.,.T.)
Return oStrCabec
/*
...
Static Function GetViewCabec(oModel,oStrCabec)
	...
	oStrCabec:AddField('CBC_LOTEVIF', '20', 'Lote VIF'				, 'Lote VIF'			,{}, 'C',,,     ,.T.									,,,{}																																		,,					,.T.,,,)
	oStrCabec:AddField('CBC_FOCO'	, '21', 'Manter Foco?'			, 'Manter Foco?'		,{}, 'C',,,     ,.T.									,,,{'S=Sim', 'N=Não'}																														,,'S'				,.T.,,,)
Return oStrCabec	
/* 
...
Static Function RefreshMultiLote(_cCli,_cLoja,_cPed,_cProd)
	...
	While !(_cAlias)->(EOF())
		...
		nTot 		:= (_cAlias)->Z7_QTDE // oGridR:GetValue("Z7G_QTDE")
		nTotVol 	:= (_cAlias)->Z7_PESO //oGridR:GetValue("Z7G_PESO")
...
/* 	oModel:GetModel('FORMRG2'):SetOptional(.T.) */
	...
	//	GridCss(oModel,oModelGM,oView)
//		LOTECLR(oModel, oModelGM)
...
//	oModelGM:LoadValue("Z6G_QTDLIB"   , &(ReadVar()))
	...
	If _Peso > 0
		...
		elseif _Peso > oModelGM:GetValue("Z6G_UNSVEN") 
...
User Function VLDLTCB()
	Local oModel 	:= FWModelActive()
	Local oView 	:= FWViewActive()
	Local oModelCB 	:= oModel:GetModel("FORMCabec")
	Local oModelGM 	:= oModel:GetModel("FORMGridM")
	Local lRet 		:= .T. 
	...
	IF oModelGM:SetValue("Z6G_LOTEVIF", &(ReadVar()))
		if oModelGM:GetValue("Z6G_QTDLIB2") >= oModelGM:GetValue("Z6G_UNSVEN")
			oModelCB:LoadValue("CBC_LOTEVIF"  , PadR('', TamSX3('Z6_LOTEVIF')[1]))
...
User Function zBuscaCSS()
    ...
    Local nAtual
    Local cCSS
    Local cPasta := "C:\totvs_relatorios\"
    Local cArq := "zBuscaCSS.txt"
    ...
    For nAtual := 1 To Len(aControles)
        ...
        If ValAtrib("aControles[nAtuPvt]:bAction") != "U" .And. aControles[nAtuPvt]:cCaption == "Confirmar"
			...
			MemoWrite(StrTran(cPasta,".xml","")+"zBuscaCSS.txt" , cCSS)
        //    MemoWrite(StrTran(cPasta,".txt","") + cArquivo, cCSS)
...
Static Function GridCss(oModel,oModelGM,oView)
	Local cCss 			:= ""
	Local aCss 			:= {}
	Local nQtdV      	:= 0
	Local nQtdD      	:= 0
...
/* 	TButton { font: bold;
	...
	TButton:focus {	
			...
			outline-width:1px; 
			outline-style:solid; 
			outline-color: #FC5157; 
			outline-radius:3px; 
			border-color:#B5363B;}
	TButton:hover {	color: #FFFFFF;
	    background-color : qlineargradient(x1: 0, y1: 0, x2: 0, y2: 1,
		stop: 0 #CC3D42, stop: 1 #A61116);    
		border-width: 1px;     
		border-style: solid;     
		border-radius: 3px;     
		border-color: #B5363B; }
	...
	if ProcName(2) == 'FWLOADVIEW'
 	oModel 		:= FWModelActive()
 	oModelGM 	:= oModel:GetModel("FORMGridM") 
*/
	...
	If !oModelGM:IsEmpty()
		...
		If oModelGM:GetValue("Z6G_VVISTA")  != 'S' .and. !lKitPrd
			If nPesoD == 0 .AND. Mod(nLinha,2) == 0 .AND. !oModelGM:IsDeleted()
				cCss := "QTableView { background-color: #CC3D42; } "
				//cCss := "QTableView { background-color: RGB(255,255,255) } "
 			ElseIf nPesoD == 0 .AND. !oModelGM:IsDeleted()
				cCss := "QTableView { background-color: #CC3D42; } "
				//cCss := "QTableView { background-color: RGB(178,203,231)  } "
			Elseif nPesoD > 0 .AND. !lLoteOk
				cCss := "QTableView { background-color: #CC3D42; } "
				//cCss := "QTableView { background-color: RGB(0, 0, 255) } "
			Elseif nPesoD > 0 .AND. lLoteOk
				cCss := "QTableView { background-color: #CC9D43; } "
				//cCss := "QTableView { background-color: brown } "
			Endif
		Elseif oModelGM:GetValue("Z6G_VVISTA")  == 'S' .and. !lKitPrd
			If nPesoD == 0 .AND. Mod(nLinha,2) != 0  .AND. !oModelGM:IsDeleted()
				cCss := "QTableView { background-color: #CC9D43; } "
				//cCss := "QTableView { background-color: yellow } "
		    ElseIf nPesoD == 0 .AND. !oModelGM:IsDeleted()
				cCss := "QTableView { background-color: #CC3D42; } "
				//cCss := "QTableView { background-color: Brown } "
			Elseif nPesoD > 0 .AND. !lLoteOk
				cCss := "QTableView { background-color: #CC9D43; } "
				//cCss := "QTableView { background-color: blue } "
			Elseif nPesoD > 0 .AND. lLoteOk
				cCss := "QTableView { background-color: #CC3D42; } "
				//cCss := "QTableView { background-color: brown } "
		...
		ElseIf oModelGM:GetValue("Z6G_VVISTA") != 'S' .And. lKitPrd
			If Mod(nLinha,2) == 0 .AND. !oModelGM:IsDeleted()
				cCss := "QTableView { background-color: #CC3D42; } "
				//cCss := "QTableView { background-color: magenta } "
			ElseIf !oModelGM:IsDeleted()
				cCss := "QTableView { background-color: #CC9D43; } "
				//cCss := "QTableView { background-color: RGB(255,0,255) } "
			Elseif lLoteOk
				cCss := "QTableView { background-color: #CC3D42; } "
				//cCss := "QTableView { background-color: rgb(255, 0, 0) } "
			Elseif oModelGM:IsDeleted()
				cCss := "QTableView { background-color: #CC9D43; } "
				//cCss := "QTableView { background-color: rgb(255, 70, 0); } "
			Endif
		Endif
	Else
 		If !oModelGM:IsDeleted()
			...
			Else
				cCss := "QTableView { background-color: #CC9D43; } "
				//cCss := "QTableView { background-color: gray } "
			Endif
		Else
			cCss := "QTableView { background-color: #CC3D42; } "
			//cCss := "QTableView { background-color: brown } "
		Endif
	EndIf
	oView:SetViewProperty('VIEW_GridM', 'SETCSS', { cCss })
	aCss := {cCss}
Return /* aCss */
...
Static Function LOTECLR(oModel, oModelGM)
	...
	If !oModelGM:IsEmpty()
		...
		If oModelGM:GetValue("Z6G_VVISTA")  != 'S' .and. !lKitPrd
			If nPesoD == 0 .AND. Mod(nLinha,2) == 0 .AND. !oModelGM:IsDeleted()
				cCss := "QLineEdit { background-color: RGB(255,255,255) } "
				oView:SetViewProperty('VIEW_GridM', 'SETCSS', {cCss})
 			ElseIf nPesoD == 0 .AND. !oModelGM:IsDeleted()
				cCss := "QLineEdit { background-color: RGB(178,203,231)  } "
				oView:SetViewProperty('VIEW_GridM', 'SETCSS', {cCss})
			Elseif nPesoD > 0 .AND. !lLoteOk
				cCss := "QLineEdit { background-color: RGB(0, 0, 255) } "
				oView:SetViewProperty('VIEW_GridM', 'SETCSS', {cCss})
			Elseif nPesoD > 0 .AND. lLoteOk
				cCss := "QLineEdit { background-color: brown } "
				oView:SetViewProperty('VIEW_GridM', 'SETCSS', {cCss})
			Endif
		Elseif oModelGM:GetValue("Z6G_VVISTA")  == 'S' .and. !lKitPrd
			If nPesoD == 0 .AND. Mod(nLinha,2) != 0  .AND. !oModelGM:IsDeleted()
				cCss := "QLineEdit { background-color: yellow } "
				oView:SetViewProperty('VIEW_GridM', 'SETCSS', {cCss})
		    ElseIf nPesoD == 0 .AND. !oModelGM:IsDeleted()
				cCss := "QLineEdit { background-color: Brown } "
				oView:SetViewProperty('VIEW_GridM', 'SETCSS', {cCss})
			Elseif nPesoD > 0 .AND. !lLoteOk
				cCss := "QLineEdit { background-color: blue } "
				oView:SetViewProperty('VIEW_GridM', 'SETCSS', {cCss})
			Elseif nPesoD > 0 .AND. lLoteOk
				cCss := "QLineEdit { background-color: brown } "
				oView:SetViewProperty('VIEW_GridM', 'SETCSS', {cCss})
		...
		ElseIf oModelGM:GetValue("Z6G_VVISTA") != 'S' .And. lKitPrd
			If Mod(nLinha,2) == 0 .AND. !oModelGM:IsDeleted()
				cCss := "QLineEdit { background-color: magenta } "	
				oView:SetViewProperty('VIEW_GridM', 'SETCSS', {cCss})
			ElseIf !oModelGM:IsDeleted()
				cCss := "QLineEdit { background-color: RGB(255,0,255) } "
				oView:SetViewProperty('VIEW_GridM', 'SETCSS', {cCss})
			Elseif lLoteOk
				cCss := "QLineEdit { background-color: rgb(255, 0, 0) } "
				oView:SetViewProperty('VIEW_GridM', 'SETCSS', {cCss})
			Elseif oModelGM:IsDeleted()
				cCss := "QLineEdit { background-color: rgb(255, 70, 0); } "
				oView:SetViewProperty('VIEW_GridM', 'SETCSS', {cCss})
			Endif
		Endif
	Else
 		If !oModelGM:IsDeleted()
			If Mod(nLinha,2) == 0
				...
				oView:SetViewProperty('VIEW_GridM', 'SETCSS', {cCss})
			Else
				cCss := "QLineEdit { background-color: gray } "
				oView:SetViewProperty('VIEW_GridM', 'SETCSS', {cCss})
			Endif
		Else
			cCss := "QLineEdit { background-color: brown } "
