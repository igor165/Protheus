...
Static Function ModelDef()
	...
	Local bLineZ6	 	:= {|| fPreZ6()}
	Local bPosZ6	 	:= {|oModelGrid, nLine, cAction, cField| FSZ6Lok(oModelGrid, nLine, cAction, cField) }
	Local bPreLZ7	 	:= {|| fPreZ7()}
	Local bPosZ7		:= {|oModelGrid, nLine, cAction, cField| FSZ7Lok(oModelGrid, nLine, cAction, cField) }
	...
	oModel:AddFields('FORMCabec', /*cOwner*/, oStrCabec, /*bPre*/, /* { |x| FVldTok(oModel, .F.)} *//*bPost*/, /*bLoad*/)
	oModel:AddGrid('FORMGridM', 'FORMCabec', oStrGridM,bPosZ6,/* bLineZ6 */)
	
	//Atribuindo formulários para o modelo
    //RodapÃ© ABA01
 	oModel:AddFields("FORMRodap1", 'FORMGridM', oStrRDp)
	oModel:SetRelation("FORMRodap1", {{'Z5G_NUM', 'RDP_PED'},{'Z6G_PRODUTO', 'RDP_PROD'}},SZ5->(IndexKey(1)))
    //GRID ABA02
	oModel:AddGrid('FORMRG2', 'FORMCabec' , oStrRLt, bPosZ7, /* bPreLZ7 */)
	//CAMPOS ABA02
...
/* 	oModelGM:oModel:GetModel("FORMGridM"):AddIndex("01",{"Z5G_NUM","Z5G_CLI","Z6G_PRODUTO"}) */
	
	//Setando a descriÃ§Ã£o do Formulário
...
Static Function ViewDef()
	...
	Local oView     := Nil
	/* Local bBloco 	:= {|oView,cId,cField,xV|MinhaAcao(oView,cId,cField,xV)} */

	//Criando os campos Virtuais do Cabeçalho e RodapÃ©.
	...
	//Criando a view que serÃ¡ o retorno da Função e setando o modelo da rotina
	...
	oView:AddGrid(  'VIEW_GridM'   	, oStrGridM , 'FORMGridM')
	...
	SetKey(VK_F12, {|| LER_LOTE()})
    //Apenas para preencher grid com dados
	oView:SetCloseOnOk({||.T.})
	
	//O Formulário da interface serÃ¡ colocado dentro do container
	...
	/* oView:SetFieldAction("Z6G_LOTEVIF", bBloco) */
Return oView

/* Static function MinhaAcao(oView,cIDView,cField,xValue)
	Local oModel := FWModelActive()
	Local oView := FWViewActive()
	Local oGridM := oModel:GetModel("FORMGridM")
	//Local oGrid := oView:GetviewObject(cIDVIEW)

	if oGridM:GetValue("Z6G_LOTEVIF")
	oGrid:SetFocus()
RETURN */
/* 
...
*/
Static Function FSZ7Lok(oModelGrid, nLine, cAction, cField)
	Local oModel 		:= FWModelActive()
	Local oView 		:= FWViewActive()
...
/* 			oModelGM:GoLine(nI)
			if 	oModelGR:GetValue("Z7G_PED") 			 == oModelGM:GetValue("Z5G_NUM") .and. ;
				oModelGR:GetValue("Z7G_PRODUTO")		 == oModelGM:GetValue("Z6G_PRODUTO") .and. ;
				AllTrim(oModelGR:GetValue("Z7G_CODBAR")) == AllTrim(oModelGM:GetValue("Z6G_LOTEVIF"))
 */
...
/* 		for nI := 1 to oModelGM:GetQtdLine()
			...
			if 	oModelGR:GetValue("Z7G_PED") 			 == oModelGM:GetValue("Z5G_NUM") .and. ;
				...
				nQtdlib2 := oModelGR:GetValue("Z7G_QTDE") - oModelGM:GetValue("Z6G_QTDLIB2")
				nQtdlib	 := oModelGR:GetValue("Z7G_PESO") - oModelGM:GetValue("Z6G_QTDLIB")
				if nQtdlib2 < 0 
					nQtdlib2 := nQtdlib2 * -1
				ENDIF
				if nQtdlib < 0 
...
/* 			for nI := 1 to oModelGM:GetQtdLine()
				...
				IF oModelGM:SeekLine({{"Z5G_NUM"		, oModelGR:GetValue("Z7G_PED") },;
								      {"Z6G_PRODUTO"	, oModelGR:GetValue("Z7G_PRODUTO")},;
								      {"Z5G_CLI"		, oModelGR:GetValue("Z7G_CLI")}})
								     /*  {"Z6G_ITEM"		, oModelGR:GetValue("Z7G_ITEM") */
				
/* 				if oModelGR:GetValue("Z7G_PED") 			== oModelGM:GetValue("Z5G_NUM") .and. ;
					oModelGR:GetValue("Z7G_PRODUTO")		== oModelGM:GetValue("Z6G_PRODUTO") .and. ;
					AllTrim(oModelGR:GetValue("Z7G_LOTE")) 	== AllTrim(SubStr(oModelGR:GetValue("Z6G_LOTEVIF"),11,6))
 */
					oModelGM:LoadValue("Z6G_QTDLIB2", oModelGR:GetValue("Z7G_PESO"))
					oModelGM:LoadValue("Z6G_QTDLIB" , oModelGR:GetValue("Z7G_QTDE"))
...
/* 	oModelR:LoadValue("RLT_VLR1", oModelGR:GetValue("Z7G_QTDE"))
	oModelR:LoadValue("RLT_VLR2", oModelGR:GetValue("Z7G_PESO"))
	oModelR:LoadValue("RLT_OBS" , AllTrim(Posicione('SZ5',1,xFilial('SZ5')+oModelGR:GetValue("Z7G_PED"), 'Z5_MENNOTA')))
	 */
	FWRestRows( aSaveLines )
	oView:Refresh()
RETURN lRet 

Static Function FSZ6Lok(oModelGrid, nLine, cAction, cField)
	Local oModel 		:= FWModelActive()
	Local oModelGM 		:= oModel:GetModel("FORMGridM")
	Local lRet			:= .T.	
	Local aSaveLines 	:= FWSaveRows()
 	...
 	if cAction == 'SETVALUE'
		if cField $ 'Z6G_QTDLIB2|Z6G_QTDLIB|Z6G_OMAVOL'
			if Empty(oModelGM:GetValue("Z5G_NUM"))
				...
				lRet := .F.
			elseif !Empty(oModelGM:GetValue("Z6G_LOTEVIF"))
				oModel:SetErrorMessage("","","","","HELP", 'Não pode ser alterado', "Quantidade com lote só pode ser alterado no lote") 
				lRet := .F.
...
Static Function GetModelCabec(oModel,oStrCabec)
	...
	oStrCabec:AddField('Tp. Cliente?'	 		, 'Tp. Cliente?'		, 'CBC_TPCLI'	, 'C', 1,0,FwBuildFeature( STRUCT_FEATURE_VALID,"Pertence('R')")					 	,,{'R=Rede', 'V=Varejo'}	                                                                                                            								,.F.,FwBuildFeature( STRUCT_FEATURE_INIPAD, "'R'" )                                                                             	,.F.,.F.,.T.)
...
Static Function GetViewCabec(oModel,oStrCabec)
	...
	oStrCabec:AddField('CBC_NTENTR'	,  '3', 'Transp. Entrega'		, 'Transp. Entrega'		,{}, 'C',,,		,.F.									,,,{}																																		,,					,.F.,,,)
	oStrCabec:AddField('CBC_NTPRIM'	,  '4', 'Transp. Primario'		, 'Transp. Primario'	,{}, 'C',,,		,.F.									,,,{}																																		,,					,.F.,,,)
	...
	oStrCabec:AddField('CBC_NF' 	,  '8', 'NF'					, 'NF'					,{}, 'C',,,		,.T.									,,,{'1=NF Normal','2=NF Automatico'}																										,1,'1'				,.T.,,,)
	oStrCabec:AddField('CBC_ORDG'	,  '9', 'Ordenação'				, 'Ordenação'			,{}, 'C',,,		,.T.									,,,{'1=Zona Crescente Roteiro Crescente','2=Zona Crescente Roteiro Decrescente','3=Codigo de Produto','4=Cidade'}							,1,'1'				,.T.,,,)
...
User Function FATB01CE()
	...
	If aParam <> NIL
		...
		cIdPonto := aParam[2]
		cIdModel := aParam[3]
	//oObjGM := oObj:GetModel('FORMGridM')
/*  	 	lIsGrid := ( Len( aParam ) > 3 )
		If lIsGrid  */
/* 		nQtdLinhas 	:= oGridM:GetQtdLine()
		nLinha 		:= oGridM:nLine */
...
/* 		EndIf */
		...
		If Alltrim(cIdPonto) == "MODELVLDACTIVE"
			...
			if Alltrim(cIdPonto) == "FORMPRE" .and. cIdModel == 'FORMRG2'
				...
				if !oGridR:IsEmpty()
					...
					if !oGridM:IsEmpty()
						oGridM:SeekLine({{"Z6G_PRODUTO",cProd},{"Z7G_PED", cPed},{"Z7G_CLI",cCli}})
					endif
	...
	/* 			cCli 	:= SubStr(oGridR:GetValue("Z7G_CLI"),1,6)
				cLoja 	:= SubStr(oGridR:GetValue("Z7G_CLI"),8,2) */

	/* 			for nI := 1 to oGridM:GetQtdLine()
					oGridM:Goline(nI)

					if oGridM:GetValue("Z6G_PRODUTO") 				== cProd .and. ;
						oGridM:GetValue("Z6G_PRODUTO") 				== cPed .and. ;
						SubStr(oGridM:GetValue("Z6G_PRODUTO"),1,6) 	== cCli .and. ;
						SubStr(oGridM:GetValue("Z6G_PRODUTO"),8,2) 	== cLoja 
						
						oGridM:Setline(nI)
						exit
					endif
				next nI */
/* 			ELSEIF Alltrim(cIdPonto) == 'FORMPRE' .AND.  cIdModel == 'FORMRG2' // cIdModel == 'FORMRLT2'
				...
				FWRestRows( aSaveLines ) */
			elseIf Alltrim(cIdPonto) == 'FORMPRE' .AND. cIdModel == 'FORMGridM'
				oModel 	 	:= FwModelActivate()
				oGridM 		:= oModel:GetModel("FORMGridM")
				oGridR 		:= oModel:GetModel("FORMRG2")
	...
	ENDIF
RETURN xRet 
/* Static Function fPreZ6()
	Local oModel 		:= FWModelActive()
	Local oModelGM 		:= oModel:GetModel("FORMGridM")
	Local oModelR 		:= oModel:GetModel("FORMRodap1")
	Local lRet 			:= .T.
	Local aSaveLines 	:= FWSaveRows()
	
	_Cli  		:= SubStr(oModelGM:GetValue("Z5G_CLI"),1,6)
	_Loja 		:= SubStr(oModelGM:GetValue("Z5G_CLI"),8,2)
	_CliNome 	:= AllTrim(oModelGM:GetValue("Z5G_CLI")) + " / " +; 
				  	Posicione('SA1',1,xFilial('SA1')+_Cli+_Loja, 'A1_NOME')
	_End  		:= AllTrim(Posicione('SA1',1,xFilial('SA1')+_Cli+_Loja, 'A1_MUN')) + " - " +;
					AllTrim(Posicione('SA1',1,xFilial('SA1')+_Cli+_Loja, 'A1_BAIRRO')) + ", " +;
					AllTrim(Posicione('SA1',1,xFilial('SA1')+_Cli+_Loja, 'A1_EST')) + ", " +;
					AllTrim(Posicione('SA1',1,xFilial('SA1')+_Cli+_Loja, 'A1_END')) 
	_cProd 		:=  AllTrim(oModelGM:GetValue("Z6G_PRODUTO")) + " " +;
					AllTrim(Posicione('SB1',1,xFilial('SB1')+;
					AllTrim(oModelGM:GetValue("Z6G_PRODUTO")), 'B1_DESC'))
	_cMsg 		:= AllTrim(Posicione('SZ5',1,xFilial('SZ5')+oModelGM:GetValue('Z5G_NUM'), 'Z5_MENNOTA'))
	_cPed 		:= SubStr(AllTrim(oModelGM:GetValue("Z5G_NUM")),Len(AllTrim(oModelGM:GetValue("Z5G_NUM"))) - 5,6)
	nTot 		:= oModelGM:GetValue("Z6G_QTDLIB")
	nTotVol 	:= oModelGM:GetValue("Z6G_OMAVOL")
...
RETURN lRet  */
/* 
    Pos Validação de Linha Grid Principal
    Formulário: FORMRG2
*/
/* Static Function fPreZ7()
	Local oModel 		:= FWModelActive()
	Local oView			:= FWViewActive()
	Local oModelGM 	    := oModel:GetModel("FORMGridM")
	Local oModelGR 	    := oModel:GetModel("FORMRG2")
	Local oModelR 		:= oModel:GetModel("FORMRLT2")
	Local lRet 			:= .T.
	Local cCli, cPed, cProd, _cMsg
	Local nTot, nTotVol

	cProd 	:= oModelGR:GetValue("Z7G_PRODUTO")
	cPed 	:= oModelGR:GetValue("Z7G_PED")
	cCli 	:= oModelGR:GetValue("Z7G_CLI")

	oModelGM:SeekLine({{"Z6G_PRODUTO",cProd},{"Z7G_PED", cPed},{"Z7G_CLI",cCli}})
	
	_cMsg 		:= AllTrim(Posicione('SZ5',1,xFilial('SZ5')+oModelGR:GetValue('Z7G_PED'), 'Z5_MENNOTA'))
	nTot 		:= oModelGR:GetValue("Z7G_QTDE")
	nTotVol 	:= oModelGR:GetValue("Z7G_PESO")
	
//	oModelGR:GoLine(nLine)
	oModelR:LoadValue('RLT_VLR1' , nTot)
 	oModelR:LoadValue("RLT_VLR2" , nTotVol)
	oModelR:LoadValue("RLT_OBS"  , _cMsg)
	oView:Refresh("VIEW_Rodap02")
RETURN lRet  */
/* 
...
User Function CEFATBPG()
	...
	While !(_cAlias)->(EOF())
		...
		oModelGM:LoadValue("Z6G_NCS"	, SubStr(AllTrim((_cAlias)->Z6_XPRDKIT),11,06)						)			 
		...
		If SZ7->(DbSeek(xFilial('SZ7')+;
			...
			DO WHILE .NOT. SZ7->(EOF()) .And. SZ7->Z7_FILIAL  == xFilial('SZ7')   .And. ;
					SZ7->Z7_PEDIDO  == (_cAlias)->Z5_NUM      .And. ;
					SZ7->Z7_CLIENTE == (_cAlias)->Z5_CLIENT   .And. ;
					SZ7->Z7_LOJA    == (_cAlias)->Z5_LOJACLI  .And. ;
					SZ7->Z7_PRODUTO == (_cAlias)->Z6_PRODUTO

					If !oModelGR:IsEmpty()
						oModelGR:AddLIne()
					EndIf
					oModelGR:LoadValue("Z7G_ITEM"		, SZ7->Z7_ITEM						)
					oModelGR:LoadValue("Z7G_CODBAR"		, SZ7->Z7_CODBAR					)
					oModelGR:LoadValue("Z7G_PRODUTO"	, SZ7->Z7_PRODUTO					)
					oModelGR:LoadValue("Z7G_LOTE"		, SZ7->Z7_LOTE						)
					oModelGR:LoadValue("Z7G_CLI"		, SZ7->Z7_CLIENTE+'-'+SZ7->Z7_LOJA	)
					oModelGR:LoadValue("Z7G_VALID"		, SZ7->Z7_VALID						)
					oModelGR:LoadValue("Z7G_QTDE"		, SZ7->Z7_QTDE						)
					oModelGR:LoadValue("Z7G_PESO"		, SZ7->Z7_PESO						)
					oModelGR:LoadValue("Z6G_QTDVEN"		, SZ7->Z6_QTDVEN					)
					oModelGR:LoadValue("Z6G_UM"			, SZ7->Z6G_UM						)
				  	oModelGR:LoadValue("Z7G_NSEQ"		, SZ7->Z7_NSEQ						)
					oModelGR:LoadValue("Z7G_PED"		, SZ7->Z7_PEDIDO					)
		...
		For nI := 1 to oModelGM:GetQtdLine()
			oModelGM:GoLine(nI)
				If oModelGM:GetValue("Z6G_UNSVEN")>0
					cCod := oModelGM:GetValue("Z6G_PRODUTO")
					nPeso_ := Posicione("SB1",1,xFilial("SB1")+cCod,"B1_PESO")
					If nPeso_ > 0 .And. Empty(oModelGM:GetValue("Z6G_OMAVOL")) .and. oModelGM:GetValue("Z6G_QTDLIB") == 0 
...
Static Function CEFATBL()
	...
	oModelCab:LoadValue("CBC_ENTREGA"	,dDataBase)
		...
		PutMv("MV_AUTODES",SubStr(cUsuario,7,15))
			...
			If MsgYesNo('Gravar numero automatico da NF no Pré Pedido ?', 'Atenção')
				If MsgYesNo('Gravar numero automatico da NF no Pré Pedido ?', 'Atenção')
					...
					If lower(cUserName) $ 'bernardo,ioliveira'
						MemoWrite(StrTran(cArquivo,".xml","")+"_LancamentoDespacho_.sql" , _cQry)
					EndIf
					...
					Do While !(_cAlias)->(EOF())
						...
						cGrpProd := AllTrim(Posicione('SB1',1,xFilial('SB1')+(_cAlias)->Z6_PRODUTO, 'B1_GRUPO'))
							...
							If (_cAlias)->Z5_EMPRESA == 'B' //(((_cAlias)->Z5_EMPRESA <> 'R' .AND. (_cAlias)->Z5_EMPRESA <> 'P')  .AND. cGrpProd == 'UNRI' ) .or. ((_cAlias)->Z5_EMPRESA <> 'R' .and. (_cAlias)->Z5_EMPRESA <> 'P' )
...
Static Function CEFATBVol1()
	...
	Local oModel		:= FWModelActive()
	Local oModelGM 		:= oModel:GetModel('FORMGridM')
	Local nlinha		:= oModelGM:GetQtdLine()
	Local nI
	Local aSaveLines 	:= FWSaveRows()
        ...
        U_PrintSX1(cPerg)
		   If Len(Directory(cArquivo + "*.*","D")) == 0
				...
				ELSE
					...
					MsgAlert('Não foi possÃ­vel criar o Diretório. Erro', CValToChar(FError()), 'Aviso')
				EndIf
        	EndIf
		lRet := .T.
	EndIf
...
Static Function CEFATBD()
	...
	if !oModelGM:IsEmpty()
		...
		If len(aVVCorte) > 0
			cMsgCorte := 'OS PEDIDOS ABAIXO SÃƒO DO TIPO VENDA Ã€ VISTA E NÃƒO PODEM SOFRER CORTES:' + CRLF + CRLF
		...
		If nTamKit > 0
			cMsgCorte := 'OS PEDIDOS ABAIXO SÃƒO DO TIPO KIT E NÃƒO PODEM SOFRER CORTES:' + CRLF + CRLF
...
User Function VLDLIB() 
	...
	if !Empty(oModelGM:GetValue("Z5G_NUM"))
 		...
 		If !Empty(oModelGM:GetValue("Z6G_LOTEVIF"))
...
/* 	if Empty(AllTrim(cLoteZ6))
	...
	SZ7->(DbSetOrder(1))
	//SZ7->(RECLOCK('SZ7',.T.))
	IF  PROCNAME(1) != 'FSZ6LOK' .and. PROCNAME(1) != 'FSZ7LOK' 
		SZ7->(RECLOCK('SZ7', lRecLock := !DbSeek(xFilial('SZ7')+;
										SubStr(cLoteZ6,1,50)+;
										SubStr(cProduto,1,15)+;
										SubStr(cPed,1,29)+;
										SubStr(cItem,1,02)+;
										SubStr(cCLI,1,06)+;
										SubStr(cLoj,1,02)+;
	...
	ENDIF
	if !oModelGR:IsEmpty()
		for nI := 1 to oModelGR:GetQtdLine()
			oModelGR:GoLine(nI)
			//If !oModelGR:IsDeleted()
				If oModelGR:GetValue("Z7G_PED") 	 == oModelGM:GetValue("Z5G_NUM") 		.And. ;
					oModelGR:GetValue("Z7G_CODBAR")  == cLoteZ6 							.And. ;
					oModelGR:GetValue("Z7G_PRODUTO") == oModelGM:GetValue("Z6G_PRODUTO") 	.And. ;
					oModelGR:GetValue("Z7G_ITEM") 	 == oModelGM:GetValue("Z6G_ITEM") 

					cCodBar	:= oModelGR:GetValue("Z7G_CODBAR")
					cLote 	:= oModelGR:GetValue("Z7G_LOTE")
					cCod    := oModelGR:GetValue("Z7G_PRODUTO")
					cPed    := oModelGR:GetValue("Z7G_PED")
					...
					dData := StoD(If(Len(AllTrim(oModelGR:GetValue("Z7G_CODBAR"))) >= 31,SubStr(oModelGR:GetValue("Z7G_CODBAR"),28,4)+SubStr(oModelGR:GetValue("Z7G_CODBAR"),26,2) + SubStr(oModelGR:GetValue("Z7G_CODBAR"),24,2),""))
				/* 	oModelGR:SetValue("Z7G_VALID", dData) */

					If SZ7->(DbSeek(xFilial('SZ7')+SubStr(cCodBar,1,50)+SubStr(cCod,1,15)+SubStr(cPED,1,29)+SubStr(cITEM,1,02)+SubStr(cCLI,1,06)+SubStr(cLOJ,1,02)+cSEQ))
						If oModelGR:IsDeleted()
								SZ7->(RECLOCK('SZ7',.F.))
									SZ7->(DbDelete())
								SZ7->(MSUNLOCK())
							/* 	GravaZ24(cCOD,cCodBar,cLOTE,cPED,nPeso,nQtde,.T.) */
						ELSE
							SZ7->(RECLOCK('SZ7',.F.))
								SZ7->Z7_FILIAL  := xFilial('SZ7')
								...
								SZ7->Z7_QTDE    := nPeca
								SZ7->Z7_PESO    := nPeso
								SZ7->Z7_PEDIDO  := cPed  // AllTrim(oModelGM:GetValue("Z5G_NUM")) 
								SZ7->Z7_ITEM    := cItem //AllTrim(oModelGM:GetValue("Z6G_ITEM"))
								SZ7->Z7_CLIENTE := cCli
								SZ7->Z7_LOJA    := cLoj
								SZ7->Z7_NSEQ	:= nLSeq
								/* SZ7->Z7_NCS 	:= cNcs */
...
/*                      else
						If !oModel]R:IsDeleted()
							SZ7->(RECLOCK('SZ7',.T.))
								...
								SZ7->Z7_PEDIDO  := cPed  // AllTrim(oModelGM:GetValue("Z5G_NUM"))
								SZ7->Z7_ITEM    := cItem //AllTrim(oModelGM:GetValue("Z6G_ITEM"))
								SZ7->Z7_CLIENTE := cCli //SubStr(oModelGM:GetValue("Z5G_CLI"),1,6)
								SZ7->Z7_LOJA    := cLoj //SubStr(oModelGM:GetValue("Z5G_CLI"),8,2)
								SZ7->Z7_NSEQ	:= nLSeq
								SZ7->Z7_PALLET	:= oModelCab:GetValue("CBC_PALLET")
							SZ7->(MSUNLOCK())
						EndIf  */
	...
	for nI := 1 to oModelGM:GetQtdLine()
        cLeg := LOTECLR(oModel,oModelGM)
        oModelGM:GoLine()
	    oModelGM:SetValue("Z6G_LEGEND",cLeg)
    NEXT
...
/* 				cQry:= " SELECT * FROM "+RetSqlName("SA5")+" A5 "
				...
				If AllTrim(AllTrim(oModelGM:GetValue("Z6G_PRODUTO")))<>AllTrim(cProduto)
					cTipe :='N'
					cTit  :='Mensagem'
...
/* 						nPsMin := SB1->B1_PESOMIN * nQtdp
					...
					IF SUBSTR(oModelGM:GetValue("Z5G_CLI"),1,6)+SUBSTR(oModelGM:GetValue("Z5G_CLI"),8,2)  == ALLTRIM(cCliCFGru)
						...
						If ALLTRIM(cMEN1) <> ""
							...
							oModelGM:SetValue("Z6G_LOTEVIF", PadR('', TamSX3('Z6_LOTEVIF')[1]))
							Return(.F.)
						ENDIF
					ENDIF
					if lRet 
						...
						If nQtd2 > oModelGM:GetValue("Z6G_UNSVEN")
							IF MSGYESNO('Quantidade total maior que quantidade do pedido ' +CRLF+;
									...
									'Qtde Despacho : ' +LTRIM(TransForm(nQtd2 									, '@E 999,999,999.999'))+CRLF+;
									'Diferença    : '  +LTRIM(TransForm(nQtd2 - oModelGM:GetValue("Z6G_UNSVEN") , '@E 999,999,999.999'))+CRLF+;
									'Aceita ?' )
								...
								oModelGR:LoadValue("Z7G_VALID"		, IIF(Len(cValidade)>0,sToD(cValidade),''))
								...
								/* oModelGM:LoadValue("Z6G_LOTEVIF"  , cLoteVif) */
								U_FATBGRV()
							ELSE 
								RETURN(.F.)
							ENDIF
	...
	if oModelGM:GetValue("Z6G_QTDLIB2") >= oModelGM:GetValue("Z6G_UNSVEN")
		oModelGM:GoLine(oModelGM:GetLine() + 1 )
		oModelGM:LoadValue("Z6G_LOTEVIF"  , PadR('', TamSX3('Z6_LOTEVIF')[1])) 
	ELSE 
		oModelGM:LoadValue("Z6G_LOTEVIF"  , PadR('', TamSX3('Z6_LOTEVIF')[1]))
	endif
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	DEFINE MsDialog Senhadlg From 067,020 To 169,312 Title OemToAnsi("LiberaÃ§Ã£o de Acesso") Pixel Style DS_MODALFRAME
...
Static Function LOTECLR(oModel, oModelGM)
	...
	If !oModelGM:IsEmpty()/* .AND. Len( aLinha[nLinha] ) >= P1_VVISTA */
