...
_cQry := " WITH DADOS AS ( " + CRLF+;
		 ...
		 " 		  , DATEDIFF(day, (SELECT MIN(SB8A.B8_XDATACO) FROM SB8010 SB8A WHERE SB8A.B8_LOTECTL = SB8.B8_LOTECTL AND SB8A.B8_FILIAL = '01' AND SB8A.B8_SALDO > 0 AND SB8A.D_E_L_E_T_ <> '*'),  GETDATE()) AS DIAS  " + CRLF+;
		 ...
		 " 		  , (SELECT COUNT(Z06.Z06_TRATO)  FROM Z06010 Z06 WHERE Z06.D_E_L_E_T_ <> '*' AND Z06.Z06_FILIAL = '01' AND Z06.Z06_DATA = Z0R.Z0R_DATA AND Z06.Z06_VERSAO = Z0R.Z0R_VERSAO AND Z06.Z06_LOTE = SB8.B8_LOTECTL) AS NRTRT  " + CRLF+;
		 " 		  , (SELECT SUM(Z04.Z04_TOTREA)   FROM Z04010 Z04 WHERE Z04.Z04_DTIMP  = DATEADD(dd, -1, cast('" + dToS( __dDtPergunte ) + "' as datetime)) AND Z04.Z04_FILIAL = '01' AND Z04.D_E_L_E_T_ <> '*' AND Z04.Z04_LOTE = SB8.B8_LOTECTL) AS Z04_TOTREA  " + CRLF+;
		 " 		  , (SELECT Z05A.Z05_KGMNDI FROM Z05010 Z05A WHERE Z05A.Z05_DATA = DATEADD(DAY, -1, Z0R.Z0R_DATA) AND Z05A.Z05_VERSAO = Z0R.Z0R_VERSAO AND Z05A.Z05_LOTE = SB8.B8_LOTECTL AND Z05A.Z05_FILIAL = '01' AND Z05A.D_E_L_E_T_ <> '*') AS KGMN  " + CRLF+;
		 " 		  , (SELECT Z05A.Z05_KGMSDI FROM Z05010 Z05A WHERE Z05A.Z05_DATA = DATEADD(DAY, -1, Z0R.Z0R_DATA) AND Z05A.Z05_VERSAO = Z0R.Z0R_VERSAO AND Z05A.Z05_LOTE = SB8.B8_LOTECTL AND Z05A.Z05_FILIAL = '01' AND Z05A.D_E_L_E_T_ <> '*') AS KGMS  " + CRLF+;
		 " 		  , (SELECT Z05A.Z05_KGMNDI FROM Z05010 Z05A WHERE Z05A.Z05_DATA = Z0R.Z0R_DATA AND Z05A.Z05_VERSAO = Z0R.Z0R_VERSAO AND Z05A.Z05_LOTE = SB8.B8_LOTECTL AND Z05A.Z05_FILIAL = '01' AND Z05A.D_E_L_E_T_ <> '*') AS KGMNDIA  " + CRLF+;
		 " 		  , (SELECT Z05A.Z05_KGMSDI FROM Z05010 Z05A WHERE Z05A.Z05_DATA = Z0R.Z0R_DATA AND Z05A.Z05_VERSAO = Z0R.Z0R_VERSAO AND Z05A.Z05_LOTE = SB8.B8_LOTECTL AND Z05A.Z05_FILIAL = '01' AND Z05A.D_E_L_E_T_ <> '*') AS KGMSDIA  " + CRLF+;
		 " 	 FROM Z08010 Z08  " + CRLF+;
		 " 	 LEFT JOIN SB8010 SB8 ON SB8.B8_X_CURRA = Z08.Z08_CODIGO AND SB8.B8_FILIAL = '01' AND SB8.D_E_L_E_T_ <> '*' AND SB8.B8_SALDO > 0  " + CRLF+;
		 " 	 LEFT JOIN Z0O010 Z0O ON Z0O.Z0O_LOTE = SB8.B8_LOTECTL AND ('" + dToS( __dDtPergunte ) + "' BETWEEN Z0O.Z0O_DATAIN AND Z0O.Z0O_DATATR OR (Z0O.Z0O_DATAIN <= '" + dToS( __dDtPergunte ) + "' AND Z0O.Z0O_DATATR = '        ')) AND Z0O.Z0O_FILIAL = '01' AND Z0O.D_E_L_E_T_ <> '*'  " + CRLF+;
		 " 	 LEFT JOIN Z0R010 Z0R ON Z0R.Z0R_DATA = '" + dToS( __dDtPergunte ) + "' AND Z0R.Z0R_VERSAO = '0001' AND Z0R.Z0R_FILIAL = '01' AND Z0R.D_E_L_E_T_ <> '*'  " + CRLF+;
		 " 	 LEFT JOIN Z05010 Z05 ON Z05.Z05_DATA = Z0R.Z0R_DATA AND Z05.Z05_VERSAO = Z0R.Z0R_VERSAO AND Z05.Z05_LOTE = SB8.B8_LOTECTL AND Z05.Z05_FILIAL = '01' AND Z05.D_E_L_E_T_ <> '*'  " + CRLF+;
		 " 	 LEFT JOIN Z0T010 Z0T ON Z0T.Z0T_DATA = Z0R.Z0R_DATA AND Z0T.Z0T_VERSAO = Z0R.Z0R_VERSAO AND Z0T.Z0T_CURRAL = Z08_CODIGO AND Z0T.Z0T_FILIAL = '01' AND Z0T.D_E_L_E_T_ <> '*'  " + CRLF+;
		 " 	 WHERE Z08.D_E_L_E_T_ <> '*'  " + CRLF+;
		 " 	   AND Z08.Z08_FILIAL = '01' " + CRLF+;
...
DEFINE MSDIALOG oDlgRotas TITLE OemToAnsi("Rotas do Trato") From aPObjs[1][1], aPObjs[1][2] To aPObjs[1][3], aPObjs[1][4] of oDlgRotas PIXEL 
	...
	TSay():New(_nLin, 005, {|| "Currais Selecionados:"}, oDlgRotas,,oTFntLg,,,,.T., CLR_BLACK, CLR_WHITE, 200, 20)
 	...
 	tButton():New(010, (aPObjs[1][4]/2) - 620, "Sugerir Rotas"      , oDlgRotas, {|| nOpcRotas := 3, SugRotas(), oDlgRotas:End()}, 60, 15,,,, .T.) // "Cria\Recria Trato"
	...
	nColDieta := 1
 	For nCntAll := 1 To Len(aCrDie)
		...
		If nColDieta == 1
			TSay():New(nLinLeg, nColLeg-(100*3), &("{|| '" + ALLTRIM(cDscDie) + "'}"), oDlgRotas,,oTFntLgN,,,,.T.,;
						aCrDie[nCntAll][2], CLR_WHITE, 140, 15)
			nColDieta++
		ElseIf nColDieta == 2
			TSay():New(nLinLeg, nColLeg-(100*2), &("{|| '" + ALLTRIM(cDscDie) + "'}"), oDlgRotas,,oTFntLgN,,,,.T.,;
						aCrDie[nCntAll][2], CLR_WHITE, 140, 15)
			nColDieta++
		ElseIf nColDieta == 3
			TSay():New(nLinLeg, nColLeg-(100*1), &("{|| '" + ALLTRIM(cDscDie) + "'}"), oDlgRotas,,oTFntLgN,,,,.T.,;
						aCrDie[nCntAll][2], CLR_WHITE, 140, 15)
			nColDieta++
		Else
...
//			AAdd(aHdrRes, {"Descricao"    , "DSCDIE"     , ""                 , 40, 0, ""                      , "", "C", ""      , "R", "", "", "", "V"})
			...
			cQryRes := " with ROT as ( " + CRLF +;
					   ...
					   " 	from Z0S010 Z0S " + CRLF +;
					   " 	left join ZV0010 ZV0 on ZV0.ZV0_FILIAL = '" + FWxFilial("ZV0") + "' " + CRLF +;
...
While nComplet < nRegistros
	...
	If lContinua
		...
		_cQry := " SELECT R_E_C_N_O_ recno " + CRLF +;
			     " FROM Z0T010 " + CRLF +;
			     " WHERE Z0T_DATA   = '" + dToS( __dDtPergunte ) + "'" + CRLF +;
...
_cQryZ0S += "	     WHERE Z0T_DATA = '" + dToS( __dDtPergunte ) + "'  " + CRLF
...
Local cRet := ""
	...
	_cQry += " FROM Z0T010 " + CRLF
	_cQry += " WHERE Z0T_DATA='" + DtoS(dData) + "'  " + CRLF
...
Local nRet := 0
	...
	_cQry += "  FROM Z0T010 Z0T" + CRLF
	_cQry += "  JOIN Z05010 Z05 ON Z0T_FILIAL = Z05_FILIAL AND Z05_DATA = Z0T_DATA AND Z0T_CURRAL = Z05_CURRAL  AND Z05.D_E_L_E_T_ = ' '" + CRLF
	_cQry += " WHERE Z05_DATA = '" + DtoS(dData) + "' AND Z0T_ROTA = '" + cRota + "' AND Z0T.D_E_L_E_T_ = ' '" + CRLF
	TCQUERY _cQry NEW ALIAS "cTMP"
