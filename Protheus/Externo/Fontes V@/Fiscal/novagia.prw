...
Static Function a972MontTrab(aApuracao,lRelDIPAM,dDataIni,dDataFim,cNrLivro,nRegime,cFilDe,cFilAte,cVLayOut,lSelFil,lAglFil,lGerCR26,lGerCR27)
	...
	For n0 := 1 to Len(aSM0)
		...
		While (cAliasSF3)->(!Eof()) .and. xFilial("SF3")==(cAliasSF3)->F3_FILIAL
			...
			*/
			...
			If !lRelDIPAM
				...
				//Para ambiente TOP o tratamento deste trecho ?feito logo após o final da query principal
				//pois este laço da SF3 em ambiente TOP ?de maneira agrupada, e desta manera não temos como chegar no nível de item
				...
				If !lRelDIPAM .And. nRegime <> 4
					...
					If (cCfop1$"26" .and. cMvUF <> "SP") .or. (cCfop1$"26" .and. !(cUF$cCodUf) .and. cMvUF == "SP")
						...
						//Tratamento em DBF para geração do Registro CR=18. O tratamento em ambiente TOP ser?tratado posteriormente
						//devido uma alteração de melhoria de processamento, pois quando o processamento ?e top,
						//o loop da SF3 ?feito de forma agrupada, e como este registro necessito de informações no nível
			...
			If lRelDIPAM .OR. nRegime <> 4
					...
					Do Case
						...
						If ( (cAliasSF3)->F3_BASEICM + (cAliasSF3)->F3_ISENICM + (cAliasSF3)->F3_OUTRICM > 0 )
							...
							If !lRelDIPAM
								...
								Quando ponto de entrada retornar todas as saídas de determinado município que se enquadre nas regras dos registros 2.3,2.4 e 2.5, registro ser?preenchido com valor um real (R$ 1,00) no arquivo da Nova Gia na CR30.
								*/
								If n23ent > 0 
									VALORENT += n23ent  
								Else
									If ExistBlock("MA972VLR") 
										...
										Else
											VALOR += n23sai  // somente ?somavo valor no cArqDipam quando ponto não alterou seu valor para 1.
										Endif	
									Else
										VALOR += n23sai 
									Endif
							...
							*/
							If ExistBlock("MA972VLR") 
								...
								If nValor == 1
									...
									municipio não possui outras saidas que ponto não alterou*/
								Else // somente ?somavo valor no cArqDipam quando ponto não alterou seu valor para 1.
									VALOR += n25Sai 
								Endif	
							Else
								VALOR += n25Sai 
				...
				//Aqui serão tratados operações de exportações caso o ambiene seja DBF, caso for TOP, ser?tratado em uma query separada,
		...
		//este trecho do zona franca ?realizado aqui fora do loop da SF3, pois trata de informação a nível de documento
		...
		If lTop .AND. !lRelDIPAM	
			...
			//aqui ir?trazer form forma agrupada por CFOP, somente nota fiscal onde o campo do TES
			//estiver configurado para agregavr valor, irei buscar o CFOP na tabela de CFOP, se 
			//o CFOP estiver l? então o valor de PIS e COFINS ser?adicionado, caso contrário não ser?
...
Static Function a972Mestre(lAutomato)
	...
	cRegistro	+=	Iif(!lAutomato,dtos(dDataBase),"00000000")		     	//03.Data de geracao do Arq.Pre-formatado
	cRegistro	+=	Iif(!lAutomato,substr(Time(),1,2)+substr(Time(),4,2)+substr(Time(),7,2),"000000") //04. Hora da Geracao
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±?
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±?
±±ºFuncao    ³a972TmIeST ?Autor ³Diego Dias          ?Data ? 28/07/16  º±?
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±?
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±?
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±?
±±ºFuncao    ³a972TmIeSD ?Autor ³Diego Dias          ?Data ? 28/07/16  º±?
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±?
