...
Static nPFILOPERA	:= 6 // aScan(aHeader,{|x| AllTrim(x[2])=="ZAD_OPERAD"})
Static nPCODOPERA	:= 7 // aScan(aHeader,{|x| AllTrim(x[2])=="ZAD_OPERAD"})
...
Local cLstCpo    := "|ZAD_DATA  |ZAD_INICIO|ZAD_FINAL |ZAD_CC    |ZAD_CCDESC|ZAD_FILOPE|ZAD_OPERAD|ZAD_NOMEOP|ZAD_OBSERV"
	...
	If nOpcA == 1 // U_VAMNT001()
		For nI := 1 to Len(oGDados:aCols)
			...
			RecLock('ZAD', .T.)
				...
				ZAD->ZAD_FILOPE	:= oGDados:aCols[nI, nPFILOPERA]
				ZAD->ZAD_OPERAD	:= oGDados:aCols[nI, nPCODOPERA]
				ZAD->ZAD_OBSERV := oGDados:aCols[nI, 09]
			ZAD->(MsUnLock())
...
Local nFim  := Val(&(Iif(INCLUI,'M','ZAD')+'->ZAD_FINAL'))

	If Type('_CITEM') == "U" // cadastro unico
		nIni	:= Val(&(Iif(INCLUI,'M','ZAD')+'->ZAD_INICIO'))
	Else
		nIni 	:= Val( aCols[ Len(aCols), nPInicio ] )
	EndIf
...
// Default _cCampo	:= AllTrim(SubStr( ReadVar(),  At( "M->", ReadVar() ) + 3 ))
	...
	ElseIf _cCampo == 'ZAD_FILOPE'
		
		cRet := xFilial('ZAD') // Space(TamSX3('ZAD_FILOPE')[1])
		If Type('aCols') == 'A' .and. nLen > 1
			If !Empty(cRet := GdFieldGet('ZAD_FILOPE', nLen-1))
				aCols[ nLen, nPFILOPERA+1 ] := aCols[ nLen-1, nPFILOPERA+1 ]
			EndIf
		EndIf
...
Local lRet   := .F.
Local xFil	 := iIf( IsInCallStack("U_MNT001VA"), GdFieldGet('ZAD_FILOPE'), M->ZAD_FILOPE )

cQuery := " SELECT DISTINCT RA_FILIAL, RA_MAT, RA_NOME
cQuery +=   " FROM " + RetSqlName("SRA") + " SRA "
cQuery +=  " WHERE SRA.RA_FILIAL  = '" + xFil + "' "
...
dbUseArea(.T.,"TOPCONN",TcGenQry(,,cQuery),cAlias,.T.,.T.)
	...
	While (cAlias)->(!Eof())
		aAdd(aCpos,{(cAlias)->RA_FILIAL, (cAlias)->RA_MAT, (cAlias)->RA_NOME})
		(cAlias)->(dbSkip())
	End
	(cAlias)->(dbCloseArea())
...
DEFINE MSDIALOG oDlg TITLE /*STR0083*/ "Operadores" FROM 0,0 TO 240,500 PIXEL

@ 10,10 LISTBOX oLbx FIELDS HEADER 'Filial', 'Matricula', 'Nome' SIZE 230,95 OF oDlg PIXEL
...
DEFINE SBUTTON FROM 107,213 TYPE 1; 
			ACTION (oDlg:End(), lRet:=.T.,;
			aRet := {oLbx:aArray[oLbx:nAt,1],oLbx:aArray[oLbx:nAt,2], oLbx:aArray[oLbx:nAt,3]});
			ENABLE OF oDlg
ACTIVATE MSDIALOG oDlg CENTER
	...
	If Len(aRet) > 0 .And. lRet
		...
		Else
			SRA->(dbSetOrder(1))
			SRA->(dbSeek(xFilial("SRA")+aRet[2]/* +aRet[1] */))
		EndIf
	EndIf
Return lRet
//Return lRet := SRA->RA_FILIAL==M->ZAD_FILOPE .AND. !(SRA->RA_SITFOLH $ "D")
