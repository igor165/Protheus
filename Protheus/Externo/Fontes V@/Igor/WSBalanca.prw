...
Else
    ...
    Else
        ...
        oObjeto := PlacaVeiculo():New( 0,;
                                    placaTGet,;
                                    AllTrim(ZPB->ZPB_CODIGO),;
                                    AllTrim(ZPB->ZPB_DATA  ),;
                                    AllTrim(ZPB->ZPB_NOMFOR),;
                                    AllTrim(ZPB->ZPB_LOJFOR),;//CPF ???????
                                    AllTrim(ZPB->ZPB_NOMMOT),;                                   
                                    AllTrim(ZPB->ZPB_CPFMOT),;                                   
                                   // AllTrim(ZPB->ZPB_),; CELULAR ????????                                  
                                    AllTrim(ZPB->ZPB_PLACA ),;
                                    AllTrim(ZPB->ZPB_PRODUT),;
                                    AllTrim(ZPB->ZPB_STATUS),;
                                    AllTrim(ZPB->ZPB_LOCAL ),;
                                    //AllTrim(ZPB->ZPB_),; // PORTEIRO ????????????
                                    "",;
                                    "",;
                                    dToC(dDataBase) + ' ' + Time() )
...
User Function WSEnviaPlaca(agendamento_id, agendamento_data, fornecedor_nome, fornecedor_cnpj_cpf,motorista_nome,;
                            motorista_cpf, motorista_phone, placa, produto_id, status, departamento_id, empresa_id, porteiro_id ) 
    Local aArea       := GetArea()
    Local aHeader     := {}
    ...
    _aJSon := '{ "agendamento_id" : "'+agendamento_id+;
             '", "agendamento_data": "'+agendamento_data+;
             '", "fornecedor_nome": "'+fornecedor_nome+;
             '", "fornecedor_cnpj_cpf": "'+fornecedor_cnpj_cpf+;
             '", "motorista_nome": "'+motorista_nome+;
