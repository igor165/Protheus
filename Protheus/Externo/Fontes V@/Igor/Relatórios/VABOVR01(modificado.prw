...
Rotina respons�vel por listar o saldo diario de cada era.
...
Static function callParam(lFirst)
	...
	DEFINE MSDIALOG oDlgWnd TITLE OemToAnsi( "Informe os parametros" ) From 0,0 TO 200,800 OF GetWndDefault() STYLE DS_MODALFRAME STATUS  PIXEL
		...
		tSay():New(037, 100,{||'At�: ' },oDlgWnd,,oFont,,,,.T.,,,200,100)
		...
		oCombo1 := TComboBox():New(33,250,{|u|if(PCount()>0,cCombo1:=u,cCombo1)},;
											aItems,100,20,oDlgWnd,, /* {||Alert('Posi��o: ' + cValToChar(oCombo1:nAt))} ;
											,,,,.T.,,,,,,,,,'cCombo1')
		*/
		oBtnLtAtu  := TButton():New( 065, 005, "Lotes Atuais" 		, oDlgWnd, ;
						{|| obj := TExFilter():New("SB8",{"B8_LOTECTL", "B8_X_CURRA"}, "Lotes", , .F.,"B8_SALDO > 0" ,aLotes), ;
...
if nHandle = -1
	...
	If lTemDados
		...
		FWMsgRun(, {|| fQuadro1() },'Gerando excel, Por Favor Aguarde...', 'Gera��o do quadro de Lotes Analitico')
...
/*--------------------------------------------------------------------------------,
 ...
 | Obs.:  -                                                                       |
 '--------------------------------------------------------------------------------*/
Static Function GetSqlRel(cTipo, _cAlias, _cChave )
Local   _cQry 		:= ""
Default _cChave		:= ""
...
ElseIf cTipo=="Itens"
	...
	_cQry := " SELECT Z0F_LOTE, Z0F_CURRAL, ZBC_PEDIDO, Z0F_DTPES, ZCC_NOMFOR, Z0F_PROD, " +CRLF
 	_cQry += "        Z0F_SEQ, Z0F_PESO, Z0F_RACA, Z0F_HRPES, Z0F_LOTORI, Z0F_DENTIC, Z0F_TAG " +CRLF
	_cQry += " FROM Z0F010 Z0F " +CRLF
	_cQry += " JOIN ZBC010 ZBC ON ZBC_FILIAL = Z0F_FILIAL AND ( ZBC_PRODUT = Z0F_PROD OR Z0F_PRDORI = ZBC_PRODUT) AND ZBC.D_E_L_E_T_ = ' ' " +CRLF
	_cQry += " JOIN ZCC010 ZCC ON ZBC_FILIAL+ZBC_CODIGO = ZCC_FILIAL+ZCC_CODIGO AND ZCC.D_E_L_E_T_ = ' ' " +CRLF
	_cQry += " WHERE " +CRLF
...
Local _cLotes		:= ""
Local nJ            := 0
Local nTotComp 		:= 0 // Total Composi��o
...
/* 		cXML := '<Worksheet ss:Name="' + U_FrmtVlrExcel(cWorkSheet+' '+AllTrim(aLotes[nPos, 1]))+ '">' + CRLF
		...
		cXML += '<Table  x:FullColumns="1" x:FullRows="1" ss:DefaultRowHeight="16">' + CRLF
		cXML += '<Column ss:AutoFitWidth="0" ss:Width="69" ss:Span="2"/>' + CRLF
		cXML += '<Column ss:Index="4" ss:AutoFitWidth="0" ss:Width="76.5"/>' + CRLF
		cXML += '<Column ss:Width="66.75"/>' + CRLF
		cXML += '<Column ss:Width="76.5" ss:Span="1"/>' + CRLF
		cXML += '<Column ss:Index="8" ss:Width="54.75"/>' + CRLF
		cXML += '<Column ss:Width="66.75"/>' + CRLF
		...
		cXML += '<Column ss:Width="58.5"/>' + CRLF
		cXML += '<Column ss:Width="57.75" ss:Span="2"/>' + CRLF
		cXML += '<Column ss:Index="19" ss:Hidden="1" ss:AutoFitWidth="0"/>' + CRLF
		...
		While !(_cAliasG)->(Eof())
			...
			if cLote != (_cAliasG)->Z0E_LOTE// +(_cAliasG)->Z0E_CURRAL+(_cAliasG)->Z0C_DTCRIA
				...
				cXML += '<Row ss:AutoFitHeight="0">' + CRLF
				cXML += '	<Cell ss:MergeAcross="'+Iif(nPosCombo==1,'15','8')+'" ss:StyleID="s65"><Data ss:Type="String">Composi��o</Data></Cell>' + CRLF
				cXML += '</Row>' + CRLF
		...
		For nJ := 1 to Len(aPedLotes)
			If !Empty( _cChave := aPedLotes[nJ, 1] + aPedLotes[nJ, 2] )
				...
				inclus�o de resumo por denti��o do lote
				*/
	...
	/* 					If lAux
					...
					While !(_cAliasI)->(Eof()) 
						// imprimir cabe�alho produto e pedido
						if cProduto <> (_cAliasI)->Z0F_PROD
							cProduto := (_cAliasI)->Z0F_PROD
 							...
 							If nTotSequ > 0
								If nTotSequ == 1
									cXML += '<Row ss:AutoFitHeight="0">' + CRLF
    								/* cXML += ' <Cell  ss:StyleID="s65"><Data ss:Type="String">Total</Data></Cell>' + CRLF */
									cXML += '<Cell ss:Index="14" ss:StyleID="s65" ss:Formula="=COUNTA(R[-1]C[-6])"><Data ss:Type="Number"></Data></Cell>' + CRLF
									cXML += '</Row>' + CRLF
								Else
									cXML += '<Row ss:AutoFitHeight="0">' + CRLF
    								/* cXML += ' <Cell  ><Data ss:Type="String">Total</Data></Cell>' + CRLF */
						...
						/* 	cXML += '<Row ss:AutoFitHeight="0"></Row>' + CRLF  */// PULAR LINHA
							...
							cXML += '  <Cell ss:StyleID="s65"><Data ss:Type="String">FORNECEDOR</Data></Cell>' + CRLF
 							If lAux
								lAux := .F.
								cXML += '  <Cell ss:Index="9" ss:StyleID="sSemDig" ss:Formula="=SUM(R[1]C:R[2000]C)"><Data ss:Type="Number"></Data></Cell>' + CRLF
							EndIf
							cXML += '  <Cell ss:StyleID="s65"><Data ss:Type="String">Sequencia</Data></Cell>' + CRLF
							cXML += '  <Cell ss:StyleID="s65"><Data ss:Type="String">Peso</Data></Cell>' + CRLF
							cXML += '  <Cell ss:StyleID="s65"><Data ss:Type="String">Raca</Data></Cell>' + CRLF
						...
						//EndIf
							...
							cXML += '  <Cell ss:StyleID="sHoraCurta"><Data ss:Type="DateTime">' + U_FrmtVlrExcel( U_HoraToExcel( /* cValToChar( */ (_cAliasI)->Z0F_HRPES),,.F. ) + '</Data></Cell>' + CRLF
...
/* 							If nTotSequ > 0
							If nTotSequ == 1
								cXML += '<Row ss:AutoFitHeight="0">' + CRLF
								cXML += '	<Cell ss:Index="9" ss:Formula="=COUNTA(R[-1]C[-6])"><Data ss:Type="Number"></Data></Cell>' + CRLF
								cXML += '</Row>' + CRLF
							Else
								cXML += '<Row ss:AutoFitHeight="0">' + CRLF
								cXML += '	<Cell ss:Index="9" ss:Formula="=COUNTA(R[-'+cValToChar(nTotSequ)+']C[-6]:R[-1]C[-6])"><Data ss:Type="Number"></Data></Cell>' + CRLF
								cXML += '</Row>' + CRLF
							EndIf	
							nTotSequ := 0
							EndIf */
...
/* 						cXML += '<Row ss:AutoFitHeight="0">' + CRLF
				...
				If nTotSequ == 1
					cXML += '<Row ss:AutoFitHeight="0">' + CRLF					
					/* cXML += ' <Cell ss:Index="13" ss:StyleID="s65"><Data ss:Type="String">Total</Data></Cell>' + CRLF */
					cXML += '<Cell ss:Index="14" ss:StyleID="s65" ss:Index="13" ss:StyleID="s65" ss:Formula="=COUNTA(R[-1]C[-6])"><Data ss:Type="Number"></Data></Cell>' + CRLF
					cXML += '</Row>' + CRLF
				Else
					cXML += '<Row ss:AutoFitHeight="0">' + CRLF
					/* cXML += ' <Cell ss:Index="13" ss:StyleID="s65"><Data ss:Type="String">Total</Data></Cell>' + CRLF */
...
Fun��o que gera os parametros na tabela SX1.
@author Renato de Bianchi
@since 17/05/2018
@version 1.0
...
/*/{Protheus.doc} Excel
Fun��o responsavel por gerar um excel a partir de um vetor
@author Renato de Bianchi
@since 17/05/2018
@version 1.0
@return ${nil}, ${sem retorno}
	...
	if !empty(cArquivo)
		...
		If (CpyS2T(GetSrvProfString ("STARTPATH","")+cArqGer, Alltrim(GetTempPath())))
			...
			If !ApOleClient( 'MsExcel' )
				MsgAlert("O excel n�o foi encontrado. Arquivo " + cArqGer + " gerado em " + GetTempPath() + ".", "MsExcel n�o encontrado" )
			Else
				oExcelApp := MsExcel():New()
			...
			EndIf
		Else
			MsgAlert("N�o foi possivel criar o arquivo " + cArqGer + " no cliente no diret�rio " + GetTempPath() + ". Por favor, contacte o suporte.", "N�o foi possivel criar Planilha." )
		EndIf
...
Fun��o que gera o XML do Excel.
@author Renato de Bianchi
@since 17/05/2018
@version 1.0
@return ${cFileName}, ${Caminho completo do arquivo}
@param lCabec, logical, Indica se usa cabe�alho
@param aItens, array, Vetor com itens a serem impressos
@param aCabec, array, Vetor com cabe�alho
...
Static function geraExcel( lCabec, aItens, aCabec, cDirServer, cNomeArq )
	...
	Else
		MsgAlert("N�o foi possivel criar a planilha. Por favor, verifique se existe espa�o em disco ou voc� possui pemiss�o de escrita no diret�rio", "Erro de cria��o de arquivo")
		ConOut("N�o foi possivel criar a planilha no diret�rio")
	 Return()
	EndIf
...
Fun��o que gera celulas do excel.
@author Andre Cruz
@since 17/05/2018
@version 1.0
