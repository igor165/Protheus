...
User Function VAMVCA01()
	...
	oBrowse:SetDescription( 'Movimenta絥s de Bovinos' )
...
User Function Z0CLEG()
	Local aLegenda    := {}
	Private cCadastro := "Movimenta絥s de Bovinos"
...
Static Function MenuDef()
	...
	// ADD OPTION aRotina TITLE 'Efetivar movimenta磯'    ACTION 'u_CancMvBv()' OPERATION 8 ACCESS 0
...
User Function LimpaZ0D()
	...
	If Z0C->Z0C_STATUS <> '4' .or. Z0C->Z0C_TPMOV <> '2'
		msgAlert("Esta opera磯 s󠩠permitida para movimenta絥s efetivadas parcialmente do tipo Aparta磯.")
		Return
	EndIf
	...
	begin transaction
		...
		If (TCSqlExec(cUpd) < 0)
			...
			ConOut("Z0D: Removidos lotes de origem n㯠pesados com sucesso! ")
		EndIf
	...
	msgInfo("Res�os da movimenta磯 eliminados com sucesso.")
...
User Function AddZ0C()
	...
	If msgYesNo("Deseja inserir uma nova movimenta磯?")
		/***********************************************
...
/*
		If !Pergunte(cPerg,.T.)
			...
			/*If cAnaSin == "A" .and. empty(cEquip)
				msgAlert("Balan硠n㯠informada para movimenta磯 anal�ca, informe uma balan硠para prosseguir.")
				RestArea(aArea)
				Return
		EndIf*/
	...
	If !(cAliasQry)->(Eof())
		...
		If dbSeek(xFilial("SB1")+(cAliasQry)->C7_PRODUTO)
					...
					msUnlock()
		EndIf*/
		/*msgAlert("Movimento N.[" + cCodMovto + " ] inclu� com sucesso! Para editar seus dados, clique em alterar.")
	else
				msgAlert("N?mero do Pedido e Item n㯠encontrados.")
				RestArea(aArea)
				Return
	EndIf
			(cAliasQry)->(dbCloseArea())
...
Static function MrkLotes(lAdd)
	...
	If IsInCallStack( 'MATA103' )
		Alert("Esta opera磯 n㯠pode ser realizada neste modo de edi磯.")
		Return .F.
	EndIf
	...
	*/
	...
	aAdd(aHeadMrk,{ "Denti磯"  , "B1_XDENTIC"	, X3Picture("B1_XDENTIC")   , TamSX3("B1_XDENTIC")[1]	, 0, "AllwaysTrue()", X3Uso("B1_XDENTIC")	, "C", "", "V" } ) // ,"","","","V","","","" } )
	...
	define msDialog oDlgMrk title "Sele磯 de Lotes Animais" /*STYLE DS_MODALFRAME*/ From aSize[1], aSize[2] To aSize[3], aSize[5] OF oMainWnd PIXEL
	...
	nLinAtu += nTamLin + 5
	If !lAdd
		oSayFil := TSay():New(nLinAtu, 02+162 ,{||'Dados da movimenta磯'},oDlg,,,,,,.T.,,,100,30)
		oSayFil:SetCss("QLabel{ color: #000; font-weight: bold; font-size: 13pt; text-decoration: underline}")
		...
		nB := (nA+nPad) + nPad + 30
		aTpMov := {"1=Recebimento","2-Apartacao","3-Manejo","4-Enfermaria","5-Reclassifica磯"}
		cTpMov := aTpMov[2]
	...
	oBtMrk	:= TButton():New( nLinAtu-5, 02, "Inverter sele磯" ,oDlgMrk, {|| MarcaDes(oGetDadMrk,"T") },60, nTamLin+4,,,.F.,.T.,.F.,,.F.,,,.F.)
...
Static Function ConfirmAdd(lAdd)
	...
	for nI := 1 to len(oGetDadMrk:aCols)
		If oGetDadMrk:aCols[ nI,1]=="LBTIK"
			...
			If !u_vldPrdBv(oGetDadMrk:aCols[ nI,nPosProd],oGetDadMrk:aCols[ nI,nPosLote])
				msgAlert("Lote n.[" + oGetDadMrk:aCols[ nI,nPosLote] + " ] jᠥstᠳendo utilizado em outra movimenta磯")
				RestArea(aArea)
				Return
			EndIf
		EndIf
	...
	If lShowMsg
		msgInfo("Movimento N.[" + cCodMovto + " ] inclu� com sucesso! Para editar seus dados, clique em alterar.")
	EndIf
...
Fun磯 para inverter a sele磯 dos produtos selecionados pelo usuᲩo.
@author Renato de Bianchi
@since 13/07/2018
@version 1.0
@Return ${nenhum}, ${nenhum retorno}
@param oObj, object, Objeto de tela que terᠳua sele磯 invertida
...
User Function ProxProd()
	...
	//S󠰥rmite a efetiva磯 se a quantidade de origem e destino estiverem iguais
	nQtdOri          := oModel:GetValue("CALC_TOT","Z0D__TOT01")
	nQtdDes          := oModel:GetValue("CALC_TOT","Z0E__TOT02")
	If nQtdOri<=0
		alert('N㯠hᠯrigens informadas, por favor, informe uma origem para continuar.')
		Return .F.
	EndIf
/* 
...
User Function TgLotZ0E() // trigger
	...
	ElseIf nRegistros>1
		...
		msgAlert('O lote: '+AllTrim(clote)+' possui animais com data de entrada diferentes, informe os campos manualmente: PESO APARTAǃO, DATA DE INICIO.')
	EndIf
	TMP->(dbCloseArea())
...
Static Function ViewDef()
	...
	// Altera o Modelo de dados quer serᠵtilizado
	...
	oView:AddUserButton( 'Efetivar movimenta磯', 'WEB', {|oView| FWMsgRun(, {|| FZ0ELok(oModel) .AND. ProcGrid( oModel, oView ) }, "Processando", "Efetivando movimenta磯 ...") } )
...
Return oView
 ...
 |                                                                                 |
 |---------------------------------------------------------------------------------|
 | Regras   : funcao nao da certo, por definicao da rotina. s󠤡ria certo se o BOV for dIferente. |
...
Return cRet
 ...
 |             Tipo Recebimento s󠰯de ser Curral;                                |
 |---------------------------------------------------------------------------------|
 | Regras   :                                                                      |
 |                                                                                 |
 |---------------------------------------------------------------------------------|
...
User Function CurPastoOuBaia( cCurral )
	...
	If (lRet := Existcpo("Z08", cCurral))    // Existcpo("Z08",FwFldGet('Z0E_CURRAL'))
		If 	FWFldGet("Z0C_TPMOV") == "2" .AND. ;
			...
			// Aviso("Aviso", ;
			// 	  'A Tipo de movimenta磯 selecionada: APARTAǃO deve ser transferido para um CURRAL do tipo PASTO.', ;
			// 	  {"Sair"} )

			lRet:=MsgYesNo( 'Um tipo de movimenta磯 do tipo APARTAǃO n㯠deveria ser transferido para um CURRAL do tipo PASTO.' + CRLF + 'Deseja continuar ?' )
			EndIf
	Else
		MsgInfo("O Curral: " + AllTrim(cCurral) + " n㯠se encontra cadastrado.")
	EndIf
Return lRet
...
User Function NewLotes( __cCampo )
	...
	If oModel:nOperation == 4 .and. FWFldGet("Z0C_TPMOV") <> '4'
		...
		Else
			MsgInfo("O lote: " + cAux + " jᠳe encontra selecionado.")
		EndIf
	EndIf
...
Static Function FZ0DLok(oModel)
	...
	If nQtdOri < nQuant
		Alert("Quantidade de origem n㯠pode ser maior que o saldo atual dos produtos.")
		Return .F.
	EndIf
...
Static Function FVldTok(oModel, lHard)
	...
	for nI := 1 to len(aDestinos)
		...
		for nJ := 1 to len(aOrigens)
			If aDestinos[nI, 1] = aOrigens[nJ, 1]
				...
				If lHard
					...
					If aDestinos[nI, 2] > aOrigens[nJ, 2]
						msgAlert("A quantidade do produto [" + AllTrim(aDestinos[nI, 1]) + " ] no destino 頳uperior ࠱uantidade na origem.")
						Return .F.
					EndIf
				EndIf
			EndIf
		...
		If nOco = 0 .AND. !(/* FwFldGet('Z0C_TPMOV') */Z0C->Z0C_TPMOV $ "25") .and. !Empty(aDestinos[nI, 1])
			msgAlert("O produto [" + AllTrim(aDestinos[nI, 1]) + " ] no destino n㯠foi encontrado na origem.")
			Return .F.
		EndIf
	Next
...
Local lRet   := .T.
	...
	If Z0C->Z0C_TPMOV == '4' .AND.; // Aparta磯
		Empty( FwFldGet( 'Z0E_OBS' ) )

		MsgAlert("Campo observa磯 n㯠preenchido na linha: " + cValToChar(oModel:GetModel( 'Z0EDETAIL' ):nLine ))
		lRet := .F.
	EndIf
		...
		While lRet .AND. nI <= oGridZ0E:Length()
			...
			If !oGridZ0E:IsDeleted()
				If (FwFldGet('Z0D_PROD') == oGridZ0E:GetValue('Z0E_PROD', nI))
					Alert('O produto: '+FwFldGet('Z0D_PROD')+' esta sendo utilizado no destino (na linha ' + cValToChar(nI) + ') e por isso n㯠pode ser exclu� na tabela de origem.')
					lRet := .F.
				EndIf
			EndIf
			nI += 1
...
Static Function FZ0ELPre()
	...
	If FWFldGet('Z0E__TOT02')>=FWFldGet('Z0D__TOT01')
		alert('Jᠦoram informadas destinos suficientes para atender as origens especIficadas, verIfique as quantidades informadas.')
		Return .F.
	EndIf
Return(.T.)
...
Static Function ProcGrid( oModel, oView)
	...
	If oModel:nOperation <> 4
		msgAlert("Esta opera磯 n㯠pode ser realizada neste modo de edi磯.")
		Return .F.
	EndIf

	If !Z0C->Z0C_STATUS /*FWFldGet\("Z0C_STATUS"\)*/ $ '14'
		msgAlert('S󠩠poss�l efetivar movimenta絥s em aberto.')
		Return .F.
	EndIf
	...
	nQtdDes := oModel:GetValue("CALC_TOT","Z0E__TOT02")
	If nQtdOri<=0
		alert('N㯠hᠯrigens informadas favor, informe uma origem para continuar.')
		Return .F.
	EndIf
	...
	If FwFldGet('Z0C_TPMOV') != "2" //se nao for apartacao
		If nQtdOri<>nQtdDes
			alert('As quantidades de origem e destino est㯠diferentes, n㯠頰oss�l efetivar a movimenta磯.')
			Return .F.
		EndIf
	else
		If nQtdOri<nQtdDes
			alert('A quantidade de origem n㯠pode ser menor que a quantidade de destino, n㯠頰oss�l efetivar a movimenta磯.')
			Return .F.
		EndIf
		lHard := .F.
	EndIf
	...
	If empty(FWFldGet("Z0C_DATA"))
		alert('Informe uma data para a movimenta磯.')
		Return .F.
	EndIf

	If (FWFldGet("Z0C_DATA") > DATE())
		alert('Data para a movimenta磯 n㯠pode ser maior que a data atual.')
		Return .F.
	EndIf
...
For nI := 1 To oGridZ0E:Length()
	...
	Next nI
	If !Empty(cMsg2)
		msgAlert("Lotes jᠯcupados: " + cMsg + CRLF +;
			'Esta opera磯 serᠣancelada.')
		Return .F.
	EndIf
...
//TODO TOSHIO - VALIDAR CURRAL SE ESTA CORRETO
	...
	Next nI
	If !Empty(cMsg3)
		msgAlert("Lote alocado em outro curral, corrija a movimenta磯: " + cMsg + CRLF +;
			'Esta opera磯 serᠣancelada.')
		Return .F.
	EndIf
	...
	If (FWFldGet("Z0C_TPMOV")=='2' .OR. FWFldGet("Z0C_TPMOV")=='5')
		If (GetMV("VA_EFETZ0F",,.T.) .and. Z0C->Z0C_TPMOV/*FWFldGet\("Z0C_TPMOV"\)*/=='2') // aparta磯
							...
							If aOrigens[nJ,3] > 0
								...
								If lCriaBov
									if !(lTransf := !U_SB1Create( { xFilial("SB1"),;			// [01] Filial
										...
										oGridZ0E:GetValue('Z0E_RACA'  , nI),;   	// [05] Ra硍
...
/* MB : 04.06.2020
  ...
  頰ara garantir que mesmo em caso de queda do sistema, onde a Z0E poderia esta DESATUALIZADA, ele pega 
  para os casos de apartacao = 2, os dados atualizados na Z0F;

  vou mudar a logica, vou for硲 a atualizacao da Z0E antes de EFETIVAR a MOVIMENTAǃO;

	//Renato - 25/07/2019
	//Adicionada funcionalidade de efetiva磯 com base na Z0F
		If GetMV("VA_EFETZ0F",,.T.) .and. FWFldGet('Z0C_TPMOV')=='2'
			...
			If !QEFE->(Eof())
				...
				while !QEFE->(Eof())
					...
					If nPosOri = 0
						msgAlert("Nao foi possivel encontrar uma origem associada ao lan硭ento de pesagem" + ;
							" do produto [" + QEFE->Z0F_PROD + " ] lote [" + AllTrim(QEFE->Z0F_LOTORI)+;
							"] RecnoZ0F [" + cValToChar(QEFE->R_E_C_N_O_)+;
							"]. A efetiva磯 n㯠poderᠳer conclu�.")
						QEFE->(dbCloseArea())
						Return .F.
					EndIf
					...
					If nPosDes = 0
						msgAlert("Nao foi possivel encontrar um registro de destino associado ao lan硭ento" + ;
...
*/
	...
	*/
		...
		If !lTransf .or. len(aTransf)==0
			Alert("Movimenta磯 n㯠realizada devido a erros nos dados.")
			DisarmTransaction()
		Else
...
*/
			...
			/* MB : 31.03.2021
				NÏ realizar MEDIA PONDERADA para TPMOV $ ('25') */
			if !(Z0C->Z0C_TPMOV $ ('25'))
		...
		EndIf
	End Transaction // esta transan磯 deve acontecer antes do cursor abaixo.
	...
	If lTransf // .and. Z0C->Z0C_TPMOV $ ("23")
		...
		ConOut(Repl("-",80))
		__cMsg := "Movimenta絥s realizadas com sucesso."+CRLF+CRLF+;
				  "Tempo de processamento: " + ElapTime( cTimeINI, Time() )
		ConOut(__cMsg)
		ConOut(Repl("-",80))
		msgInfo(__cMsg, "Opera磯 conclu�")
	EndIf
...
Static Function doTransf(oModel, aTransf)
	...
	Local lOk           := .T.
	Local aItem         := {}
	Local nOpcAuto      := 3 // Indica qual tipo de a磯 serᠴomada (Inclus㯯Exclus㯩
	...
	If !SB1->(MsSeek(xFilial("SB1")+cProd))
		lOk := .F.
		Alert("Produto n㯠encontrado, feche a movimenta磯 e verIfique se o produto foi exclu�.")
	Else
		cProd 	:= SB1->B1_COD
		cDescri	:= SB1->B1_DESC
		cUM 	:= SB1->B1_UM
...
User Function CancMvBv()
	...
	Begin Transaction
		If Z0C->Z0C_STATUS = '3' .or. Z0C->Z0C_STATUS = '4'
			...
			If msgYesNo("Confirma o estorno?")
				for nL := 1 to len(aSequen)
					...
					If lRet
						...
						MsgInfo("Movimentos exclu�s com sucesso!", "OPERAǃO CONCLǗA")
					EndIf
				Next
			...
			EndIf
		Else
			Alert("N㯠hᠭovimentos efetivados para estornar.")
		EndIf
	End Transaction
	RestArea(aArea)
Return
...
Static Function undoTransf(cSequen)
	...
	// Local nOpcAuto      := 6 // Indica qual tipo de a磯 serᠴomada (Inclus㯯Exclus㯩
	...
	If !MYD3->(Eof())
		...
		//Else
		If lMsErroAuto
			msgInfo("Erro ao excluir movimenta絥s exclus㯡", "ATENǃO")
			MostraErro()
			DisarmTransaction()
		EndIf
		...
		lMsErroAuto := .T.
		msgInfo("Registros n㯠encontrados nas movimenta絥s de estoque (Tabela SD3).", "ATENǃO")
	EndIf
	MYD3->(dbCloseArea())
Return !lMsErroAuto
...
Static function MrkSeqs()
	...
	*/
	...
	lOk := .F.
	define msDialog oDlgMrk title "Sele磯 de Sequenciais a serem estornados" /*STYLE DS_MODALFRAME*/ From aSize[1], aSize[2] To aSize[3]/2, aSize[5]/2 OF oMainWnd PIXEL
	//oDlgMrk:lMaximized := .T. //Maximiza a janela
...
User Function vlSdOri()
	...
	If nTotOri <= 0
		Alert("Quantidade de origem n㯠pode ser 0 (Zero).")
		lRet := .F.
		Return lRet
	EndIf
...
User Function vlSdDest()
	...
	If Z0C->Z0C_TPMOV/*FWFldGet\("Z0C_TPMOV"\)*/=="5" .AND. Empty(cProduto)
		Alert('Campo Ra硠/ Sexo n㯠foram preenchido.')
		Return .F.
	EndIf

	//Verifica se a quantidade de origem estᠺerada
	If oModel:GetValue("CALC_TOT","Z0D__TOT01") <= 0
		Alert("Quantidade de origem estᠺerada, informe as origens antes dos destinos.")
		Return .F.
	EndIf
	...
	If nTotDest < 0
		Alert("Quantidade de destino n㯠pode ser negativa.")
		lRet := .F.
		Return lRet
	EndIf
	...
	If nTotDest > oModel:GetValue("CALC_TOT","Z0D__TOT01")
		Alert("Quantidade de destino n㯠pode ser maior que quantidade de origem.")
		lRet := .F.
		Return lRet
	EndIf
	...
	If nQtdAux < nQDest
		Alert("Quantidade de destino do produto [" + FwFldGet("Z0E_PROD") +;
			...
			" n㯠pode ser maior que quantidade de origem [ " +;
...
EndSQL
If (cAliasQry)->(Eof())
	alert("O produto informado n㯠existe.")
	lRet := .F.
EndIf
(cAliasQry)->(dbCloseArea())
...
If lRet
	...
	EndIf
	If !(cAliasQry)->(Eof())
		alert("Este produto" + iIf(!empty(cLote),"/lote "," ") + " jᠥstᠳendo transferido pela movimenta磯 [" + (cAliasQry)->CODIGO + " ] que estᠥm aberto.")
		lRet := .F.
	EndIf
	(cAliasQry)->(dbCloseArea())
EndIf
...
User Function vldLotBv(cLote, lSelf)
	...
	EndSQL
	If (cAliasQry)->(Eof())
		alert("O lote informado n㯠existe.")
		lRet := .F.
	EndIf
	(cAliasQry)->(dbCloseArea())
	...
	If lRet
		...
		EndSQL
		If !(cAliasQry)->(Eof())
			alert("Este lote [" + AllTrim(cLote) + " ] jᠥstᠳendo transferido pela movimenta磯 [" + (cAliasQry)->CODIGO + " ] que estᠥm aberto.")
			lRet := .F.
		EndIf
		(cAliasQry)->(dbCloseArea())
	EndIf
	...
	If aParam <> NIL
		...
		If cIdPonto == 'MODELVLDACTIVE'
			...
			If oObj:nOperation = 4 .or. oObj:nOperation = 5
				If Z0C->Z0C_STATUS=="3"
					ApMsgInfo('N㯠頰oss�l realizar opera絥s com movimenta絥s jᠥfetivadas.')
					Return .F.
				EndIf
				...
				If msgYesNo("Este registro encontra-se ativo, deseja continuar?")
					Help(NIL, NIL, "BLOQ. DE ACAO", NIL, "Esta movimenta磯 foi marcada como n㯠ativa", 1, 0, NIL, NIL, NIL, NIL, NIL, {"Acesse a movimenta磯 novamente para continuar."})
					RecLock("Z0C")
						Z0C->Z0C_ATIVO := .F.
					msUnlock()
				EndIf
			...
			If Z0C->Z0C_TPMOV='2' //Se Apartacao
				...
				EndSQL
				If !QRYATV->(Eof())
					Help(NIL, NIL, "BLOQ. DE ACAO", NIL, "A balan硠desta movimenta磯 jᠥstᠥm uso pela movimenta磯 [" + QRYATV->Z0C_CODIGO + " ]", 1, 0, NIL, NIL, NIL, NIL, NIL, {"Aguarde a finaliza磯 da pesagem em andamento."})
					QRYATV->(dbCloseArea())
					Return .F.
				EndIf
				QRYATV->(dbCloseArea())
...
static function Selecao(oModel, oView)
	...
	If Z0C->Z0C_TPMOV/* FwFldGet('Z0C_TPMOV') */ != "2"
		msgInfo('A Pesagem 頰ermitida apenas quando o tipo de movimenta磯 頢Aparta磯"')
		Return
	EndIf
	...
	*/
	...
	oSayTempo := TSay():New(nLinAtu, 10,{|| 'Processamento de Animais: (Aparta磯)  Tempo: '+cTempo+'  M餩a p/ animal: '+cMedia},oDlg,,,,,,.T.,,,500,10)
	...
	//nMilissegundos := 3000 // Disparo serᠤe 2 em 2 segundos
...
User Function ForceOri()
	...
	If nPosS > 0 .and. aSaldos[nPosS, 4] = aSaldos[nPosS, 3]
		msgInfo("O lote selecionado jᠦoi atendido totalmente nas pesagens anteriores, verIfique se foi selecionado o lote correto.")
		Return nil
	EndIf*/
				...
				If oGetDadRan:aCols[ nI, aScan( aHeadRan, { |x| AllTrim(x[2]) == "ZV2_PESINI"})] > 0 .and. oGetDadRan:aCols[ nI, aScan( aHeadRan, { |x| AllTrim(x[2]) == "ZV2_PESFIM"})] > 0
					...
					If oGetDadRan:oBrowse:nAt != nI
						If nPesIni >= oGetDadRan:aCols[ nI, aScan( aHeadRan, { |x| AllTrim(x[2]) == "ZV2_PESINI"})] .and. nPesIni <= oGetDadRan:aCols[ nI, aScan( aHeadRan, { |x| AllTrim(x[2]) == "ZV2_PESFIM"})]
							msgAlert("O Peso Inicial informado na linha [" + cValToChar(oGetDadRan:oBrowse:nAt) + " ] jᠥncontra-se informado na linha: " + cValToChar(nI))
							Return .F.
						EndIf
						...
						If nPesFim >= oGetDadRan:aCols[ nI, aScan( aHeadRan, { |x| AllTrim(x[2]) == "ZV2_PESINI"})] .and. nPesFim <= oGetDadRan:aCols[ nI, aScan( aHeadRan, { |x| AllTrim(x[2]) == "ZV2_PESFIM"})]
							msgAlert("O Peso Final informado na linha [" + cValToChar(oGetDadRan:oBrowse:nAt) + " ] jᠥncontra-se informado na linha: " + cValToChar(nI))
							Return .F.
						EndIf
						...
						If oGetDadRan:aCols[ nI, aScan( aHeadRan, { |x| AllTrim(x[2]) == "ZV2_PESINI"})] >= nPesIni  .and. oGetDadRan:aCols[ nI, aScan( aHeadRan, { |x| AllTrim(x[2]) == "ZV2_PESINI"})] <= nPesFim
							msgAlert("O Peso Inicial informado na linha [" + cValToChar(nI) + " ] jᠥncontra-se informado na linha: " + cValToChar(oGetDadRan:oBrowse:nAt))
							Return .F.
						EndIf
						...
						If oGetDadRan:aCols[ nI, aScan( aHeadRan, { |x| AllTrim(x[2]) == "ZV2_PESFIM"})] >= nPesIni .and. oGetDadRan:aCols[ nI, aScan( aHeadRan, { |x| AllTrim(x[2]) == "ZV2_PESFIM"})] <= nPesFim
							msgAlert("O Peso Final informado na linha [" + cValToChar(nI) + " ] jᠥncontra-se informado na linha: " + cValToChar(oGetDadRan:oBrowse:nAt))
							Return .F.
						EndIf
					EndIf
					...
					for nL := 1 to len(oGetDadRan:aCols)
						If nL != nI
							If oGetDadRan:aCols[ nI, aScan( aHeadRan, { |x| AllTrim(x[2]) == "ZV2_PESINI"})] >= oGetDadRan:aCols[nL, aScan( aHeadRan, { |x| AllTrim(x[2]) == "ZV2_PESINI"})] .and. oGetDadRan:aCols[ nI, aScan( aHeadRan, { |x| AllTrim(x[2]) == "ZV2_PESINI"})] <= oGetDadRan:aCols[nL, aScan( aHeadRan, { |x| AllTrim(x[2]) == "ZV2_PESFIM"})]
								msgAlert("O Peso Inicial informado na linha [" + cValToChar(nI) + " ] jᠥncontra-se informado na linha: " + cValToChar(nL))
								Return .F.
							EndIf
							...
							If oGetDadRan:aCols[ nI, aScan( aHeadRan, { |x| AllTrim(x[2]) == "ZV2_PESFIM"})] >= oGetDadRan:aCols[nL, aScan( aHeadRan, { |x| AllTrim(x[2]) == "ZV2_PESINI"})] .and. oGetDadRan:aCols[ nI, aScan( aHeadRan, { |x| AllTrim(x[2]) == "ZV2_PESFIM"})] <= oGetDadRan:aCols[nL, aScan( aHeadRan, { |x| AllTrim(x[2]) == "ZV2_PESFIM"})]
								msgAlert("O Peso Final informado na linha [" + cValToChar(nI) + " ] jᠥncontra-se informado na linha: " + cValToChar(nL))
								Return .F.
							EndIf
						EndIf
					Next
...
Static Function DefOrigem()
	...
	If !empty(cLoteOri)
		...
		If nPosS == 0
			msgAlert("N㯠existe saldo dispon�l para o lote de origem selecionado. Serᠵtilizado o pr󸩭o lote dispon�l.")
		else
...
User Function Registrar( oModel )
	...
	If nPeso < 0
		msgAlert("O peso informado n㯠pode ser negativo, realize a pesagem novamente.")
		Return nil
	EndIf

	If nQtdDes >= nQtdOri
		msgInfo("A quantidade informada nos lotes origem jᠦoi registrada. VerIfique se a quantidade de origem estᠣorreta.")
		Return nil
	EndIf
	...
	If empty(cLote)
		msgAlert("Peso n㯠se encaixa nos par䭥tros especIficados. Informe uma parametriza磯 de pesos vᬩda que permita a escolha do lote e curral adequado.")
		Return nil
	EndIf
...
User Function calcular_destino()
	...
	oSayTempo:SetText('Processamento de Animais: (Aparta磯)  Tempo: '+cTempo+'  M餩a p/ animal: '+cMedia)
...
User Function SalvarGeral( oModel, oView )
	...
	If Z0C->Z0C_STATUS /*FWFldGet\("Z0C_STATUS"\)*/ <> '1' .and. Z0C->Z0C_STATUS /*FWFldGet\("Z0C_STATUS"\)*/ <> '4'
		alert('S󠩠poss�l alterar movimenta絥s em aberto.')
		Return .F.
	EndIf
						...
						If oGridZ0D:GetValue('Z0D_PROD',nIZ0D) == oGridZ0E:GetValue('Z0E_PROD',nIZ0E)
							...
							If cChvZ0D <> cChvZ0E
								...
								If !(lErro := U_SB1Create( { xFilial("SB1"),;										// [01] Filial
									...
									oGridZ0E:GetValue('Z0E_RACA'  , nIZ0E),;   		// [05] Ra硍
...
Default nOpc        := 1
	...
	nPosZ0D       := aScan( oGridZ0D:aDataModel, { |x| x[1,1,6] == FwFldGet('Z0E_PROD') } )
		If nPosZ0D <= 0
		Alert("ERRO" + CRLF + "O Produto: "+FwFldGet('Z0E_PROD')+" n㯠foi localizado na Origem" )
		Return .F.
		EndIf
	...
	oGridZ0D:GoLine(nPosZ0D)
	   ...
	   ((LEFT(FwFldGet('Z0D_SEXO'), 1) == "F")  .AND. LEFT(FwFldGet('Z0E_SEXO'), 1) $ "CM")
		...
		Alert("ATENǃO" + CRLF + "Sexos invertidos n㯠sao permitidos" )
		lRet := .F.
			EndIf
	Else
	...
	nPosZ0D       := aScan( oGridZ0D:aDataModel, { |x| x[1,1,6] == GdFieldGet('Z0F_PROD') } )
		If nPosZ0D <= 0
		Alert("ERRO" + CRLF + "O Produto: "+GdFieldGet('Z0F_PROD')+" n㯠foi localizado na Origem" )
		Return .F.
		EndIf
