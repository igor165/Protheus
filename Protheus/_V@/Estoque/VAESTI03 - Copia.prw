...
/* 	if oCab:GetValue("ZMS_TIPO") $ 'P' .and. &(ReadVar()) > 1
	...
	if oCab:GetValue("ZMS_TIPO") $ 'M|A|P|' .and. &(ReadVar()) > 1
		nSaldo := Posicione("SB8",3,xFilial("SB8")+;
            ...
            For nI := 1 To nLinhas
                ...
                If !oGridM:isDeleted()
                    RecLock('ZMS', lRecLock := !DbSeek( xFilial("ZMS") +;
                        ...
                        ZMS->ZMS_DATA   	:= dData //oCab:GetValue('ZMS_DATA')	
					...
					if lRet
						...
						if cCampo == 'M->ZMS_PRDBOV'
							IF oCab:GetValue("ZMS_TIPO") == 'N'
								...
								SB1->(DbSeek(xFilial("SB1") + AllTrim(SB8->B8_PRODUTO)))	
...
User Function I03VPRDM()
	...
	ELSE
		IF oCab:GetValue("ZMS_TIPO") $ 'N|M|A' /* .and. lRet */
			...
			_cQry += " where ZMS_FILIAL = '"+xFilial("ZMS")+"'" + CRLF
