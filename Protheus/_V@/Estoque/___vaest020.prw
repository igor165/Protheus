...
for i := 1 to Len(aDados)
    if i == 1
        ...
        if aDados[i][03] != aDados[i-1][03]
            nBakCont += 1
            aAdd(aCopy,{aDados[i][01],;
                        aDados[i][02],;
                        aDados[i][03],;
                        aDados[i][04],;
                        aDados[i][05],;
            ...
            aAdd(aCopy[nBakCont][6],{   UPPER(aDados[i][06]),;
                                        aDados[i][10],;
                                        Val(aDados[i][08])})
        endif
    endif
...
for i := 1 to Len(aCopy)
    ...
    for x := 1 to Len(aCopy[i,6])
        if aCopy[i,6,x,3] <= 0
            aDel(aCopy[i,6],x)
            aSize(aCopy[i,6],len(aCopy[i,6]) - 1)
        endif
    next x
    ...
    if len(aCopy[i,6]) > 0
        ...
        U_GravaArq( iIf(IsInCallStack("U_JOBPrcLote"), cFile, ""),;
                        "Processando [VAEST020: ProcZ02]" + _ENTER_ + "Processando dados ["+ StrZero(i,5) + " de " + StrZero(Len(aCopy[i,6]),5) + ": " + AllTrim(aCopy[i,04]) +"]",;
                        .T./* lConOut */,;
                        /* lAlert */ )
        //Toshio                                                    /*  */               /**/                 /**/
                    //static function ProcLote(   cLote        , cRacao, nQuant, cArmzRac  ,  cArmz       )
        FWMsgRun(, {|| aAuxNumOp := ProcLote(   aCopy[i,04]    , aCopy[i,06]                , aDados[i,09] )},;
                            "Processando [VAEST020: ProcZ02]",;
                            "Processando dados ["+ StrZero(i,5) + " de " + StrZero(Len(aCopy[i,6]),5) + ": " + AllTrim(aCopy[i,04]) +"]")
        
        AAdd( aNumOp , aAuxNumOp )
        if !Empty(aNumOp)
            Z04->(DbSetOrder(1))
            if Z04->(DbSeek(xFilial('Z04')+cSequencia+aCopy[i,04]))
                while rTrim(xFilial('Z04')+cSequencia+aCopy[i,04]) == rTrim(Z04->(Z04_FILIAL+Z04_SEQUEN+Z04_LOTE))
                    RecLock('Z04', .f.)
