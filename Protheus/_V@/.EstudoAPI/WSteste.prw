...
WSRESTFUL zWSProdutos DESCRIPTION 'WebService Cadastro de Produtos'
    ...
    //Métodos
    WSMETHOD GET  ID  DESCRIPTION 'Retorna o registro pesquisado' WSSYNTAX '/zWSProdutos/get_id?{cUserName, cPassword}'     PATH 'get_id'        PRODUCES APPLICATION_JSON
...
WSMETHOD GET ID WSRECEIVE cUserName,cPassword WSSERVICE zWSProdutos
    Local lRet       := .T.
    Local jResponse  := JsonObject():New()
    Local cQry       := ""
    Local cALias     := GetNextAlias()

    cQry := "select * from SYS_USR where USR_CODIGO = '"+::cUserName+"'"
    
    MpSysOpenQuery(cQry, cAlias)

    if !(cALias)->(EOF())
        if (cAlias)->USR_MSBLQL == '2'
            PswOrder(1)
            If PswSeek((cAlias)->USR_ID)
                if PswName(::cPassword)
                    jResponse['Usuario'] := 'Valido'
                else
                    SetRestFault(500, 'Senha inválida') //caso queira usar esse comando, você não poderá usar outros retornos, como os abaixo
                endif
            EndIf
        else 
            SetRestFault(500, 'Usuario bloqueado') //caso queira usar esse comando, você não poderá usar outros retornos, como os abaixo
        endif
    else
        SetRestFault(500, 'Usuario nao existe') //caso queira usar esse comando, você não poderá usar outros retornos, como os abaixo
    endif
