...
//	_CDESCRI := IIF(EMPTY(_CDESCRI)," "+ALLTRIM(POSICIONE("ST9",1,XFILIAL("ST9")+FPP->FPP_FROTA,"T9_CODFA")),_CDESCRI)	// POSICIONE("ST9",1,XFILIAL("ST9")+FPP->FPP_FROTA,"T9_CODFA")),_CDESCRI) //T9_NOME
	IF EMPTY(_CDESCRI)
	...
	ELSE
		IF .F.  //DESABILITADO POR JOÃO CARLOS EM 15/06/2015
			...
			OOBJPRINT:SAY( NLIN  , 0920 , ALLTRIM(TRANSFORM(FPP->FPP_VALHOR,"@E 999,999.99"))           ,OFONT10 , 100 )
			OOBJPRINT:SAY( NLIN  , 1130 , ALLTRIM(TRANSFORM(FPP->FPP_TRSPES,"@E 999,999.99"))           ,OFONT10 , 100 )
			OOBJPRINT:SAY( NLIN  , 1270 , ALLTRIM(TRANSFORM(FPP->FPP_VLRMOB+FPP->FPP_VLRDES,"@E 9,999,999.99"))             ,OFONT10 , 100 )  
		 //	OOBJPRINT:SAY( NLIN  , 1480 , ALLTRIM(TRANSFORM(FPP->FPP_ANCORA+FPP->FPP_TELESC,"@E 9,999,999.99"))             ,OFONT10 , 100 )  	
			OOBJPRINT:SAY( NLIN  , 1480 , ALLTRIM(TRANSFORM(FPP->FPP_MONTAG+FPP->FPP_DESMON,"@E 9,999,999.99"))             ,OFONT10 , 100 )  
		 //	OOBJPRINT:SAY( NLIN  , 1880 , ALLTRIM(TRANSFORM(FPP->FPP_ACESSO,"@E 999,999.99"))            ,OFONT10 , 100 )
			OOBJPRINT:SAY( NLIN  , 1680 , ALLTRIM(TRANSFORM(FPP->FPP_VALSEG,"@E 999,999.99"))            ,OFONT10 , 100 )
			OOBJPRINT:SAY( NLIN  , 1880 , ALLTRIM(TRANSFORM(FPP->FPP_VLTOTM,"@E 999,999.99"))            ,OFONT10 , 100 )
			OOBJPRINT:SAY( NLIN  , 2070 , ALLTRIM(TRANSFORM(FPP->FPP_VALISS,"@E 999,999.99"))            ,OFONT10 , 100 )
			OOBJPRINT:SAY( NLIN  , 2220 , ALLTRIM(TRANSFORM(FPP->FPP_VLRTOT,"@E 999,999.99"))            ,OFONT10 , 100 )     

			OOBJPRINT:SAY( NLIN  , 2370+NCOL     , ALLTRIM(TRANSFORM(FPA->FPA_HRFRAQ,"@E 999,999.99"))  ,OFONT10 , 100 )     
			OOBJPRINT:SAY( NLIN  , 2520+NCOL     , ALLTRIM(TRANSFORM(FPA->FPA_VLHREX,"@E 999,999.99"))  ,OFONT10 , 100 )     
...
NLIN+=50
/*OOBJPRINT:SAY( NLIN  , 0060   , "TOT. HORA: "	    + ALLTRIM(TRANSFORM(NTOTHOR   , "@E 9,999.99"))	    + SPACE(04) +;
                                "TOT. MED HR  : "   + ALLTRIM(TRANSFORM(NTOTVLRHR , "@E 9,999,999.99"))	+ SPACE(04) +;
                                "TOT. KM  : "	    + ALLTRIM(TRANSFORM(NTOTKM    , "@E 9,999.99"))	    + SPACE(04) +;								
                                "TOT. MED KM  : "   + ALLTRIM(TRANSFORM(NTOTVLRKM , "@E 9,999,999.99"))	+ SPACE(04) +;								
								"TOT.MEDICAO R$ "	+ ALLTRIM(TRANSFORM(NTOTMED   , "@E 9,999,999.99"))	+ SPACE(04) +;
								"DESCONTO R$ "		+ ALLTRIM(TRANSFORM(NDESCON   , "@E 9,999,999.99"))	+ SPACE(04) +;
								"ACRESCIMO R$ "		+ ALLTRIM(TRANSFORM(NACRESC   , "@E 9,999,999.99"))	+ SPACE(04) +;
								"TOT.ISS R$ "		+ ALLTRIM(TRANSFORM(NTOTISS   , "@E 9,999,999.99"))	+ SPACE(04) +;
								"TOT.MO R$ "		+ ALLTRIM(TRANSFORM(NTOTMO    , "@E 9,999,999.99"))	+ SPACE(04) +;
								"TOT.REAL R$ "		+ ALLTRIM(TRANSFORM(NTOTTOT   , "@E 99,999,999.99"))+ SPACE(04) +;
								_CDESCRI	,OFONT10 , 100 )*/
...
//								"TOTAL MO   R$"		+ TRANSFORM(NTOTMO    , "@E 99,999,999.99")	+ SPACE(04) +;
//								"FROTA FANTASIA"    + _CDESCRI			  ,OFONT10 , 100 )
...
NLIN+=50
/*OOBJPRINT:SAY( NLIN  , 0060  ,"TIPO ISS: " 	+ CTPISS	+ SPACE(20) +;
								"% ISS: " 		+ TRANSFORM(CPERISS , "@E 99.99")	+ SPACE(20) +;
								"TIPO SEGURO: " + CTPSEGURO  						+ SPACE(20) +;
								"% SEGURO: " 	+ TRANSFORM(CPERSEGU,"@E 999.999")	+ SPACE(20) +;
								"% MO: "		+ TRANSFORM(CPERMO,"@E 999.99")		+ SPACE(20) +;
								"FORMA DE PAGAMENTO: " +  POSICIONE("SE4",1,XFILIAL("SE4")+CCONDPAG,"E4_DESCRI")        ,OFONT10 , 100 ) */
...
WHILE FP6->(!EOF() .AND. FP6_FILIAL+FP6_PROJET+FP6_OBRA==XFILIAL("FP6")+FPN->FPN_PROJET+FPN->FPN_OBRA)
	...
	ENDIF
	/*
	IF FP6->(FP6_XTIPOC<>"M")  //TIPO DE COBRANÇA (M=MENSAL,E=ESPORADICO)
		FP6->(DBSKIP())     // COMENTADO POR LUCAS 30/12/2015 POIS NÃO EXISTE O CAMPO
		LOOP
	ENDIF
	*/

/* ---------------- CONTEÚDO COLOCADO EM COMENTÁRIO POIS NÃO EXISTEM OS CAMPOS!
	IF FP6->(FP6_XPREVI>FPN->FPN_DTFIM)  //PREVISÃO DE CONSUMO
		FP6->(DBSKIP())
		LOOP
	ENDIF
	FP6->(AADD(ATABZAA,{FP6_CODIGO,FP6_DESCRI, FP6_XQUANT, (FP6_VALCOB-FP6_XVLDES),ROUND(FP6_XQUANT*(FP6_VALCOB-FP6_XVLDES),2),FP6_PRODUT,POSICIONE("SB1",1,XFILIAL("SB1")+FP6_PRODUT,"B1_DESC")}))
*/
...
IF LTEMZAA
	...
	FOR NPOS:=1 TO LEN(ATABZAA)
		...
		//CDET1:=FPG->(TRANSFORM(ATABZAA[NPOS,3],"@E 999,999,999.99"))  // SIGALOC94-504 (CEC 29942 ) - Jose Eulalio - 14/09/2022 - Lui solicitou retirar todas chamadas da FPG
		OOBJPRINT:SAY( NLIN  , 1100  , CDET1 ,OFONT10 , 100 , , , 1 )
		//CDET1:=FPG->(TRANSFORM(ATABZAA[NPOS,4],"@E 999,999,999.99")) // SIGALOC94-504 (CEC 29942 ) - Jose Eulalio - 14/09/2022 - Lui solicitou retirar todas chamadas da FPG
		OOBJPRINT:SAY( NLIN  , 1500  , CDET1 ,OFONT10 , 100 , , , 1 )
		//CDET1:=FPG->(TRANSFORM(ATABZAA[NPOS,5],"@E 999,999,999.99"))	// SIGALOC94-504 (CEC 29942 ) - Jose Eulalio - 14/09/2022 - Lui solicitou retirar todas chamadas da FPG
	...
	//CDET1:=FPG->(TRANSFORM(NVALTOT,"@E 999,999,999.99"))  // SIGALOC94-504 (CEC 29942 ) - Jose Eulalio - 14/09/2022 - Lui solicitou retirar todas chamadas da FPG
...
LTEMZC1:=.F.
 ...
 // SIGALOC94-504 (CEC 29942 ) - Jose Eulalio - 14/09/2022 - Lui solicitou retirar todas chamadas da FPG
 /*
FPG->(DBSETORDER(1))
FPG->(DBSEEK(XFILIAL("FPG")+FPN->FPN_PROJET))
WHILE FPG->(!EOF() .AND. FPG_FILIAL+FPG_PROJET==XFILIAL("FPG")+FPN->FPN_PROJET)
	IF .F.//FPG->(!FPG_NRAS==FPN->FPN_AS)
		FPG->(DBSKIP())
		LOOP
	ENDIF
	IF FPG->(!FPG_COBRA=="S")  //COBRAR CLIENTE (S/N)
		FPG->(DBSKIP())
		LOOP
	ENDIF
	LTEMZC1:=.T.
...
//	OOBJPRINT:SAY( NLIN+50  , 0000    , REPLICATE("-",330+NAUX),OFONT10 , 100 )
	 ...
	 /* 
	NLIN+=70
	OOBJPRINT:SAY( NLIN  , 1000+NAUXO  , STR0035     ,OFONT05 , 100 ) //"CUSTOS EXTRAS"
	LIMP:=.F.    
	...
	FPG->(DBSETORDER(1))
	FPG->(DBSEEK(XFILIAL("FPG")+FPN->FPN_PROJET+cNRAS))
	//cNRAS:= FPG->FPG_NRAS
	WHILE FPG->(!EOF() .AND. FPG_FILIAL+FPG_PROJET==XFILIAL("FPG")+FPN->FPN_PROJET)
		IF .F.//FPG->(!FPG_NRAS==FPN->FPN_AS)
			FPG->(DBSKIP())
		...
		IF LVEZ
			LVEZ:=.F.
			CDET1:=STR0036 //"DESPESA"
			OOBJPRINT:SAY( NLIN  , NPOS001  , CDET1 ,OFONT05 , 100 )
			CDET1:=STR0037 //"DOC.ORIGEM"
			OOBJPRINT:SAY( NLIN  , NPOS002  , CDET1 ,OFONT05 , 100 )
			CDET1:=STR0038 //"DESC.PRODUTO"
			OOBJPRINT:SAY( NLIN  , NPOS003  , CDET1 ,OFONT05 , 100 )
			CDET1:=STR0031 //"QUANTIDADE"
			OOBJPRINT:SAY( NLIN  , NPOS004  , CDET1 ,OFONT05 , 100 , , , 1 )
			CDET1:=STR0033 //"VALOR TOTAL"
			OOBJPRINT:SAY( NLIN  , NPOS005  , CDET1 ,OFONT05 , 100 , , , 1 )
			CDET1:=STR0039 //"DATA"
			OOBJPRINT:SAY( NLIN  , NPOS006  , CDET1 ,OFONT05 , 100 )
			...
			IF NLIN >= 2300
				...
				OOBJPRINT:SAYBITMAP( 55, 20,CLOGO   , 690, 0280 ) //553 X 224
				NLIN:=NLIN +0050
				OOBJPRINT:SAY( NLIN  , 1000+NAUXO  , STR0040     ,OFONT05 , 100 ) //"OBSERVAÇÕES - CUSTOS EXTRAS"
				NLIN:=NLIN +0100
			ENDIF    
		ENDIF
	
		CDET1:=FPG->FPG_CUSTO
		OOBJPRINT:SAY( NLIN  , NPOS001  , CDET1 ,OFONT10 , 100 )
		CDET1:=FPG->FPG_DOCORI
		OOBJPRINT:SAY( NLIN  , NPOS002  , CDET1 ,OFONT10 , 100 )
		CDET1:=FPG->FPG_DESCRI
		OOBJPRINT:SAY( NLIN  , NPOS003  , CDET1 ,OFONT10 , 100 )
		CDET1:=FPG->(TRANSFORM(FPG_QUANT,"@E 999,999,999.99"))
		OOBJPRINT:SAY( NLIN  , NPOS004  , CDET1 ,OFONT10 , 100 , , , 1 )
		CDET1:=FPG->(TRANSFORM(FPG_VALOR,"@E 999,999,999.99"))
		OOBJPRINT:SAY( NLIN  , NPOS005  , CDET1 ,OFONT10 , 100 , , , 1 )
		CDET1:=FPG->(DTOC(FPG_DTENT))
		OOBJPRINT:SAY( NLIN  , NPOS006  , CDET1 ,OFONT10 , 100 )
	
		NVALTOT+=FPG->FPG_VALOR
		NVALGER+=FPG->FPG_VALOR
	...
	NLIN+=50
	CDET1:=STR0041 //"TOTAL CUSTOS EXTRAS"
	OOBJPRINT:SAY( NLIN  , NPOS003  , CDET1 ,OFONT05 , 100 )
	//CDET1:=FPG->(TRANSFORM(NVALTOT,"@E 999,999,999.99")) // SIGALOC94-504 (CEC 29942 ) - Jose Eulalio - 14/09/2022 - Lui solicitou retirar todas chamadas da FPG
	OOBJPRINT:SAY( NLIN  , NPOS005  , CDET1 ,OFONT05 , 100 , , , 1 )
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPROGRAMA  ³ MONTAMED  º AUTOR ³ IT UP BUSINESS     º DATA ³ 05/05/2007 º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDESCRICAO ³ CRIA TABELA TEMPORARIA CONTENDO DADOS PARA IMPRESSAO       º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUSO       ³ ESPECIFICO GPO                                             º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
...
CQUERY := ""
CQUERY += " SELECT FPN_PROJET,FPN_OBRA,FPN_COD,FPN_DTINIC,FPN_DTFIM,FPN_OS,FPN_TIPO, "
CQUERY += " FPN_HORAI,FPN_HORAF,FPN_HORTOT,FPN_FROTA,"
CQUERY += " FPN_PROJET,FPN_CLI,FPN_LOJA,FPN_CLINOM, " //,FPN_CLIINS ,FPN_CLIEND, FPN_CLICGC
CQUERY += " FPN_CLIMUN,FPN_CLIBAI,FPN_CLIEST,FPN_CLICEP,FPN_CLICON,FPN_CLIDEP,FPN_CLIEMA, "
CQUERY += " FPN_CLIDDD,FPN_CLITEL,FPN_CLIFAX,FPN_DATINC,FP1_ORIGEM,FP1_MUNORI,FP1_NOMORI "
CQUERY += " FROM "+ RETSQLNAME("FPN")  + " ZLF ," + RETSQLNAME("FP0")  + " ZA0,"+ RETSQLNAME("FP1")  + " ZA1"
CQUERY += " WHERE "
CQUERY += " FPN_PROJET = '"+MV_PAR01+"' AND "
CQUERY += " FPN_PROJET = FPN_PROJET AND "
CQUERY += " FPN_OBRA = '"+MV_PAR02+"' AND "
CQUERY += " FPN_OBRA = FP1_OBRA AND "
CQUERY += " FPN_COD = '"+MV_PAR03+"' AND "
CQUERY += " ZLF.D_E_L_E_T_ = ' ' AND ZA0.D_E_L_E_T_ = ' ' "
...
// ======================================================================= \\
    ...
    OOBJPRINT:SAY( 330+NLIN  , 1140  , TRANSFORM(FP0->FP0_CLICGC,"@R 99.999.999/9999-99")    ,OFONT10 , 100 )
...
//	OOBJPRINT:SAY( 0920+NLIN  , 1100+NAUXO  , "ORDENS DE SERVICO "       , OFONT05, 100 )
	OOBJPRINT:SAY( 0920+NLIN  , 1100+NAUXO  , STR0065, OFONT05, 100 ) //"MEDICAO CONTRATO/LOCACAO "
	 ...
	 //	OOBJPRINT:SAY(0970+NLIN  , 1480  , "ANC/TELE."                ,OFONT05 , 100 )                        	
		OOBJPRINT:SAY(0970+NLIN  , 1480/*1680*/  , STR0094                 ,OFONT05 , 100 )                        		 //"MON/DES."
	 //	OOBJPRINT:SAY(0970+NLIN  , 1860  , "CHUMBADOR"                ,OFONT05 , 100 )
