...
LOCAL   _lValida  := .T. // valida se pode entrar no módulo
...
If GetRPORelease() == "12.1.033"
	/*
	If !EXISTBLOCK("A100DEL")
		_lValida := .F.
	EndIF
	If !EXISTBLOCK("a103devol")
		_lValida := .F.
	EndIF
	...
	If !EXISTBLOCK("MNTA4206")
		_lValida := .F.
	EndIF
	If !EXISTBLOCK("MS520DEL")
		_lValida := .F.
	EndIF
	If !EXISTBLOCK("MS520VLD")
		_lValida := .F.
	EndIF
	...
	If TQY->(FIELDPOS("TQY_STTCTR")) == 0
		_lValida := .F.
	EndIF
	If STJ->(FIELDPOS("TJ_XAS")) == 0
		_lValida := .F.
	EndIF
	*/

	If !_lValida
		MsgAlert(STR0545,STR0523) //"Falta atualizar os pontos de entrada e os campos exclusívos do módulo."###"Atenção!"
		Return .F.
	EndIF
...
IF FP0->FP0_STATUS != "3" .AND. FP0->FP0_STATUS != "5" .AND. FP0->FP0_STATUS != "8"
	Help(Nil,Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	         Nil,STR0030,1,0,Nil,Nil,Nil,Nil,Nil,; //"Impressão de autorização de serviço não permitida."
			 {STR0031+SUPERGETMV("MV_LOCX248",.F.,STR0002) + STR0032}) //"O "###"Projeto"###" deve estar com status igual (3)Aprovado, ou (5)Fechado, ou (8)Faturado."
ELSE
	DO CASE
	...
	OTHERWISE
		Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	 				...
	 				{STR0034}) //"Não existe AS definida para este tipo de Serviço."
	ENDCASE
ENDIF
...
IF PSWADMIN(,,__CUSERID) == 0
	...
	//SIGALOC94-458 - Jose Eulalio - 16/03/2022 - Chamado 29839 - Menu Duplicado

	//AADD(AROTINA1     , {SUPERGETMV("MV_LOCX248",.F.,STR0002)                  , "LOCA00112"  , 0 , 6}) //"Projeto"
	//AADD(AROTINA1     , {STR0035                                           , "LOCA00113"  , 0 , 6}) //"Localidades"
	AADD(AROTINA1     , {STR0036                                                , "LOCA00114"  , 0 , 6}) //"Locais"
	//AADD(AROTINA1     , {STR0037                                                , "LOCA00115"  , 0 , 6}) //"Etapas"
	//AADD(AROTINA1     , {STR0038  + SUPERGETMV("MV_LOCX248",.F.,STR0002)   , "LOCA00116"  , 0 , 6}) //"Rotas  X "###"Projeto"
	//AADD(AROTINA1     , {STR0039  + SUPERGETMV("MV_LOCX248",.F.,STR0002)   , "LOCA00117"  , 0 , 6}) //"Guind. X "###"Projeto"
	//AADD(AROTINA1     , {STR0040  + SUPERGETMV("MV_LOCX248",.F.,STR0002)   , "LOCA00118"  , 0 , 6}) //"Transp.X "###"Projeto"
	//AADD(AROTINA1     , {STR0041  + SUPERGETMV("MV_LOCX248",.F.,STR0002)   , "LOCA00119"  , 0 , 6}) //"Cargas X "###"Projeto"
	//AADD(AROTINA1     , {STR0042  + SUPERGETMV("MV_LOCX248",.F.,STR0002)   , "LOCA00120"  , 0 , 6}) //"RAT    X "###"Projeto"
	//AADD(AROTINA1     , {STR0043  + SUPERGETMV("MV_LOCX248",.F.,STR0002)   , "LOCA00121"  , 0 , 6}) //"Custos X "###"Projeto"
	IF SUPERGETMV("MV_LOCX227",.F.,.T.)
		//AADD(AROTINA1 , {STR0044  + SUPERGETMV("MV_LOCX248",.F.,STR0002)   , "LOCA00122"  , 0 , 6}) //"Respon.X "###"Projeto"
		AADD(AROTINA1 , {STR0045                                     , "LOCA00123"  , 0 , 6}) //"Responsabilidades"
	ENDIF
	//AADD(AROTINA1     , {STR0046  + SUPERGETMV("MV_LOCX248",.F.,STR0002)   , "LOCA00124"  , 0 , 6}) //"M.O.   X "###"Projeto"
 //	AADD(AROTINA1     , {"LICIT. X "  + SUPERGETMV("MV_LOCX248",.F.,"PROJETO")   , "U_CADZAD"  , 0 , 6})
	//IF SUPERGETMV("MV_LOCX225",.F.,.T.)
	//	AADD(AROTINA1 , {STR0047+SUPERGETMV("MV_LOCX248",.F.,STR0002)   , "LOCA00125"  , 0 , 6}) //"Cj.Transp.X "###"Projeto"
	//ENDIF
	//AADD(AROTINA1     , {STR0048  + SUPERGETMV("MV_LOCX248",.F.,STR0002)   , "LOCA00126"  , 0 , 6}) //"Follow X "###"Projeto"
	//AADD(AROTINA1     , {STR0049  + SUPERGETMV("MV_LOCX248",.F.,STR0002)   , "LOCA00127"  , 0 , 6}) //"Grua   X "###"Projeto"
	//AADD(AROTINA1     , {"CONCORRENTES"                                          , "LOCA00128"  , 0 , 6})
	IF SUPERGETMV("MV_LOCX226",.F.,.T.)
	//	AADD(AROTINA1 , {STR0050  + SUPERGETMV("MV_LOCX248",.F.,STR0002)   , "LOCA00129"  , 0 , 6}) //"Doctos.X "###"Projeto"
		AADD(AROTINA1 , {STR0051                                            , "LOCA00130"  , 0 , 6}) //"Documentos"
	ENDIF
	//AADD(AROTINA1     , {STR0052+SUPERGETMV("MV_LOCX248",.F.,STR0002) , "LOCA00131"  , 0 , 6}) //"Acess.Gruas X "###"Projeto"
	//AADD(AROTINA1     , {"LOCA00132"                                                , "LOCA00132"  , 0 , 6})
	//AADD(AROTINA1     , {STR0053  + SUPERGETMV("MV_LOCX248",.F.,STR0002)   , "LOCA00133"  , 0 , 6}) //"Trecho X "###"Projeto"
	//AADD(AROTINA1     , {STR0054                                               , "LOCA00134"  , 0 , 6}) //"Escolta"
	//AADD(AROTINA1     , {STR0055                                            , "LOCA00135"  , 0 , 6}) //"Escolta SP"
...
IF CNIVEL >= 5
	...
	AADD(AROTINA      , {STR0071                                             , "LOCA00102"      , 0 , 6}) //"Emitir AS"
	...
	ENDIF
	AADD(AROTINA      , {STR0077                                                 , "LOCA00191"    , 0 , 6})  //"R.A.T"
...
PRIVATE CCADASTRO := STR0079 + SUPERGETMV("MV_LOCX248",.F.,STR0002) //"MANUTENÇÃO "###"PROJETO"
...
ELSE											// --> CASO NÃO EXISTA OU NÃO ESTEJA HABILITADO, CONSIDERA (EVENTUAL) P.E. 
	...
	IF LEN(CSELECREC) = 1 						// --> SÓ EXITE UMA RECEITA CONFIGURADA NO PARÂMETRO.
		...
		ELSE 
			Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	         			Nil,STR0084,1,0,Nil,Nil,Nil,Nil,Nil,; //"O módulo não está adequadamente configurado para o tipo de receita."
			 			{STR0085}) //"Utilize o tipo de receita Locação."
			RETURN .F. 
		ENDIF 
...
IF FP0->(DBSEEK(XFILIAL("FP0")+CPROJETNEW))
	Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	{STR0088+SUPERGETMV("MV_LOCX248",.F.,STR0002)+" "+ALLTRIM(CPROJETNEW)}) //"JÁ existe o "###"PROJETO"
...
IF !OFOLDER:NOPTION == NFOLDERGUI 
	Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0101}) //"Selecionar a aba Locação."
	RETURN .F. 
ENDIF 

// ALTERADO POR FRANK EM 31/08/2020 COMPATIBILIZACAO DE VERSÃO
//@ 0,0 TO 90,250 MSDIALOG ODLGQ TITLE "DUPLICA A ÚLTIMA LOCAÇÃO"
DEFINE MSDIALOG ODLGQ TITLE STR0102 FROM 0,0 TO 90,250 //OF ODLG3 PIXEL //"DUPLICA A ÚLTIMA LOCAÇÃO"
	@ 00,01 TO 03,15
	@ 001,002 SAY STR0103 //"Quantidade de linhas a duplicar:"
...
IF !LOK .OR. EMPTY(_NNUMDUP) .OR. !MSGYESNO(STR0104 + ALLTRIM(STR(_NNUMDUP)) + STR0105 , STR0097)  //"DUPLICA A ÚLTIMA LOCAÇÃO "###" VEZES ??"###"GPO - LOCC001.PRW"
	RETURN .F. 
ENDIF 
...
IF .T. //LDPLIT removido temporariamente a liberacao vinha de um parametro - Frank em 27/06/2022
	IF NOPC == 3 .OR. NOPC == 4
		AADD(ABUTTONS , {"ANALITIC",{|| LOCA080(FP0->FP0_PROJET) },OEMTOANSI(STR0108 ),OEMTOANSI(STR0108)}) //"Duplica Itens"###"Duplica Itens"
	ENDIF
ENDIF

IF ! FP0->FP0_TIPOSE $ "T" .AND. POPC == 2
...
IF FP0->FP0_TIPOSE == "L" .AND. FP0->FP0_TIPFAT == "L"
		AADD(ABUTTONS , {STR0111,{|| CONSLIFT() },OEMTOANSI(STR0112),OEMTOANSI(STR0112)}) //"OBJETIVO"###"Consulta LIFT"###"Consulta LIFT"
ENDIF
...
IF FWHEN()
    ...
    IF !FP0->FP0_TIPOSE $ "T" .AND. POPC <> 0
	...
	CASE FP0->FP0_TIPOSE == "E" .AND. POPC <> 0			// NÃO FOR FOLLOW-UP
		AADD(ABUTTONS , {"ANALITIC",{||FDUPLIGUI()} , OEMTOANSI(STR0118),OEMTOANSI(STR0119)}) //"Duplica Locação"###"Locação"
	CASE FP0->FP0_TIPOSE == "T" .AND. POPC <> 0			// NÃO FOR FOLLOW-UP
		// removido por Frank em 05/04/2022 por questoes dos debitos tecnicos
		//AADD(ABUTTONS , {"S4WB005N",{||IMPPLAN()}   , OEMTOANSI(STR0120   ),OEMTOANSI(STR0121)}) //"Packing List"###"Packing"
		AADD(ABUTTONS , {"ANALITIC",{||FDUPLITRA()} , OEMTOANSI(STR0123 ),OEMTOANSI(STR0124)}) //"Duplica Viagem"###"Dupl. Viagem"
	ENDCASE
ENDIF
...
//VALID(ODLG) // inserção da função de validação no botao salvar - Frank em 12/08/2021
...
IF EXISTBLOCK("LC001GA")
	DO CASE
	CASE FP0->FP0_TIPOSE == "T" .AND. VALTYPE(ODLGCAR) == "O"
		ODLGCAR:ACOLS := EXECBLOCK("LC001GA",.T.,.T.,{ODLGCAR:AHEADER, ODLGCAR:ACOLS, FP0->FP0_PROJET,_LALT})
	CASE ALLTRIM(FP0->FP0_TIPOSE) $ "E#G" .AND. VALTYPE(ODLGGUI) == "O"
		ODLGGUI:ACOLS := EXECBLOCK("LC001GA",.T.,.T.,{ODLGGUI:AHEADER, ODLGGUI:ACOLS, FP0->FP0_PROJET,_LALT})
	CASE ALLTRIM(FP0->FP0_TIPOSE) $ "L#P" .AND. VALTYPE(ODLGPLA) == "O"
...
IF NFOLDERRES > 0
	...
	//FSALVARRES("FP6",ACLONE(ODLGRES:AHEADER),ACLONE(ODLGRES:ACOLS)) // Frank 01/03/22
...
CASE NINDO == NFOLDERCUS						// ABA BASE DE CALCULO
	IF VALTYPE(ODLGCAR) == "O"  				// SE O OBJETO JÁ FOI CRIADO
		...
		IF !EMPTY(CJUNTO)
			...
			Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   		Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 		{STR0156+CSEQCAR+STR0157+CJUNTO}) //"A carga: "###" vai junto com a carga: "
...
CLABEL := STR0162 //" DIMENSÕES CONJUNTO TRANSPORTADOR "

NPOS   := 0
@ NLINS1+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0163) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Comprimento ====>"
@ NLINS1+(NLINA*(  NPOS-1)),NCOLS2 SAY OEMTOANSI(STR0164    ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Largura ====>"
@ NLINS1+(NLINA*(  NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0165     ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Altura ====>"
@ NLINS1+(NLINA*(  NPOS-1)),NCOLS4 SAY OEMTOANSI(STR0166       ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Peso ====>"
...
CLABEL := STR0167 //" DIMENSÕES CARGAS "
NPOS++
NPOSAUX := NPOS
...
CLABEL := STR0168 //" DIMENSÕES TOTAIS "
NPOS++
NPOSAUX := NPOS
NLINAUX := NLINS1+(NLINA*(NPOSAUX))
@ NLINS1+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0163) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Comprimento ====>"
...
LOCAL   AAREA      := GETAREA()
LOCAL   CTITULO    := STR0169 //"CONSULTA ROTAS"
LOCAL   LOK	   	   := .F.
LOCAL   ODLG3  	   := NIL
LOCAL   CORIANT
LOCAL   CDESANT
...
IF EMPTY( CORIGEM )
	Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   			Nil,STR0170,1,0,Nil,Nil,Nil,Nil,Nil,; //"Falta o preenchimento de informação."
	 			{STR0171 + IIF(FP0->FP0_TIPOSE=="T",STR0172,STR0035)}) //"Favor preencher o campo origem na aba: "###"Viagens"###"Localidades"
	RETURN .F.
ENDIF
...
IF EMPTY( CDESTIN )
	Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   			Nil,STR0170,1,0,Nil,Nil,Nil,Nil,Nil,; //"Falta o preenchimento de informação."
	 			{STR0173 + IIF(FP0->FP0_TIPOSE=="T",STR0172,STR0035)}) //"Favor preencher o campo destino na aba: "###"Viagens"###"Localidades"
	RETURN .F.
ENDIF
...
CCAB1 += STR0174  +CORIGEM+" - "+ALLTRIM(FP2->FP2_DESCRI)+" / "+FP2->FP2_ESTADO+SPACE(10) //"ORIGEM: "
POSICIONE("FP2" , 1 , XFILIAL("FP2")+CDESTIN , "FP2_DESCRI") 
CCAB1 += STR0175 +CDESTIN+" - "+ALLTRIM(FP2->FP2_DESCRI)+" / "+FP2->FP2_ESTADO //"DESTINO: "
...
DEFINE MSDIALOG ODLG3 TITLE CTITULO FROM 000,000 TO 550,758 OF ODLG3 PIXEL
	...
	@ 235,060 BUTTON OBUTTETA PROMPT STR0176 SIZE 60,10 ACTION (LOK:=.T.,CROTATRB:=ACAMPOSITE[OLBXITENS:NAT,01],CLOSE(ODLG3),) PIXEL //"Seleciona Rota"
	@ 235,130 BUTTON OBUTTETA PROMPT STR0177   SIZE 60,10 ACTION (IIF(FINCLUIETA(CORIGEM,CDESTIN,ACAMPOSITE[OLBXITENS:NAT,01],ACAMPOSITE[OLBXITENS:NAT,02]) , CLOSE(ODLG3) , )) PIXEL  //"Incluir Rota"
	@ 235,200 BUTTON OBUTTETA PROMPT STR0178    SIZE 60,10 ACTION (IIF(FALTERAETA(CORIGEM,CDESTIN,ACAMPOSITE[OLBXITENS:NAT,01],ACAMPOSITE[OLBXITENS:NAT,02]) , CLOSE(ODLG3) , )) PIXEL  //"Altera Rota"
	@ 235,270 BUTTON OBUTTETA PROMPT STR0179        SIZE 60,10 ACTION (LOK:=.F.,CROTATRB:=ACAMPOSITE[OLBXITENS:NAT,01],CLOSE(ODLG3),) PIXEL //"Cancela"
...
IF FP0->FP0_TIPOSE == "E"
	CLABEL := STR0180 //" TARIFAS PRINCIPAIS IDA-MOBILIZAÇÃO "
ELSE
...
CASE FP0->FP0_TIPOSE == "E"
	CLABEL := STR0182 //" CUSTOS LOCAÇÃO "
	NPOS   := 0
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0183        ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"PREV.BASE"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0184          ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.BASE"
    @ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0185       ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.TURNO 1"
    @ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0186       ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.TURNO 2"
    @ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0187       ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.TURNO 3"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0188    ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.BASE TOTAL"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0189          ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.MOB."
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0190       ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.DESMOB."
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0191  ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.TRANSP.ACESS"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0192           ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.ISS"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0193        ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.SEGURO"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0194   ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VALOR A COBRAR"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0195     ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VLR.TOTAL AS"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI("LOCR03102"      	    ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER]
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0196 ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"RETORNO IMEDIATO"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0197 ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"% CUSTO INDIRETO"
	...
	CLABEL := STR0198  //" CUSTOS LOCAÇÃO ('LOCALIDADE')"
	NPOS   := 0
	@ NLINS3+(NLINA*(++NPOS-1)),NCOLS2 SAY OEMTOANSI(STR0184		    ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.BASE"
	@ NLINS3+(NLINA*(++NPOS-1)),NCOLS2 SAY OEMTOANSI(STR0185       ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.TURNO 1"
    @ NLINS3+(NLINA*(++NPOS-1)),NCOLS2 SAY OEMTOANSI(STR0186       ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.TURNO 2"
    @ NLINS3+(NLINA*(++NPOS-1)),NCOLS2 SAY OEMTOANSI(STR0187       ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.TURNO 3"
	@ NLINS3+(NLINA*(++NPOS-1)),NCOLS2 SAY OEMTOANSI(STR0188    ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.BASE TOTAL"
	@ NLINS3+(NLINA*(++NPOS-1)),NCOLS2 SAY OEMTOANSI(STR0189          ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.MOB."
	@ NLINS3+(NLINA*(++NPOS-1)),NCOLS2 SAY OEMTOANSI(STR0190	    ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.DESMOB."
	@ NLINS3+(NLINA*(++NPOS-1)),NCOLS2 SAY OEMTOANSI(STR0191  ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.TRANSP.ACESS"
	@ NLINS3+(NLINA*(++NPOS-1)),NCOLS2 SAY OEMTOANSI(STR0192           ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.ISS"
	@ NLINS3+(NLINA*(++NPOS-1)),NCOLS2 SAY OEMTOANSI(STR0193        ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.SEGURO"
	@ NLINS3+(NLINA*(++NPOS-1)),NCOLS2 SAY OEMTOANSI(STR0194   ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VALOR A COBRAR"
	@ NLINS3+(NLINA*(++NPOS-1)),NCOLS2 SAY OEMTOANSI(STR0195     ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VLR.TOTAL AS"
	@ NLINS3+(NLINA*(++NPOS-1)),NCOLS2 SAY OEMTOANSI("LOCR03102"		    ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER]
	@ NLINS3+(NLINA*(++NPOS-1)),NCOLS2 SAY OEMTOANSI(STR0196 ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"RETORNO IMEDIATO"
	...
	CLABEL := STR0199 //" CUSTOS LOCAÇÃO ('PROJETO')"
	NPOS   := 0
	@ NLINS4+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0184		    ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.BASE"
	@ NLINS4+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0185       ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.TURNO 1"
    @ NLINS4+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0186       ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.TURNO 2"
    @ NLINS4+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0187       ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.TURNO 3"
	@ NLINS4+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0188    ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.BASE TOTAL"
	@ NLINS4+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0189          ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.MOB."
	@ NLINS4+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0190	    ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.DESMOB."
	@ NLINS4+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0191  ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.TRANSP.ACESS"
	@ NLINS4+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0192           ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.ISS"
	@ NLINS4+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0193        ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.SEGURO"
	@ NLINS4+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0194   ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VALOR A COBRAR"
	@ NLINS4+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0195     ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VLR.TOTAL AS"
	@ NLINS4+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI("LOCR03102"		    ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER]
	@ NLINS4+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0196 ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"RETORNO IMEDIATO"
...
CASE FP0->FP0_TIPOSE == "L"
	...
	*/
	...
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0200        ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Qtd. Base"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0201       ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Quantidade"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0202        ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Acréscimo"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0203   ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Valor Unitário"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0204      ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Valor Bruto"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0205       ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"% Desconto"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0206       ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Valor Base"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0207   ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Resp.Frete Ida"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0208 ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Resp.Frete Volta"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0209    ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Vlr.Frete Ida"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS3 SAY OEMTOANSI(STR0210  ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Vlr.Frete Volta"
	...
	*/
	...
	CLABEL := STR0211 //" ISS E SEGURO LOCAÇÃO "
	NPOS   := 0.3
	/*
	...
	*/
	...
	@ NLINS3+(NLINA*(++NPOS-1)),NCOLS4 SAY OEMTOANSI(STR0212            ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"% ISS"
	@ NLINS3+(NLINA*(++NPOS-1)),NCOLS4 SAY OEMTOANSI(STR0213        ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Valor ISS"
	@ NLINS3+(NLINA*(++NPOS-1)),NCOLS4 SAY OEMTOANSI(STR0214     ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Valor Seguro"
...
CASE FP0->FP0_TIPOSE == "T"
	CLABEL  := STR0215 //" VR.FRETE INFORMADO"
	NPOSAUX := NPOS
	NLINAUX := NLINS2+(NLINA*(NPOSAUX))
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0216       ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"DIAS VAZIO"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0217   ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"DIAS CARREGADO"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0218         ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"KM VAZIO"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0219     ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"KM CARREGADO"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0220  ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"TIPO DE CÁLCULO"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0184          ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.BASE"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0188    ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.BASE TOTAL"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0221       ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VR.TARIFAS"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0222   ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VALOR DO FRETE"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0223 ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VALOR PRETENDIDO"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0224    ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VALOR FECHADO"
	...
	CLABEL  := STR0225 //" RENTABILIDADE"
	NPOSAUX := NPOS
	NLINAUX := NLINS2+(NLINA*(NPOSAUX))
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI("LOCR03102"		    ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER]
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0196	) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"RETORNO IMEDIATO"
	@ NLINS2+(NLINA*(++NPOS-1)),NCOLS1 SAY OEMTOANSI(STR0226		) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"VALOR VIAGEM"
	...
	CLABEL  := STR0227 //" DIMENSÕES DO CONJ.TRANSPORTADOR"
	NPOS    := 0
	NPOSAUX := NPOS
	NLINAUX := NLINS9+(NLINA*(NPOSAUX))
	@ NLINS9+(NLINA*(++NPOS-1)),NCOLS9+000 SAY OEMTOANSI(STR0228    ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Descrição"
	NPOS += 4
	@ NLINS9+(NLINA*(++NPOS-1)),NCOLS9+000 SAY OEMTOANSI(STR0229  ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Comprimento"
	@ NLINS9+(NLINA*(  NPOS-1)),NCOLS9+052 SAY OEMTOANSI(STR0230      ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Largura"
	@ NLINS9+(NLINA*(  NPOS-1)),NCOLS9+104 SAY OEMTOANSI(STR0231       ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Altura"
	@ NLINS9+(NLINA*(  NPOS-1)),NCOLS9+156 SAY OEMTOANSI(STR0232         ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Peso"
	...
	CLABEL  := STR0233 //" DIMENSÕES DA CARGA"
	NPOSAUX := NPOS
	NLINAUX := NLINS9+(NLINA*(NPOSAUX))
	@ NLINS9+(NLINA*(++NPOS-1)),NCOLS9+000 SAY OEMTOANSI(STR0228    ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Descrição"
	@ NLINS9+(NLINA*(++NPOS-1)),NCOLS9+000 SAY OEMTOANSI(STR0229  ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Comprimento"
	...
	CLABEL  := STR0234 //" DIMENSÕES TOTAIS"
	NPOSAUX := NPOS
	NLINAUX := NLINS9+(NLINA*(NPOSAUX))
	@ NLINS9+(NLINA*(++NPOS-1)),NCOLS9+000 SAY OEMTOANSI(STR0229  ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Comprimento"
	@ NLINS9+(NLINA*(  NPOS-1)),NCOLS9+052 SAY OEMTOANSI(STR0230      ) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Largura"
...
CLABEL := STR0235 //" KM VAZIO "
@ NLINS1+5, NCOLS1 SAY OEMTOANSI(STR0236) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Km Vazio ===>    "
...
CLABEL := STR0237 //" KM CARREGADO "
@ NLINS1+5, NCOLS2 SAY OEMTOANSI(STR0238) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Km Carregado ===>"
...
CLABEL := STR0239 //" KM TOTAL "
@ NLINS1+5, NCOLS3 SAY OEMTOANSI(STR0240) SIZE 50,8 PIXEL OF OFOLDER:ADIALOGS[NFOLDER] //"Km Total ===>    "
			...
			IF !EMPTY(CETAPA) .AND. !EMPTY(CDE)
				IF FTEMVAZIO(FP0->FP0_TIPOSE)
					IF ! CVAZIO $ "VC"
						Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   					Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 					{STR0241}) //"Falta preencher vazio, ou carregado"
						LRET := .F.
						EXIT
					ENDIF
				ENDIF
				...
				IF FTEMIDAVOL(FP0->FP0_TIPOSE)
					IF ! CIDAVOL $ "IV"
						Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   					Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 					{STR0242}) //"Falta preencher se a direção do trecho é Ida, ou Volta."
						LRET := .F.
						EXIT
					ENDIF
				ENDIF
...
IF ! MSGYESNO(STR0246 , STR0097)  //"CONFIRMA A EXECUÇÃO DO CÁLCULO ?"###"GPO - LOCC001.PRW"
	RETURN NIL
ENDIF
...
IF OFOLDER:NOPTION == NFOLDERGUI
	IF VALTYPE(M->FP4_PERISS) == "N" .AND. ALLTRIM(GetSx3Cache(&(LOCXCONV(2)),"X3_CAMPO")) == "FP4_PERISS"
		...
		IF !ACOLS[NPOS,LEN(AHEADER)+1] .AND. !EMPTY(COBRA)  //!DELETED()
			...
			IF FP1->(FP1_TEMVIS=="S")
			 ...
			 //	FP5->FP5_COD    := GETSXENUM("FP5","FP5_COD")
				...
				FP5->FP5_ITEM   := "001"
				IF     FP0->FP0_TIPOSE == "T"
					 CTIPOSE := STR0260 //"TRANSPORTE"
				ELSEIF FP0->FP0_TIPOSE == "L"
					 CTIPOSE := STR0119 //"LOCAÇÃO"
				ELSE
					 CTIPOSE := STR0261 //"EQUIPAMENTOS"
				ENDIF
				FP5->FP5_TPSERV := CTIPOSE 	// FP0->FP0_TIPOSE
				IF     CTIPORAT == "R"
                    FP5->FP5_REVISA	:= STRZERO(NREVIS,3)
...
IF FP0->FP0_TIPOSE == "T"
	@ NLIN2+005,INT((NCOL2-NCOL1)/2)-30 BUTTON OBUTTPROJ PROMPT STR0270 SIZE 60,10 ACTION CALCSISTEM(ODLGTRE:AHEADER,ODLGTRE:ACOLS) OF OFOLDER:ADIALOGS[NFOLDER] PIXEL  //"Calcular"
ENDIF
		...
		IF !EMPTY(CCOD)
		   ...
		   IF ST9->(DBSEEK(XFILIAL("ST9") + CCOD))
		      IF ALLTRIM(ST9->T9_CODESTO) <> ALLTRIM(CPRD)
					Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0273 + ALLTRIM(CCOD) + STR0274 + ALLTRIM(CPRD) + STR0275}) //"A frota informada ("###") não possuí o produto ("###") vinculado"
		         	LRET := .F.
		      ELSEIF ST9->T9_SITBEM <> "A"
					Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0276}) //"O campo Situação do Bem está diferente de A=Ativo."
				 
		         LRET := .F.				   
		      ELSEIF ST9->T9_SITMAN <> "A"
				 Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0277}) //"O campo Situação Manutenção está diferente de A=ATIVO."
		         LRET := .F.				   
		      ELSEIF ST9->T9_TIPOSE <> "E" 
				 Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0278}) //"O campo Tipo Serviço está diferente de E=EQUIPAMENTO."
		         LRET := .F.				   
		      ENDIF
		   ELSE
			  Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0273 + ALLTRIM(CCOD) + STR0279}) //"A frota informada ("###") não está cadastrada."
		      LRET := .F.
		   ENDIF
		ENDIF
	ENDIF
...
CASE UPPER(CCAMPO) == "FP4_CODMAO"
	IF VALTYPE(ODLGGUI)=="O"  					// SE O OBJETO JÁ FOI CRIADO
		...
		IF !EMPTY(CCOD) .AND. !EMPTY(CCODGUI)
			Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   					Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 					{STR0280}) //"Informar equipamento, ou mão de obra."
	...
	IF VALTYPE(ODLGGUI)=="O" 					// SE O OBJETO JÁ FOI CRIADO
		...
		IF !EMPTY( ALLTRIM( DTOS(DDTINI) ) ) .AND. !EMPTY( ALLTRIM( DTOS(DDTFIM) ) )
			IF DDTINI > DDTFIM
			   Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   					Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 					{STR0281}) //"A data inicial não pode ser maior que a data final."
			   RETURN .F.
			ENDIF
		ENDIF
		/*
...
CASE UPPER(CCAMPO) == "FP4_HRINI"
	IF VALTYPE(ODLGGUI)=="O"  					// SE O OBJETO JÁ FOI CRIADO
        ...
        IF SUBSTR(CHRINI,1,2) == "24"
			Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   					Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 					{STR0282}) //"O horário inicial não pode ser 24:00 tem que ser 00:00"
			RETURN .F.
		ENDIF
	ENDIF
...
CASE UPPER(CCAMPO) == "FP4_HRFIM"
	IF VALTYPE(ODLGGUI)=="O"  					// SE O OBJETO JÁ FOI CRIADO
		...
		IF SUBSTR(CHRFIM,1,2) == "00"
			Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0283}) //"O horário inicial não pode ser 00:00 tem que ser 24:00."
			RETURN .F.
		ENDIF
	ENDIF						
...
IF OFOLDER:NOPTION != NFOLDERCAR
	Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0290}) //"Favor selecionar a aba cargas."
	RETURN .F.
ENDIF

CARQ := CGETFILE("ARQUIVO CSV.  | *.CSV|", OEMTOANSI(STR0292),0,,.T.,GETF_LOCALHARD+GETF_LOCALFLOPPY)

IF EMPTY(CARQ)
	AVISO(STR0293,STR0294,{"OK"},,STR0295) //"INCONSISTENCIA"###"INFORME UM ARQUIVO A SER IMPORTADO."###"ATENCAO:"
	RETURN .F.
ENDIF
...
NAX		:= 0
WHILE !FT_FEOF()
	INCPROC(STR0296) //"PROCESSANDO ARQUIVO..."
...
IF SELECT("TRBZAG") > 0
	TRBFPA->(DBCLOSEAREA())
ENDIF
_CQUERY := " SELECT FPA_NFREM , FPA_OBRA , FPA_SEQGRU , FPA_AS , ZAG.R_E_C_N_O_ ZAGRECNO , ZAG.R_E_C_N_O_ FPARECNO, " + CRLF
...
ELSEIF CTIPO == "E"
	CDESC := STR0261 //"EQUIPAMENTOS"
ELSEIF CTIPO == "T"
	CDESC := STR0260 //"TRANSPORTE"
ENDIF
	...
	IF ((NPOSOBRA > 0) .AND. (NPOSOBRA <= LEN(ORET:ACOLS[ORET:NAT]))) .AND. ; 
	   ...
	   ((NPOSEQUI > 0) .AND. (NPOSEQUI <= LEN(ORET:ACOLS[ORET:NAT])))
		IF !(EMPTY(ORET:ACOLS[ORET:NAT, NPOSEQUI]))
			...
			CMSGAUX := STR0331 //"PARA EDITAR ESSE TURNO POSICIONE O CURSOR NA ABA LOCAÇÃO "
			IF (NLINSUP > 0) .AND. (NLINSUP < LEN(ORET:ACOLS))
				CMSGAUX += STR0332 + ALLTRIM(ORET:ACOLS[NLINSUP, NPOSOBRA]) + STR0333 + ALLTRIM(ORET:ACOLS[NLINSUP, NPOSSEQ]) //"E NA LINHA DA OBRA "###" E SEQ. GUIND. "
			ELSE
				CMSGAUX += STR0334 //"E NA LINHA DO ITEM SUPERIOR DO GRUPO."
			ENDIF

			MSGALERT(CMSGAUX , STR0029+alltrim(upper(Procname())))  //"RENTAL: "
...
//ODLGVT := MSDIALOG():NEW(000,000,300,660,STR0335,,,,,,,,,.T.) //"MANUTENÇÃO DE TURNOS"
ODLGVT := MSDIALOG():NEW(000,000,320,660,STR0335,,,,,,,,,.T.) //"MANUTENÇÃO DE TURNOS"
...
OGETVT := MSNEWGETDADOS():NEW(05   ,05   ,145    ,330   ,NSTYLE, "LOCA00186()","LOCA00187()","+FPE_TURNO" , ACPOALTER ,  , 7 , /*"FIELDOK"*/ , /*"SPRDEL"*/ , /*"DELOK"*/ , ODLGVT  ,AHEADER,ACOLS)

//OSBUTTON1 := SBUTTON():NEW(135,300,1,{||ODLGVT:END(), TIRDEL(ORET), RETGETD() },ODLGVT,.T.,,)
OSBUTTON1 := SBUTTON():NEW(147,303,1,{||ODLGVT:END(), TIRDEL(ORET), RETGETD() },ODLGVT,.T.,,)
...
Local _cEquip
Local _nLinha
Local _LOCA008 := EXISTBLOCK("LOCA008")
Local EFROM	:= ALLTRIM(GETMV("MV_RELFROM"))
Local CPARA	:= SUPERGETMV("MV_LOCX057",NIL,NIL,"LOLIVEIRA@ITUP.COM.BR")
Local _lErro := .F.
...
IF VALTYPE(ODLGCNJ)=="O"
	...
	FOR NPOS:=1 TO LEN(ODLGCNJ:ACOLS)
		...
		/* substituido pela funcao de validacao do x3_obrigat - frank em 20/10/2021
		IF EMPTY(ODLGCNJ:ACOLS[NPOS][ASCAN(ODLGCNJ:AHEADER,{|X|ALLTRIM(X[2])=="FPS_DTLIM"})])
			IF ! ODLGCNJ:ACOLS[NPOS,28]    //SE A LINHA ESTIVER DELETADA NÃO DEVERÁ VALIDAR A DATA LIMITE.
				...
				Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0341}) //"A data limite não foi preencida."
		...
		IF ODLGCNJ:ACOLS[NPOS,LEN(ODLGCNJ:AHEADER)+1]
			...
			IF FQ5->(DBSEEK(XFILIAL("FQ5")+(ODLGCNJ:ACOLS[NPOS][ASCAN(ODLGCNJ:AHEADER,{|X|ALLTRIM(X[2])=="FPS_VIAGEM"})]) ))
				IF FQ5->FQ5_STATUS == "6"
					Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0342}) //"ASF Não poderá ser deletada, pois já teve Liberação."
					LSALTD := .F.
				ELSE
					CCC		:= ""
					CTITULO	:= STR0343 + FQ5->FQ5_SOT //"CANCELADA  ASF - PROJETO "
					CMSG	:= STR0344+FQ5->FQ5_AS+ CRLF //"ESTE EMAIL É PARA INFORMAR QUE NO SISTEMA FOI CANCELADA A ASF DE NUMERO: "
					CCCO	:= ""
					LOCA05909(EFROM , CPARA , CCC , CTITULO , CMSG , NIL , CCCO)
	...
	FOR NPOS:=1 TO LEN(ODLGCNP:ACOLS)
		IF EMPTY(ODLGCNP:ACOLS[NPOS][ASCAN(ODLGCNP:AHEADER,{|X|ALLTRIM(X[2])=="FQ7_DTLIM"})])
			IF !ODLGCNP:ACOLS[NPOS,LEN(ODLGCNP:ACOLS[NPOS])]   //SE A LINHA ESTIVER DELETADA NÃO DEVERÁ VALIDAR A DATA LIMITE.
				...
				Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0345}) //"Data limite não preenchida."
	...
	FOR _NIx := 1 TO LEN(_aEquip)
		...
		FOR _NPx := 1 TO LEN(_aEquip)
			If _aEquip[_NPx,1] == _cEquip .and. _nPx <> _nIx .and. !empty(_aEquip[_NPx,1])
				Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
				{STR0548}) //"Existem equipamentos repetidos"
				Return .F.
			EndIF
		Next
	Next
	...
	IF FP0->FP0_TIPOSE == "L"				// --> FP0_TIPOSE: T=TRANSPORTE ; E=EQUIPAMENTOS ; L=LOCACAO 		// --> FP0_TIPFAT: M=MEDICAO ; P=PADRAO 
		...
		If _lValFPA
			FOR _NI := 1 TO LEN(ODLGPLA:ACOLS)
				IF !ODLGPLA:ACOLS[_NI][LEN(ODLGPLA:AHEADER)+1]				// --> NÃO ESTÁ DELETADO. 
					/* substituido pela funcao de validacao do x3_obrigat - frank em 20/10/2021
					IF EMPTY(ODLGPLA:ACOLS[_NI][NPSTPBAS]) .OR. EMPTY(ODLGPLA:ACOLS[_NI][NPSPREDI]) .OR. EMPTY(ODLGPLA:ACOLS[_NI][NPSDTINI]).or.;
					   EMPTY(ODLGPLA:ACOLS[_NI][NPSDTINI]).OR. EMPTY(ODLGPLA:ACOLS[_NI][NPSDTFIM]) //.OR. EMPTY(ODLGPLA:ACOLS[_NI][NPSVRHOR])  
						Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
						Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
						{STR0346}) //"Verifique se os campos: 'BASE CALCULO', 'QTDE MESES', 'DT.INICIO' OU 'DT.FINAL' estão preenchidos."
						LSALTD := .F.
						EXIT
					ENDIF
					*/
...
IF VALTYPE(ODLGGUI)=="O"
	...
	CMSG := STR0349+ CRLF //"A(S) FROTA(S) ABAIXO ESTÁ(ÃO) COM DATA(S) ENCAVALADA(S)."
	...
	FOR NX := 1 TO LEN(ODLGTRA:ACOLS)
		IF !ODLGTRA:ACOLS[NX,LEN(ODLGTRA:AHEADER)+1]
			...
			IF EMPTY(ODLGTRA:ACOLS[NX,NPOSORIG]) .OR. EMPTY(ODLGTRA:ACOLS[NX,NPOSDEST])
				Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0351}) //"Verifique se os campos da aba 'VIAGENS': DT. INI/DT FIM/ORIGEM E DESTINO estão preenchidos."
				LSALTD := .F.
				EXIT
			ENDIF
			*/
...
DEFINE MSDIALOG ODLG TITLE STR0465 FROM 010,005 TO 550,900 PIXEL // "LIFT - CONSULTA HISTÓRICO DE A.S."
	OPNLSOL := TPANEL():NEW(0 , 0 , "" , ODLG , NIL , .T. , .F. , NIL , NIL , 0 , 050 , .F. , .T.) 
	OPNLSOL:ALIGN := CONTROL_ALIGN_TOP 

	@ 005,005 SAY SUPERGETMV("MV_LOCX248",.F.,STR0002)+": " FONT OFONT1 PIXEL OF OPNLSOL //"PROJETO"
	@ 005,070 SAY _CPROJ      FONT OFONT1 COLOR CLR_BLUE PIXEL OF OPNLSOL

	@ 020,005 SAY STR0368    FONT OFONT1                PIXEL OF OPNLSOL //"A.S.: "
	@ 020,070 SAY _CAS        FONT OFONT1 COLOR CLR_BLUE PIXEL OF OPNLSOL

	@ 035,005 SAY STR0324 FONT OFONT1                PIXEL OF OPNLSOL //"Cliente: "
...
@ 005,005 SAY    STR0369 FONT OFONT                  PIXEL OF OPNLZP5 //"Remessa: "
@ 005,100 BUTTON STR0003 SIZE 040,010 ACTION(REMLEG()) PIXEL OF OPNLZP5  //"Legenda"
...
@ 005,005 SAY    STR0370 FONT OFONT                PIXEL OF OPNLZP6 //"Faturamento: "
@ 005,100 BUTTON STR0003 SIZE 040,010  ACTION(PEDLEG())  PIXEL OF OPNLZP6  //"Legenda"
@ 005,160 BUTTON STR0371  SIZE 040,010  ACTION(CONSFIN()) PIXEL OF OPNLZP6  //"Fatura"

IF LEN(ABRZP6) == 0
	ABRZP6	:= {{"","","","","","","","","","","",""}}
ENDIF

@ 020,001 LISTBOX OLISTP6 FIELDS HEADER "",STR0372,STR0373,STR0374,STR0375,STR0376,STR0377,STR0378 SIZE 440,60 OF OPNLZP6 PIXEL  //"A.S."###"PEDIDO DE VENDA"###"EMISSAO"###"PERÍODO INICIO"###"PERÍODO FIM"###"NOTA"###"SERIE"
...
BRWLEGENDA(STR0379,'LEGENDA',{{'BR_VERDE'		   , STR0466			},; //'REMESSA ENVIADA'
								{'BR_AMARELO'	   , STR0467			},; // '1 - FATURAMENTO'
								{'BR_AZUL'		   , STR0468			},; // '2 - FATURAMENTO'
								{'BR_CINZA'		   , STR0469		},; // 'DEVOLUÇÃO NO PRAZO'
								{'BR_VERMELHO'	   , STR0470	},; // 'DEVOLUÇÃO PRAZO CARENCIA'
								{'BR_MARRON_OCEAN' , STR0471 }}) // 'DEVOLUÇÃO 2 - FATURAMENTO'
...
// ======================================================================= \\
BRWLEGENDA(STR0380,'LEGENDA',{	{'BR_VERDE'		,STR0472	},; //'EM ANDAMENTO'
								{'BR_AZUL'		,STR0473	},; //'FATURADO'
								{'BR_VERMELHO'	,STR0474    },; //'PERDIDO'
								{'BR_CANCEL'	,STR0475    }}) //'EXCLUIDO'
RETURN .T.
...
IF EMPTY(ABRZP6[OLISTP6:NAT,7])
	Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	{STR0381}) //"Não existem registros para a consulta."
	RETURN .F.
ELSE
	IF SELECT("TMPSE1") > 0
		TMPSE1->(DBCLOSEAREA())
	...
	DEFINE MSDIALOG ODLGFIN TITLE STR0382 FROM 010,005 TO 200,540 PIXEL  //"CONSULTA FATURA"
		@ 010,005 LISTBOX OLISTS1 FIELDS HEADER "",STR0371,STR0362,STR0360,STR0367,STR0383 SIZE 260,60 OF ODLGFIN PIXEL  //"FATURA"###"PARCELA"###"PREFIXO"###"VALOR"###"DT.BAIXA"
...
IF     FP0->FP0_TIPOSE == "T"				// --> TRANSPORTE
	...
	DBUSEAREA(.T. , "TOPCONN" , TCGENQRY(,,CQUERY) , (CGETNA) , .F. , .T.) 
 	...
 	IF (CGETNA)->REGS > 0
		Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0384}) //"É obrigatório preencher a data e hora inicial/final e condição de pagamento."
	...
	DBUSEAREA(.T. , "TOPCONN" , TCGENQRY(,,CQUERY) , (CGETNA) , .F. , .T.) 
 	...
 	IF (CGETNA)->REGS > 0
		...
		Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0385}) //"É obrigatório preencher o transporte do conjunto transportador."
	...
	IF ! LRET
		...
		Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0386}) //"Não foi possível gerar contrato deste projeto, pois uma ou mais frotas estão com datas encavaladas na programação diária de transporte."
		PRINTREGS()
	ENDIF
...
ELSEIF FP0->FP0_TIPOSE == "E"				// --> EQUIPAMENTOS
	...
	DBUSEAREA(.T. , "TOPCONN" , TCGENQRY(,,CQUERY) , (CGETNA) , .F. , .T.) 
 	...
 	IF (CGETNA)->REGS > 0
		...
		Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0387}) //"É obrigatório preencher data e hora inicial/final e condição de pagamento."
...
ELSEIF FP0->FP0_TIPOSE == "L"				// --> LOCAÇÃO
	...
	ELSE
		...
		DBUSEAREA(.T.,"TOPCONN",TCGENQRY(,,CQUERY),(CGETNA),.F.,.T.)
	 	...
	 	IF LRET
			_CMSG := ""
			/* removido por Frank em 09/06/22 não existe FP0_TIPFAT IN ('A','B')
	 		IF SELECT("TRBSB2") > 0
				TRBSB2->(DBCLOSEAREA())
			ENDIF
...
IF OFOLDER:NOPTION == NFOLDERGUI 													// VERIFICAR SE ESTÁ NO FOLDER GUINDASTE, PARA VERIFICAR CONDIÇÃO ABAIXO 
	IF VALTYPE(M->FP4_PERISS) == "N" .AND. ALLTRIM(GetSx3Cache(&(LOCXCONV(2)),"X3_CAMPO")) == "FP4_PERISS" 	// VERIFICAR SE TA POSICIONADO NO CAMPO	// SE O CAMPO ESTIVER SENDO EDITADO O VALOR ATUAL NÃO ESTÁ ATUALIZADO NO ACOLS. 
		NPERISS := M->FP4_PERISS
	ENDIF
ENDIF
...
IF EMPTY(OTURNO_COLS)
	...
	(LOCXCONV(1))->(DBSETORDER(1))
	(LOCXCONV(1))->(DBSEEK("FPE"))
	WHILE (LOCXCONV(1))->(!EOF()) .AND. GetSx3Cache(&(LOCXCONV(2)),"X3_ARQUIVO") == "FPE"
		IF X3USO( &(LOCXCONV(3)) ) .AND. CNIVEL >= GetSx3Cache(&(LOCXCONV(2)),"X3_NIVEL") //SX3->X3_NIVEL
		  	IF ! GetSx3Cache(&(LOCXCONV(2)),"X3_CAMPO") $ "FPE_DIASEM|FPE_VROPER"
	         	 NUSADO++
		         AADD(AHEADERAUX, {TRIM(GetSx3Cache(&(LOCXCONV(2)),"X3_TITULO")),;    // 01 - TITULO      		
		                                GetSx3Cache(&(LOCXCONV(2)),"X3_CAMPO")  ,;    // 02 - NOME DO CAMPO      
		                                GetSx3Cache(&(LOCXCONV(2)),"X3_PICTURE"),;    // 03 - MASCARA DO CAMPO   
		                                GetSx3Cache(&(LOCXCONV(2)),"X3_TAMANHO"),;    // 04 - TAMANHO            
		                                GetSx3Cache(&(LOCXCONV(2)),"X3_DECIMAL"),;    // 05 - DECIMAIS           
		                                GetSx3Cache(&(LOCXCONV(2)),"X3_VALID")  ,;    // 06 - VALIDACAO          
		                                GetSx3Cache(&(LOCXCONV(2)),"X3_USADO")  ,;    // 07 - USADO              
		                                GetSx3Cache(&(LOCXCONV(2)),"X3_TIPO")   ,;    // 08 - TIPO DO CAMPO      
		                                ""             ,;    // 09 - ALIAS               
		                                GetSx3Cache(&(LOCXCONV(2)),"X3_CONTEXT")})    // 10 - VIRTUAL OU REAL    
			ENDIF
	 	ENDIF
	    (LOCXCONV(1))->(DBSKIP())
	ENDDO
...
PRIVATE CCADASTRO := STR0409 //"CADASTRO DE ESCOLTA"
...
DBSETORDER(1)
DBSEEK(CALIASE)
WHILE !(LOCXCONV(1))->(EOF()) .AND. GetSx3Cache(&(LOCXCONV(2)),"X3_ARQUIVO") == CALIASE
	IF !(GetSx3Cache(&(LOCXCONV(2)),"X3_CAMPO") $ "ZAN_FILIAL") .AND. CNIVEL >= GetSx3Cache(&(LOCXCONV(2)),"X3_NIVEL") .AND. X3USO( &(LOCXCONV(3)) ) .AND. ALLTRIM(&(LOCXCONV(2))) $ "ZAN_COD|ZAN_ALEMOA|ZAN_BLOQSR|ZAN_TPROD|ZAN_TPPIST"
		AADD(ACPOENCH,GetSx3Cache(&(LOCXCONV(2)),"X3_CAMPO"))
	ENDIF
	(LOCXCONV(1))->(DBSKIP())
ENDDO
...
DBSEEK("ZAN")
AHEADERZAN := {}
WHILE !(LOCXCONV(1))->(EOF()).AND.(GetSx3Cache(&(LOCXCONV(2)),"X3_ARQUIVO") == "ZAN" )
	IF X3USO( &(LOCXCONV(3)) ).AND. CNIVEL >= GetSx3Cache(&(LOCXCONV(2)),"X3_NIVEL") .AND. !(ALLTRIM(&(LOCXCONV(2))) $  "ZAN_COD|ZAN_ALEMOA|ZAN_BLOQSR|ZAN_TPROD|ZAN_TPPIST")
		NUSADO++ //:= NUSADO+1 

		AADD(AHEADERZAN,{ ALLTRIM(GetSx3Cache(&(LOCXCONV(2)),"X3_TITULO")),;   
...
ODLGZAN := MSDIALOG():NEW(ASIZE[7],ASIZE[1],ASIZE[6],ASIZE[5],CCADASTRO,,,,,,,,,.T.)
	AADD(ACOMBO,STR0410) //"S=SIM"
	AADD(ACOMBO,STR0411) //"N=NAO"
	
	// 1=MUNICIPAL;2=ESTADUAL;3=FEDERAL
	AADD(ACOMBO2,STR0412) //"M=MUNICIPAL"
	AADD(ACOMBO2,STR0413) //"E=ESTADUAL"
	AADD(ACOMBO2,STR0414) //"F=FEDERAL"
	// 1=SIMPLES;2=DUPLA;3=AMBAS
	AADD(ACOMBO3,STR0415) //"S=SIMPLES"
	AADD(ACOMBO3,STR0416) //"D=DUPLA"
	AADD(ACOMBO3,STR0417) //"A=AMBAS"
	
	@ 0.6,01.0  SAY OEMTOANSI(STR0418) OF ODLGZAN //"Código"
	@ 0.5,06.0  MSGET    CCODIGO	PICTURE PESQPICT("ZAN","ZAN_COD")  VALID CHECKSX3("ZAN_COD") WHEN(.F.) 
	@ 0.6,12.0  SAY      OEMTOANSI(STR0419)       OF ODLGZAN //"ALEMOA ?"
	@ 0.5,17.0  COMBOBOX OALEMOA VAR CALEMOA ITEMS ACOMBO  SIZE 50,010 VALID FZANVAL("ALEMOA")   WHEN(LEDIT) 
	@ 0.6,24.0  SAY      OEMTOANSI(STR0420)   OF ODLGZAN //"Bloq.Serra ?"
	@ 0.5,29.0  COMBOBOX OBLQSER VAR CBLQSER ITEMS ACOMBO  SIZE 50,010 VALID FZANVAL("BLOQSR")   WHEN(LEDIT) 
	@ 0.6,36.0  SAY      OEMTOANSI(STR0421) OF ODLGZAN //"Tipo Rodovia ?"
	@ 0.5,41.0  COMBOBOX OTPROD  VAR CTPROD  ITEMS ACOMBO2 SIZE 50,010 WHEN(LEDIT)
	@ 0.6,48.0  SAY      OEMTOANSI(STR0422)   OF ODLGZAN //"Tipo Pista ?"
	...
	WHILE !ZAN->(EOF()) .AND. ZAN->(ZAN_ALEMOA+ZAN_BLOQSR+ZAN_TPROD) == CALEMOA+CBLQSER+CTPROD
		IF ZAN->ZAN_TPPIST == CTPPIST .OR. CTPPIST == "A" .OR. ZAN->ZAN_TPPIST == "A"
			IF CCODIGO != ZAN->ZAN_COD
				Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0423}) //"Já existe um cadastro com esses dados, troque uma das chaves principais (ALEMOA,BLOQ.SERRA,TIPO ROD.,TIPO PISTA)"
				LRET := .F.
				EXIT
			ENDIF
		ENDIF
...
IF CCAMPO == "ZAN_NIVEL"
	FOR NX := 1 TO LEN(OGETD1:ACOLS)
		IF OGETD1:ACOLS[NX,GDFIELDPOS("ZAN_NIVEL", OGETD1:AHEADER)] == NNIVEL .AND. NX !=OGETD1:NAT
			Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0424}) //"Este nível já foi informado."
			LRET := .F.
			EXIT
		ENDIF
	NEXT NX
...
AADD(AROTINA , {STR0057  , "AXPESQUI"  , 0 , 1}) //"PESQUISAR"
AADD(AROTINA , {STR0058 , "LOCA00183" , 0 , 2}) //"VISUALIZAR"
AADD(AROTINA , {STR0060    , "LOCA00183" , 0 , 3}) //"INCLUIR"
AADD(AROTINA , {STR0061    , "LOCA00183" , 0 , 4}) //"ALTERAR"
AADD(AROTINA , {STR0161    , "LOCA00183" , 0 , 5}) //"EXCLUIR"
...
IF !OFOLDER:NOPTION==NFOLDERTRA
	Help(Nil,	Nil,STR0029+alltrim(upper(Procname())),; //"RENTAL: "
	   				Nil,STR0033,1,0,Nil,Nil,Nil,Nil,Nil,; //"Inconsistência nos dados."
	 				{STR0426}) //"Favor selecionar a ABA viagens."
	RETURN .F.
ENDIF

//@ 0,0   TO 90,250 MSDIALOG ODLGQ TITLE "DUPLICA A VIAGEM SELECIONADA"
DEFINE MSDIALOG ODLGQ TITLE STR0427 FROM 0,0 TO 90,250 //"DUPLICA A VIAGEM SELECIONADA"

	@ 00,01 TO 03,15
	@ 001,002 SAY STR0428 //"Qquantidade de linhas a duplicar:"
	@ 001,012 GET _NNUMDUP PICTURE "999" WHEN FWHEN()
	@ 025,060 BMPBUTTON TYPE 1 ACTION ( LOK := .T., CLOSE(ODLGQ) )
	@ 025,085 BMPBUTTON TYPE 2 ACTION CLOSE(ODLGQ)
ACTIVATE MSDIALOG ODLGQ CENTERED
...
IF ! MSGYESNO(STR0429 +CSEQTRA+ " - "+ ALLTRIM(STR(_NNUMDUP)) + STR0105 , STR0097)  //"DUPLICA A VIAGEM "###" VEZES ??"###"GPO - LOCC001.PRW"
	RETURN .F.
ENDIF

_lMV_LOCX227 := SUPERGETMV("MV_LOCX227",.F.,.T.)
...
OREPORT   := TREPORT():NEW("LOCA001", OEMTOANSI(STR0430), /*CPERG*/, {|OREPORT| LOCC001IMP(OREPORT)}) //"INCONSISTENCIAS"

OSECTION1 := TRSECTION():NEW(OREPORT, OEMTOANSI(STR0431), {} ) //"FROTA"
OSECTION1:SETHEADERSECTION(.T.)
TRCELL():NEW(OSECTION1,'CFROTA'     , /*"PDA"*/,	STR0431		, "@!"/*"@R 99.999.999/9999-99"*/  , 40 ,.F. , ) //"FROTA"
TRCELL():NEW(OSECTION1,'CDATINI'    , /*"PDA"*/,	STR0432	, "@!"/*"@R 99.999.999/9999-99"*/  , 10 ,.F. , ) //"DATA INI."
TRCELL():NEW(OSECTION1,'CDATFIM'    , /*"PDA"*/,	STR0433	, "@!"/*"@R 99.999.999/9999-99"*/  , 10 ,.F. , ) //"DATA FIM"
...
FOR NX := 1 TO LEN(AFROTA)
	...
	FOR NY := 1 TO LEN(AERROR)
		OREPORT:PRINTTEXT(STR0434+AERROR[NY,1]+" - "+DTOC(AERROR[NY,2])+STR0435+ALLTRIM(AERROR[NY,3])+" / "+ALLTRIM(AERROR[NY,4])+")",OREPORT:ROW(),10) //"		ENCAVALADO: "###" - AS/PROJETO/OBRA: ("
		OREPORT:SKIPLINE()
	NEXT NY
	OREPORT:THINLINE()
NEXT NX
...
IF POPC <> 0									// NÃO FOR FOLLOW-UP
	...
	CASE FP0->FP0_TIPOSE == "T"					// ==> TRANSPORTE
		...
		AADD(ATITLES , STR0441)  //"&VIAGENS"
		NFOLDERTRA := LEN(ATITLES)

		AADD(ATITLES , STR0442 )  //"&CARGAS"
		NFOLDERCAR := LEN(ATITLES)

		IF SUPERGETMV("MV_LOCX225",.F.,.T.)
			AADD(ATITLES , STR0443)  //"C&ONJ.TRANSPORTADOR"
			NFOLDERCON := LEN(ATITLES) 
		ENDIF 

		AADD(ATITLES , STR0444)  //"&TRECHOS"
	...
	CASE FP0->FP0_TIPOSE == "E"					// ==> EQUIPAMENTO
		...
		ENDIF
		AADD(ATITLES , STR0447)  //"ORI&GEM/DESTINO"
...
DEFINE MSDIALOG OTELA FROM 0, 0 TO ASIZE[6],ASIZE[5] TITLE CTITULO OF GETWNDDEFAULT() PIXEL 
	...
	//@ 140, 200 BUTTON OBUTOBS PROMPT STR0509   SIZE 70,10 ACTION TELAOBS() OF OPANEL PIXEL // "Observações <F11>"
...
Local lValida   := !empty(FWX3Titulo( "TQY_STTCTR" ))
...
IF FP0->FP0_TIPOSE == "L"
	...
	@ 159,090 MSGET oxb1desc var POSICIONE("SB1",1,XFILIAL("SB1")+FPA->FPA_PRODUT,"B1_DESC")  PICTURE "@!" WHEN .F. SIZE 150,8 OF ODLG PIXEL COLOR CLR_BLUE 
