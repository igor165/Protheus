...
Function F198GerTrb(aSelFil)
	...
	Local oQryFk2       := Nil
	Local oRegCheq      := Nil
	Local cMAEmpSED     := FWModeAccess("SED",1)
	Local cMAUniSED     := FWModeAccess("SED",2)
	Local cMAFilSED     := FWModeAccess("SED",3)
	Local cMAEmpSE5     := FWModeAccess("SE5",1)
	Local cMAUniSE5     := FWModeAccess("SE5",2)
	Local cMAFilSE5     := FWModeAccess("SE5",3)
	Local cFilSE5       := xFilial("SE5")
	Local aCampos       := {}
	Local cNumCheq      := ""
	Local cEdPai        := Padr(" ", TamSx3("ED_PAI")[1], " ")
	Local cFilOriAnt    := ""
	Local cChaveCh      := ""
	Local lAchouSE1     := .F.
	Local lAchouSE2     := .F.
