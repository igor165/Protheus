				...
				If TFL->( DbSeek(xFilial("TFL")+cCodLocal) )
					...
					If TFL->TFL_TOTRH > 0
						If !lDsgCN
							...
							cItemTFF	:= cItem
						Else
							aPrdsRH := GetPrdCTR(aLocal[nCont,1],aLocal[nCont,2	], "RH",TFJ->TFJ_CODIGO)
...
/*/
//--------------------------------------------------------------------------------------------------------------------
Static Function GetPrdCTR(cCodPai,cLocal,cOrc,cCodOrc)
...
If TFJ->( DbSeek(xFilial("TFJ")+cCodOrc) )
	...
	If cOrc == "RH"
		BeginSql Alias cAliasRH
			SELECT TFF_PRODUT,TFF_QTDVEN,TFF_PRCVEN,TFF.R_E_C_N_O_, TFF_TESPED,
					TFF_TXLUCR, TFF_TXADM, TFF_VALDES
			FROM %Table:TFF% TFF
