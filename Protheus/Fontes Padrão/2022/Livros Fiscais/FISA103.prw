...
Static Function Reg0000 (oProcess, oBloco0, oTabela,aNumeracao,lCancel)
    ...
    aAdd(aReg0000,IIF (Upper(SM0->M0_ESTENT) == "EX","",IIF(Len(Alltrim(SM0->M0_CODMUN))<=5,UfCdIBGE(SM0->M0_ESTENT),"")+SM0->M0_CODMUN)) /*10-Código do Municipio do Estabelecimento Informante*/
    ...
    Do While !(cAliasCLP)->(Eof ())
        ...
        IF EMPTY((cAliasCLP)->CLP_CNPJ)
            ...
            IF (SA1->(dbSeek(xFilial("SA1")+(cAliasCLP)->CLP_COD+(cAliasCLP)->CLP_LOJA))) .And. (cAliasCLP)->CLP_TPPART == "2"
                ...
                aAdd(aReg0150,IIF(lExterior,"",SA1->A1_INSCR))
            ...
            ELSE
                ...
                IF (SA2->(dbSeek(xFilial("SA2")+(cAliasCLP)->CLP_COD+(cAliasCLP)->CLP_LOJA)))
                    ...
                    aAdd(aReg0150,IIF(lExterior,"",SA2->A2_INSCR))
...
Static Function Processa(oProcess, oBloco5,oTabela,aNumeracao,cChaveBlc5,dDataDe,dDataAte,lCancel )
    ...
    IF MV_PAR10 == 1 .Or. (Month(MV_PAR01) == 12 .And. Month(MV_PAR02) = 12)
        ProInvent(oProcess,oBloco5,oTabela,dDataDe,dDataAte)
    ENDIF
    ...
    Do While !(cAlias)->(Eof ())
        ...
        IF !(cAlias)->CLR_FICHA$"67|68"
            ...
            IF !Empty(cRegist) .And. (nPosAb:= aScan(aRegAbert,{|X| X[2] == (cAlias)->CLR_FICHA .AND. X[3] == (cAlias)->F04_PROD})) > 0
                
                //Verifica se os dados ja existe no array
                IF (cCont := aScan(aReg,{|X| X[2] == (cAlias)->CLR_FICHA .AND. X[3] == (cAlias)->F04_PROD})) == 0
        ...
        ELSE
            ...
            aAdd(aServTran[nPos],(cAlias)->CLQ_TPDOC)                       //6-Tipo do documento conforme a coluna Código chave da tabela 4. nTpDoc
...
Static Function ProInvent(oProcess,oBloco5,oTabela,dDataDe,dDataAte)
    Local cAliasCLV := ""
    Local aRegCab   := {}
    Local aRegInv   := {}
    ...
    BeginSql Alias cAliasCLV
        ...
        AND      CLV.%NOTDEL%
        AND     (SUBSTRING(CLV.CLV_PERIOD,5,2) = %exp:cPerDe%
        OR       SUBSTRING(CLV.CLV_PERIOD,5,2) = %exp:cPerAte%)
...
Static Function TabTemp()
    ...
    Local cDir      := Alltrim(MV_PAR05)+ Alltrim(cfilAnt)
