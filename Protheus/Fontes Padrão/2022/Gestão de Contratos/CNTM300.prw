...
Static Function detailCtr( oSelf )
	...
	If Empty(cGrupos)
		cJoinCNN := "CNN.CNN_USRCOD = '" + cCodUsr + "' AND "
	Else	
		cJoinCNN := "( CNN.CNN_USRCOD = '" + cCodUsr + "' OR CNN.CNN_GRPCOD IN ("+ cGrupos +") ) "
	EndIf
	...
	While ( cAliasQry )->( ! Eof() ) 
		...
		If ( cAliasQry )->CNB_RATEIO == '2'
			...
			If !Empty( ( cAliasQry )->CNB_CC ) .Or. !Empty( ( cAliasQry )->CNB_CONTA ) .Or. !Empty( ( cAliasQry )->CNB_ITEMCT ) .Or. !Empty( ( cAliasQry )->CNB_CLVL )
				...
				aRateio[1]['amount']['symbol'] := cSimbolo
...
Static Function WS300Rat( cContra, cRev, cPlan, cItem, nMoeda )
	...
	While ( cAliasQry )->( ! Eof() ) 
		...
		aRateio[nAux]['amount']['symbol']	:= cSimbolo
