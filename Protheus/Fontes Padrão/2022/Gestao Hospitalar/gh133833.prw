...
±±³Fun‡ao    ³GH133833  ³ Autor ³ MICROSIGA             ³ Data ³ 13/09/07 ³±±
...
Private lAtual_GAY   := .F. // Status se criou os campos
Private lAtual_GDV   := .F. // Status se criou os campos
...
Processa({|| ProcATU()},"Processando [GH133833]","Aguarde , processando preparação dos arquivos")
...
If (lOpen := IIF(Alias() <> "SM0", MyOpenSm0Ex(), .T. ))
	...
	If lOpen
		...
		If lOpen
			...
			DEFINE MSDIALOG oDlg TITLE "Atualizador [GH133833] - Atualizacao concluida." From 3,0 to 340,417 PIXEL
...
Static Function FS_RefGDV()
 Local aVetGDV 	:= {}
 Local nForCpo  := 0
 Local aArea    := GetArea() 

 aVetGDV := HS_ChvCrd({"GAY", "GDV"}, .F.)
 
 DbSelectArea("GDV")
 DbSetOrder(2)
 
 lAchou := DbSeek(xFilial("GDV") + GAY->GAY_TIPCRE + GAY->GAY_CODPRE + aVetGDV[1])
 RecLock("GDV", !lAchou)
 GDV->GDV_FILIAL := xFilial("GDV")
   
 If !lAchou
 	GDV->GDV_TIPCRE := GAY->GAY_TIPCRE    
  GDV->GDV_CODPRE := GAY->GAY_CODPRE
  GDV->GDV_PRIORI := "9999"
