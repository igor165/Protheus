...
User Function DSVFAT1()
	...
	If( nMinuto >= nInterv )
		...
		cQry := "SELECT SFP.FP_SERIE, SFP.FP_NUMINI, SFP.FP_NUMFIM, SFP.FP_ESPECIE	"	+ PRX_LIN
		cQry += "     , SFP.FP_PV, SFP.FP_ULTDTHR, SFP.FP_TOTTENT, SFP.FP_QTDTENT	"	+ PRX_LIN
		cQry += "     , SFP.FP_FILUSO, SFP.FP_FILIAL, SFP.FP_NFEEX					"	+ PRX_LIN
		
		//#TB20190408 Thiago Berna - Ajuste para considerar corretamente o numero inicial e final da NF
		cQry += "     , R_E_C_N_O_ RECNO											"	+ PRX_LIN
		
		cQry += "  FROM " + RetSqlName('SFP') + " SFP								"	+ PRX_LIN
		cQry += " WHERE SFP.FP_FILIAL = '" + xFilial('SFP') + "'					"	+ PRX_LIN
		cQry += " 	AND SFP.D_E_L_E_T_ = ' '										"	+ PRX_LIN
		cQry += "   AND SFP.FP_ESPECIE IN ( " + cSfpEsp + " )						"	+ PRX_LIN	//// FILTRO PARA LOCALIZAR APENAS CONTROLE DE FORMULÁRIOS PARA FACTURAS=(1-NF/2-NCI/3-NDI/4-NCC/5-NDC) REMITOS=(6-RFC/7-RFD)
		cQry += "   AND RTRIM(SFP.FP_SERIE) IN ( " + cSerVld + " )					"	+ PRX_LIN	
		cQry += " ORDER BY SFP.FP_ESPECIE, SFP.FP_PV, SFP.FP_SERIE					"	+ PRX_LIN
		
		cQry := ChangeQuery( cQry )
		dbUseArea( .T., "TOPCONN", TCGENQRY(,,cQry),cAliTmp, .F., .T.)
		(cAliTmp)->( DbGoTop() )
		...
		While( (cAliTmp)->( ! Eof() ) )
			...
			If( (cAliTmp)->FP_ESPECIE $ "6|7" )		/// ROTINA PARA TRANSMISSÃO DE REMITO    
				...
				cAutRet := StaticCall( ArgRemElet, ArgRem2, nVezes, "SF2", lAutoNf )
			...
			ElseIf( (cAliTmp)->FP_ESPECIE $ "4" .and. (cAliTmp)->FP_NFEEX == "6" )	/// ROTINA PARA TRANSMISSÃO DE REGIME NOMINAÇÃO    
				...
				cQry := " SELECT SF1.F1_FILIAL, SF1.F1_EMISSAO, SF1.F1_FORMUL, SF1.F1_DOC  "	+ PRX_LIN
				cQry += "      , SF1.F1_SERIE, SF1.F1_FORNECE, SF1.F1_LOJA, SF1.F1_ESPECIE "	+ PRX_LIN
				cQry += "      , SF1.F1_CAEE, SF1.F1_FLFTEX, SF1.F1_TIPO, SF1.F1_TPVENT    "	+ PRX_LIN
				cQry += "   FROM " + RetSqlName('SF1') + " SF1                             "	+ PRX_LIN
				cQry += "  WHERE SF1.D_E_L_E_T_     = ' '                                  "	+ PRX_LIN
				cQry += "    AND SF1.F1_FILIAL      = '" + xFilial('SF1') + "'             "	+ PRX_LIN
				If Upper(TcGetDb()) $ "ORACLE"
					cQry += "    AND SUBSTR(F1_DOC,1,4)     = '" + cPtoVnd + "'                    "	+ PRX_LIN
				ELSE
					cQry += "    AND LEFT(F1_DOC,4)     = '" + cPtoVnd + "'                    "	+ PRX_LIN
				ENDIF
				cQry += "	 AND SF1.F1_SERIE       = '" + cAutSer + "'                    "	+ PRX_LIN
				cQry += "	 AND SF1.F1_FLFTEX     != 'S'                                  "	+ PRX_LIN
				cQry += "	 AND RTRIM(F1_ESPECIE) IN ('NCC')                          "	+ PRX_LIN      
				cQry += " ORDER BY SF1.F1_FILIAL,SF1.F1_SERIE, SF1.F1_DOC					"	+ PRX_LIN
				
				cQry := ChangeQuery( cQry )
				dbUseArea( .T., "TOPCONN", TCGENQRY(,,cQry),cAliTm2, .F., .T.)
				(cAliTm2)->( DbGoTop() )
				...
				While( (cAliTm2)->( ! Eof() ) )
					...
					cAutRet := StaticCall( ARGNFe, ARGNNFe2, nVezes==0, "SF1", lAutoNf )
			...
			ElseIf( (cAliTmp)->FP_ESPECIE $ "4"  )	/// TRANSMISION NCC WEBSERVER NACIONAL    
				...
				cQry += "	 AND SF1.F1_SERIE       = '" + cAutSer + "'                    "	+ PRX_LIN
				cQry += "	 AND SF1.F1_FLFTEX     != 'S'                                  "	+ PRX_LIN
				cQry += "	 AND RTRIM(F1_ESPECIE) = 'NCC'                          "	+ PRX_LIN
				cQry += " ORDER BY SF1.F1_FILIAL,SF1.F1_SERIE, SF1.F1_DOC					"	+ PRX_LIN
			...
			Else		/// CHAMADA DA ROTINA DE TRANSMISSÃO DE FACTURA
				...
				cQry := " SELECT SF2.F2_FILIAL, SF2.F2_EMISSAO, SF2.F2_FORMUL, SF2.F2_DOC  "	+ PRX_LIN
				cQry += "      , SF2.F2_SERIE, SF2.F2_CLIENTE, SF2.F2_LOJA, SF2.F2_ESPECIE "	+ PRX_LIN
				cQry += "      , SF2.F2_CAEE, SF2.F2_FLFTEX, SF2.F2_TIPO, SF2.F2_TPVENT    "	+ PRX_LIN
				cQry += "   FROM " + RetSqlName('SF2') + " SF2                             "	+ PRX_LIN
				cQry += "  WHERE SF2.D_E_L_E_T_     = ' '                                  "	+ PRX_LIN
				cQry += "    AND SF2.F2_FILIAL      = '" + xFilial('SF2') + "'             "	+ PRX_LIN
				If Upper(TcGetDb()) $ "ORACLE"
					cQry += "    AND SUBSTR(F2_DOC,1,4)     = '" + cPtoVnd + "'                    "	+ PRX_LIN
				ELSE
					cQry += "    AND LEFT(F2_DOC,4)     = '" + cPtoVnd + "'                    "	+ PRX_LIN
				ENDIF
				cQry += "	 AND SF2.F2_SERIE       = '" + cAutSer + "'                    "	+ PRX_LIN
				cQry += "	 AND SF2.F2_FLFTEX     != 'S'                                  "	+ PRX_LIN
				If( cTipFac $ "1|5|7") 
					cQry += "	 AND RTRIM(F2_ESPECIE) IN ('NF','NDC')                     "	+ PRX_LIN
				Else
					cQry += "	 AND RTRIM(F2_ESPECIE) IN ('NF','NDC','NDI','NCI')         "	+ PRX_LIN
				EndIf 
				cQry += " ORDER BY SF2.F2_FILIAL,SF2.F2_SERIE, SF2.F2_DOC					"	+ PRX_LIN
				...
				While( (cAliTm2)->( ! Eof() ) )
					...
					cAutRet := StaticCall( ARGNFe, ARGNNFe2, nVezes==0, "SF2", lAutoNf )
		...
		While( (cAliTmp)->( ! Eof() ) )
			...
			//#TB20190408 Thiago Berna - Ajuste para considerar corretamente o numero inicial e final da NF
			cQrySFP := "SELECT      COALESCE ((	SELECT MIN(SF3.F3_NFISCAL)					"	+ PRX_LIN
			cQrySFP += "			FROM " + RetSqlName('SF3') + " SF3 						"	+ PRX_LIN
			cQrySFP += "			WHERE SF3.F3_FILIAL = SFP.FP_FILUSO						"	+ PRX_LIN
			cQrySFP += "			AND SF3.F3_CAE = '' 									"	+ PRX_LIN
			cQrySFP += "			AND SF3.F3_SERIE= SFP.FP_SERIE				 			"	+ PRX_LIN
			cQrySFP += "			AND SF3.D_E_L_E_T_ = ' ' ) ,SFP.FP_NUMINI) FP_NUMINI,	"	+ PRX_LIN
			cQrySFP += "            COALESCE ((	SELECT MAX(SF3.F3_NFISCAL)					"	+ PRX_LIN
			cQrySFP += "			FROM " + RetSqlName('SF3') + " SF3 						"	+ PRX_LIN
			cQrySFP += "			WHERE SF3.F3_FILIAL = SFP.FP_FILUSO						"	+ PRX_LIN
			cQrySFP += "			AND SF3.F3_CAE = '' 									"	+ PRX_LIN
			cQrySFP += "			AND SF3.F3_SERIE= SFP.FP_SERIE				 			"	+ PRX_LIN
			cQrySFP += "			AND SF3.D_E_L_E_T_ = ' ' ) ,SFP.FP_NUMFIM) FP_NUMFIM	"	+ PRX_LIN

			cQrySFP += "  FROM " + RetSqlName('SFP') + " SFP								"	+ PRX_LIN
			cQrySFP += " WHERE SFP.R_E_C_N_O_ = '" + AllTrim(Str((cAliTmp)->RECNO)) + "'	"	+ PRX_LIN

			cQrySFP := ChangeQuery( cQrySFP )

			If Select(cAliSFP) > 0
				(cAliSFP)->(DbCloseArea())
			EndIf

			dbUseArea( .T., "TOPCONN", TCGENQRY(,,cQrySFP),cAliSFP, .F., .T.)
			(cAliSFP)->( DbGoTop() )
