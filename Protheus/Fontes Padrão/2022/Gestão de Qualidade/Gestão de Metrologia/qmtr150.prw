...
If DbSeek(xFilial("QM4")+cPecaRev)
	...
	If QM4->QM4_TPMSA == "1" .and. QM4->QM4_TIPO == "A"
		a150TabEns()
		a150Monta()
		...
		If Alltrim(M->QM4_TPMSA) == "1" .and. M->QM4_TIPO == "A"
			qmt150UseRef()
		Endif	
...
For nI := 1 To nLimite
	...
	For nCont1 := 1 To Len(aCols)
		...
		oPrint:Box( lin-20, 30, lin+60, nFimBox )
			
		// Construcao da grade
		oPrint:Line( lin-20, 0360, lin+60, 0360 )   	// vertical
		oPrint:Line( lin-20, 0450, lin+60, 0450 )   	// vertical
			
		If cEns <> aCols[nCont1,nPosEns]
			oPrint:Say(lin,0040,Subs(aCols[nCont1,nPosEns],1,20),oFont08) // Ensaiador
		Endif
			
		oPrint:Say(lin,0385,Alltrim(Str(aCols[nCont1,nPosCic])),oFont08) // Ciclo
			
		If nI == 1
			If M->QM4_TIPO <> "A"
				oPrint:Say(lin,0460,Transform(aCols[nCont1,nPosPec],cPict),oFont08) // Peca1
			Else
				If aCols[nCont1,nPosAtr] == "1"
					oPrint:Say(lin,0460,STR0032,oFont08) //"Passa"
				Else
					oPrint:Say(lin,0460,STR0033,oFont08) //"Nao Passa"
				Endif
			Endif
		Endif
		...
		For x := nDe To nAte
			...
			oPrint:Line( lin-20, nCoord-40, lin+60, nCoord-40 )   	// vertical
				
			If M->QM4_TIPO <> "A"
				oPrint:Say(lin,nCoord-20,Transform(aCols[nCont1,x], cPict),oFont08)
			Else
				If aCols[nCont1,x] == "1"
					oPrint:Say(lin,nCoord-20,STR0032,oFont08) //"Passa"
				Else
					oPrint:Say(lin,nCoord-20,STR0033,oFont08) //"Nao Passa"
				Endif
			Endif
	...
	If M->QM4_TIPO $ "EP"
		...
		oPrint:Box( lin-20, 30, lin+60, nFimBox )
		oPrint:Say( lin, 70, STR0034, oFont10 )		//"Medias da Peca"
		...
		For x := nDe To nAte
			...
			oPrint:Line( lin-20, nCoord-40, lin+60, nCoord-40 )   	// vertical
			oPrint:Say(lin,nCoord-20,Transform(aNMedp[x], cPict),oFont08)
		Next x
	Endif
...
If M->QM4_TIPO $ "EP"
	...
	oPrint:Say(lin,400,"Rp:", oFont08)
	oPrint:Say(lin,450,AllTrim(Transform(aFim[1,6], cPict)), oFont08) // nRp
	
	oPrint:Line( lin-10, 695, lin-10, 720 ) 	// RBB
	oPrint:Line( lin-05, 695, lin-05, 720 )
	oPrint:Say(lin,700,"R:", oFont08)
	oPrint:Say(lin,750,AllTrim(Transform(aFim[1,7], cPict)), oFont08) // RBB
