...
Static Function fCarrRis( cOper, dDtIniCond, cChave )
	...
	If Len( aAmbExp ) > 0
		cXml += 		'<infoAmb>'
		cXml += 			'<localAmb>'	+ aAmbExp[ 1, 2 ]	+ '</localAmb>' //Obrigatório
		cXml += 			'<dscSetor>'	+ aAmbExp[ 1, 3 ]	+ '</dscSetor>' //Obrigatório
		cXml += 			'<tpInsc>'		+ aAmbExp[ 1, 4 ]	+ '</tpInsc>' //Obrigatório
		cXml += 			'<nrInsc>'		+ aAmbExp[ 1, 5 ]	+ '</nrInsc>' //Obrigatório
		cXml += 		'</infoAmb>'
	EndIf
	cXml += 			'<infoAtiv>'
				...
				If Len( aEPIs ) > 0
					...
					For nEPI := 1 To Len( aEPIs )
						...
						If !Empty( aEPIs[ nEPI, 2 ] )
							cXml +=		'<docAval>'			+ aEPIs[ nEPI, 2 ] + '</docAval>' //Certificado de Aprovação ou Documento de Avaliação do EPI
						Else
...
Static Function fInconsis( aIncEnv, dDtIniCond, cNumMat, cCCusto, cDepto, cFuncao )
	...
	//Busca a filial de envio a ser considerada nas validações
	cFilAnt := cFilEnv
...
Static Function fGetAmbExp( cCCusto, cDepto, cFuncao, cNumMat, dDtIniCond )
	...
	ElseIf lTOU .And. cEntAmb == "5" //Funcionário
		...
		If lMDTA165 //Caso for chamado pela rotina de Ambiente Físico
	...
	If !Empty( cSeekTNE ) //Caso exista um ambiente vinculado
		...
		//Caso o relacionamento do ambiente for por funcionário e seja chamada pelo cadastro de ambiente
		If lTOU .And. cEntAmb == "5" .And. lMDTA165

			//Adiciona as informações do modelo no array
			aAdd( aInfAmb, { AllTrim( oModel:GetValue( "TNEMASTER", "TNE_CODAMB" ) ), ;
								IIf( oModel:GetValue( "TNEMASTER", "TNE_LOCAMB" ) == "1", "1", "2" ), ; //Informação obrigatória a ser enviada na tag <localAmb>
								AllTrim( SubStr( MDTSubTxt( Upper( oModel:GetValue( "TNEMASTER", "TNE_MEMODS" ) ) ), 1, 99 ) ), ; //Informação obrigatória a ser enviada na tag <dscSetor>
								IIf( oModel:GetValue( "TNEMASTER", "TNE_TPINS" ) == "2", "3", IIf( oModel:GetValue( "TNEMASTER", "TNE_TPINS" ) == "3", "4", oModel:GetValue( "TNEMASTER", "TNE_TPINS" ) ) ), ; //Informação obrigatória a ser enviada na tag <tpInsc>
								oModel:GetValue( "TNEMASTER", "TNE_NRINS" ) } ) //Informação obrigatória a ser enviada na tag <nrInsc>
		...
		Else
			...
			If dbSeek( xFilial( "TNE" ) + cSeekTNE )
				...
				//Adiciona as informações da TNE no array
				aAdd( aInfAmb, { AllTrim( TNE->TNE_CODAMB ), ;
								IIf( TNE->TNE_LOCAMB == "1", "1", "2" ), ; //Informação obrigatória a ser enviada na tag <localAmb>
								AllTrim( SubStr( MDTSubTxt( Upper( TNE->TNE_MEMODS ) ), 1, 99 ) ), ; //Informação obrigatória a ser enviada na tag <dscSetor>
								IIf( TNE->TNE_TPINS == "2", "3", IIf( TNE->TNE_TPINS == "3", "4", TNE->TNE_TPINS ) ), ; //Informação obrigatória a ser enviada na tag <tpInsc>
								TNE->TNE_NRINS } ) //Informação obrigatória a ser enviada na tag <nrInsc>
			EndIf
...
@return	aInfRis, Array, Array contendo as informações de exposição dos riscos
...
Static Function fGetRisExp( cNumMat, dDtIniCond, nOper, lIncons )
	...
	//Variáveis utilizadas para busca da informação do Risco
	Local cNumRis := ""
	Local cAgente := ""
	Local nQtAgen := 0
	Local cUniMed := ""
		...
		If !( lMDTA180 .And. nOper == 5 .And. cNumRis == M->TN0_NUMRIS )
			...
			If ( nPosRis := aScan( aInfRis, { | x | x[ 3 ] == Posicione( "TMA", 1, xFilial( "TMA" ) + cAgente, "TMA_ESOC" ) .And. ;
				...
				aAdd( aInfRis, { AllTrim( cNumRis ), ;
								...
								AllTrim( MDTSubTxt( Posicione( "TMA", 1, xFilial( "TMA" ) + cAgente, "TMA_DESCRI" ) ) ), ; //Informação a ser enviada na tag <dscAgNoc>
								...
								cValToChar( Posicione( "TLK", 1, xFilial( "TLK" ) + cAgente, "TLK_DEQTDE" ) ), ; //Informação a ser enviada na tag <limTol>
			...
			Else
				...
				If Val( aInfRis[ nPosRis, 6 ] ) < nQtAgen
					...
					aInfRis[ nPosRis, 4 ] := AllTrim( MDTSubTxt( Posicione( "TMA", 1, xFilial( "TMA" ) + cAgente, "TMA_DESCRI" ) ) ) //Informação a ser enviada na tag <dscAgNoc>
					...
					aInfRis[ nPosRis, 7 ] := cValToChar( Posicione( "TLK", 1, xFilial( "TLK" ) + cAgente, "TLK_DEQTDE" ) ) //Informação a ser enviada na tag <limTol>
...
Static Function fGetResAmb( dDtIniCond, aRisTrat )
	...
	For nCont := 1 To Len( aUsus )
		...
		//Verificação pontual para casos onde tabela TMK estiver compartilhada
					...
					If !Dbseek( cXFilTNF + cMat + aTNFobr[ nFor1 ] )
						...
						If ( nPosCAP := aScan( aInfEpi, { | x | x[ 2 ] == AllTrim( TNF->TNF_NUMCAP ) } ) ) == 0
							...
							aAdd( aInfEpi, {;
								...
								AllTrim( Posicione( "SB1", 1, xFilial("SB1") + aEpiEso[ nFor3, 1 ], "B1_DESC" ) ),; // Informação a ser enviada na tag <dscEPI>
				...
				While TNF->( !Eof() ) .And. cXFilTNF + cMat + aTNFobr[nFor1] == TNF->( TNF_FILIAL + TNF_MAT + TNF_CODEPI )
					...
					If ( nPosCAP := aScan( aInfEpi, { | x | x[ 2 ] == AllTrim( TNF->TNF_NUMCAP ) } ) ) == 0
						aAdd( aInfEpi, { AllTrim( TNF->TNF_CODEPI ), ;
										...
										AllTrim( Posicione( "SB1", 1, xFilial("SB1") + TNF->TNF_CODEPI, "B1_DESC" ) ) ,; //Informação a ser enviada na tag <dscEPI>
										cPROTEC, ; //Informação a ser enviada na tag <eficEpi>
										cMedPrt, ; //Informação a ser enviada na tag <medProtecao>
										cCndFun, ; //Informação a ser enviada na tag <condFuncto>
										cUsuIni, ; //Informação a ser enviada na tag <usoInint>
...
Abre o arquivo SX6 na empresa passada por parâmetro

@return	Nil, Nulo

@sample	fOpenSX6( "T3" )

@param	cEmpCgh, Caracter, Indica a empresa para que será aberta o arquivo SX6
...
Static Function fOpenSX6( cEmpCgh )
	...
	//Abre o arquivo na empresa passada por parâmetro
