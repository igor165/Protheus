...
Function MNTA085()
	...
	If !FindFunction( 'MNTAmIIn' ) .Or. MNTAmIIn( 95 )
		...
		If !Empty( Alltrim( SuperGetMv( 'MV_NGSTAFG', .F., '' ) ) ) .Or. !Empty( Alltrim( SuperGetMv( 'MV_NGPNGR', .F., '' ) ) )
			...
			aAdd( aFieldTmp, { STR0030, 'SIMILAR', 'C', TAMSX3('T9_CODBEM')[1], 0, '@!', .F., STR0023}) // "Pneu Similar" # "Código do bem que será utilizado como referência para gerar pneus."
...
Static Function ModelDef()
	...
	oStTMP:SetProperty( 'SIMILAR', MODEL_FIELD_VALID, FWBuildFeature( STRUCT_FEATURE_VALID, 'MNTA085VLD("SIMILAR")' ) )
	oStTMP:AddTrigger('SIMILAR', 'NOMEPNEU', {|| .T. }, { || Trigger()  })
...
Static Function ViewDef()
	...
	oStTMP:SetProperty( 'SIMILAR', MVC_VIEW_LOOKUP, 'PNEUSI' ) // F3
	oStTMP:SetProperty( 'SIMILAR', MVC_VIEW_CANCHANGE, .T. ) // Alterável
	...
	oStTMP:SetProperty( 'SIMILAR', MVC_VIEW_GROUP_NUMBER, '2' )
...
Function MNTA085VLD( cField )
	...
	ElseIf cField == 'SIMILAR'

		
		If ExistCpo( 'ST9', oModel:GetValue('SIMILAR') )

			dbSelectArea( 'ST9' )
			dbSetOrder( 1 ) // T9_FILIAL + T9_CODBEM
			If dbSeek( xFilial( 'ST9' ) + oModel:GetValue( 'SIMILAR' ) ) .And. ST9->T9_CATBEM != '3'
...
Static Function MNTA085PN( oViewPai )
	...
	Local cSimilar  := oModelPai:GetValue( 'MASTER', 'SIMILAR')
	Local cError    := ''
	Local aValues   := {}
...
Static Function CommitInfo( oModel )
	...
	Local cSimilar   := oModel:GetValue('MASTER', 'SIMILAR')
...
Static Function Trigger()
	...
	Local oModel := FwModelActive()
	Local cRet   := NGSEEK( 'ST9', oModel:GetValue( 'MASTER', 'SIMILAR'), 1, 'T9_NOME' )
