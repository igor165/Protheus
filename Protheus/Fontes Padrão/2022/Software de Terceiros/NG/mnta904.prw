...
Static Function Visualize(cTrb,cAlias)
	...
	//³Pega o primeiro indice / chave da cAlias ³
	...
	If cAlias == "TAF"
		...
		dbSetOrder(2)
		If dbSeek("001"+Substr(oTree:GetCargo(),1,3))
			If (cTRBTAF)->TIPO == "1"
...
Static Function LoadTree(oTree,cTrbTree)
	...
	aAdd( aCampos, { 'RESTRI', 'C', TamSX3( 'TUB_RESTRI' )[1], 0 } )
	aAdd( aCampos, { 'CODPRO', 'C', TamSX3( 'TAF_CODNIV' )[1], 0 } )
	aAdd( aCampos, { 'CODCON', 'C', TamSX3( 'TAF_CODCON' )[1], 0 } )
...
Static Function MarkTree(oTree,cTrbTree,lRecursiva)
	...
	Local cCodPro := SubStr(oTree:GetCargo(),1,3)
	Local nX := 1
	Local nRet := 0
	Local cRestri := ""
	Local nLenSon := 1
...
Static Function LoadMarkTree(aTables,oTree)
	...
	While !Eof()
		...
		If lRet .And. (cTrbTree)->CODPRO != "001"
