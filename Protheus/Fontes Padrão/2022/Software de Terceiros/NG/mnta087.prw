...
Static Function ModelDef()
	...
	oStruct1:SetProperty( 'SIMILAR', MODEL_FIELD_VALID, FWBuildFeature( STRUCT_FEATURE_VALID, 'MNTA087VLD("SIMILAR")' ) )
	oStruct1:AddTrigger('SIMILAR', 'NOMEPNEU', {|| .T. }, { || Trigger()  })
...
Static Function ViewDef()
	...
	oStruct1:SetProperty( 'SIMILAR', MVC_VIEW_LOOKUP, 'PNEUSI' ) // F3
	oStruct1:SetProperty( 'SIMILAR', MVC_VIEW_CANCHANGE, .T. ) // Alterável
	oStruct1:SetProperty( 'SIMILAR', MVC_VIEW_GROUP_NUMBER, '2' )
...
Static Function LoadStart()
	...
	aAdd( aField1, { "Pneu Similar",            'SIMILAR',	 'C', TAMSX3('T9_CODBEM')[1], 0, '@!', STR0004, '' }) //'Código do bem que será utilizado como referência para gerar pneus.'
...
Function MNTA087VLD( cField )
	...
	Local oGrid     := oModel:GetModel('DETAIL')
	Local cSimilar  := oMaster:GetValue('SIMILAR')
	Local nIndex    := 0
	Local nCurrent  := 0
	Local cFamily   := ''
