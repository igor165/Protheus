	...
	ElseIf ::IsCreateSO() // Verifica se é chamado pela Geração de OS.
		...
		If Empty(cError) .And. nRecord > 0
			...
			For nInd := 1 To Len(::aFieldSO[nRecord, __AFIELDSO_SO__])
				...
				// 6.12 - Validações do campo "Real. Início"
					...
					If Empty(cError)
						...
						If ::aFieldSO[nRecord,__AFIELDSO_SO__,nInd,2] == dDataBase
							If ::aFieldSO[nRecord,__AFIELDSO_SO__,nHourIn,2]  > SubStr(Time(),1,5)
								cError := STR0048 // "A 'Hora Inicio Parada Real' é maior que a hora atual."
							EndIf
						ElseIf ::aFieldSO[nRecord,__AFIELDSO_SO__,nDateIn,2] > dDataBase
							cError := STR0049 // "A 'Data Parada Real Inicio' não pode ser maior que a data atual."
						EndIf
					EndIf

				// 6.13 - Validações do campo "Real. Início"
...
Busca uma imagem específica no Banco de Conhecimento
...
@return xBytes, Bytes, Retorna a imagem em Bytes, se não, retorna nulo.
/*/
//------------------------------------------------------------------------------
Method GetFile(cFile) Class MntSR
	...
	cPath := cMsDocPath + cSlash + cFile // Complementa o nome da imagem com o caminho até o diretório
	xBytes := StaticCall( ngwsutil, ReadBytes, cPath)  // Le os Bytes da imagem chamando uma função static do fonte ngwsutil
