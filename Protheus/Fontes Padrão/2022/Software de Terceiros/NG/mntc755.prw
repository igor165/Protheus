...
Static Function fI5CarERP()
	...
	If !Empty(cOP)
		//Busca por Numero da O.S. ou Ordem de Producao
		dbSelectArea("SCP")
		dbGoTop()
		dbSeek(xFilial("SCP"),.T.)
		ProcRegua(LastRec() - RecNo())

		While !Eof() .And. SCP->CP_FILIAL == xFilial("SCP")
			IncProc(STR0183) //"Carregando ERP - Solicita็๕es ao Armaz้m..."

			If AllTrim(SCP->CP_NUMOS) == cOS .Or. AllTrim(SCP->CP_OP) == AllTrim(cOP)
