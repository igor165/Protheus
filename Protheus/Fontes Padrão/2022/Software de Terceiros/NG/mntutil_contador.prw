			...
			If Len(aESTSTZ) > 0
					...
					If dbSeek(cFilCon +  aESTSTZ[mm,1])
						If ST9->T9_TEMCONT $ 'P/I'
							If aSCAN(aESTREC,{|x| x[1] == aESTORD[mm][1]}) = 0
								...
								Aadd(aESTREC,{aESTORD[mm][1],aESTORD[mm][2],aESTORD[mm][3],aESTORD[mm][6], STZ->TZ_BEMPAI})
							EndIf
						EndIf
					Endif
				Next mm
				...
				For nn := 1 To Len(aESTREC)
					...
					If dbSeek(cFilCon+aESTREC[nn][1])
						...
						aAdd( aComponents, { aESTREC[nn,1], (vARQCON[2])->( Recno() ), nDiffFilho, lViraFilho, nPOSCONT, dLEIT, nTIPOC, .T., .T., Nil, cFilCon, cFilHis } )
...
@param lSKIP        , Lógico  , Indica se dever selecionar um registro anterior ao atual ( bem/filho).
		...
		While !Eof() .And. cFilHist == &(aARQHist[2]) .And. &(aARQHist[3]) == cVBem
			...
			If !lAcumulou
				...
				If cTipLanAtu == "Q" .Or. cTipLanAtu == "V"
					...
					dbSkip(-1)
					If &(aARQHist[2]) == cFilHist .And. &(aARQHist[3]) == cVBem
						//Se o atual for uma Quebra, o acumulado atual e' o mesmo do anterior
						If cTipLanAtu == "Q"
							nACUMSTP := &(aARQHist[5])
							lAcumulou := .T.
			...
			EndIf
			If !lAcumulou
				//Se for controlado pelo Bem Pai, verifica se o esse mesmo reporte para o Pai foi uma Virada
				If &(aARQCont[8]) == "P"
					aAreaSSTP  := STP->(GetArea())
					aAreaSave  := GetArea()
			...
			//Se ja' acumulou o contador, acrescenta a posicao atual
			If lAcumulou
				If !lEntrada
					nACUMSTP += If(cTipLanAtu == "Q", 0, &(aARQHist[4])) //Numa quebra, o contador acumulado e' o mesmo do anterior
				EndIf
			Else //Se nao acumulou o contador, faz o calculo normal
...
Function NGGetCont( cCodBem, dData, cHora, cLanex, lCont, cFilStp, cFather )
	...
	If lLanex .And. lCont
		cDtHrAux := fLastMove( cCodBem, dDataLei, cHora )
	EndIf

	If lCont .And. !Empty( cDtHrAux )

		// busca ultimo contador (movimentação ou dos tipos AIQV)
		BeginSql Alias cAliasQry
			...
			WHERE STP.TP_CODBEM = %exp:cCodBem%
				...
				AND ( ( STP.TP_DTLEITU || STP.TP_HORA <= %exp:dDataLei+cHora%
						AND STP.TP_TIPOLAN IN ( 'A', 'I', 'Q', 'V'  ) )
					OR  ( STP.TP_DTLEITU || STP.TP_HORA = %exp:cDtHrAux% ) )
	...
	If !lPROBPA
		...
		If aVETOR[1]
			...
			If !NGBlCont( cVBEM, .F. )  // Se controla apenas por abastecimento
				While !Eof()// valida com o próximo registro de Abastecimento, Quebra ou Virada
					If !(&(vARQHI[7]) $ "AQV") .And.  !(STP->TP_DTLEITU == dVDATA .And. STP->TP_HORA == cHORA)
						Dbskip()
					Else
						Exit
					EndIf
...
Function NGVALIVARD(cVBEM,nPOSCON,dVLEIT,cVHORA,nTIPOC,lSAIDA,nITEMC,cFilTroc)
	...
	If ExistBlock("NGCHECVAR")
		xValor := ExecBlock( "NGCHECVAR", .F., .F., { lSAIDA, cVBEM, nPOSCON, dVLEIT, cVHORA, nTIPOC, nVARDCAL, nVARMEN, aVETCHK } )
...
Function NGAUTONOMIA(cBem,dData,cHora,nPosCont,lGetVar,cCombus,nTIPC,cFiliST9, lPerg)
	...
	Local cAutonomia := GetNewPar("MV_NGAUTON"," ")
	Local cMsgAuto   := ''
	...
	/*1- Nao consite autonomia
	2- Consistir sempre independente da rotina
	3- Consistir somente nas rotinas de abastecimento*/
	If (cAutonomia == '1') .Or. (cAutonomia == '3' .And. FunName()!='MNTA635' .And. FunName()!='MNTA655' .And.;
			FunName()!='MNTA700' .And. FunName()!='MNTA656')
		Return IIf(lGetVar,.T.,{.T.,' '})
	Endif
...
Function NGIncTTV(cPosto,cLoja,cTanque,cBomba,dData,cHora,cTipoLa,nPosCon,nQuanti,cNAbast,cNumDoc,cSerieDoc)//,cFunc)
	...
	If dbSeek(xFilial("TQJ")+cPosto+cLoja+cTanque+cBomba)
		...
		dbSelectArea("TTV")
		dbSkip()
		While !Eof() 	.And.	xFilial("TTV") == TTV->TTV_FILIAL .And. TTV->TTV_POSTO == cPosto .And. TTV->TTV_LOJA == cLoja ;
		.And. TTV->TTV_TANQUE == cTanque .And. TTV->TTV_BOMBA == cBomba

			If TTV->TTV_MOTIVO != "3"
				RecLock("TTV",.F.)
				TTV->TTV_ACUMCO += nQuanti
				If (TTV->TTV_POSINI + nQuanti) > TQJ->TQJ_LIMCON //Indica se havera uma virada
				...
				If (TTV->TTV_POSFIM + nQuanti) > TQJ->TQJ_LIMCON //Indica se havera uma virada
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Fun‡…o    ³ NGCONTPER ³ Autor ³ Evaldo Cevinscki Jr. ³ Data ³12/11/2007³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descri‡…o ³Calcula o Km percorrido entre dois abastecimentos.          ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³ Uso      ³ SigaMNT                                                    ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³         ATUALIZACOES SOFRIDAS DESDE A CONSTRU€AO INICIAL.             ³±±
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
Function NGCONTPER(cBemF,cDtCon,cHrCon)
	Local nConAnt := 0, nContAtu := 0

	cBemF := cBemF + Space(Len(ST9->T9_CODBEM)-Len(cBemF))
	dbSelectArea("STP")
	dbSetOrder(9)
	dbSeek(cBemF+cDtCon+cHrCon,.T.)
	While !Eof() .And. STP->TP_CODBEM == cBemF .And. STP->TP_DTLEITU == StoD(cDtCon) .And. STP->TP_HORA == cHrCon
		If (STP->TP_TIPOLAN == 'A' .Or. STP->TP_TIPOLAN == 'I' )
			nContAtu := STP->TP_ACUMCON
			Exit
		EndIf
		dbSelectArea("STP")
		dbSkip()
	End
	If !Eof() .And. STP->TP_CODBEM == cBemF
		If !Bof()
			dbSkip(-1)
		EndIf
		While !Bof()
			If STP->TP_CODBEM == cBemF
				If (STP->TP_TIPOLAN == 'A' .Or. STP->TP_TIPOLAN == 'I' )
					nConAnt := STP->TP_ACUMCON
					Exit
				EndIf
			Else
				Exit
...
Static Function fUpdCnt(aCampos, cTabela, cBem, cField, dData, cHora, cCont )
	...
	If cTabela $ "TQ2/HTJ/TPN/TPW/TQA/TQB/TQN/TR9/TUZ/TTI"
	...
	dbUseArea(.T., "TOPCONN", TcGenQry(,,cSelect), cAliasVl, .F., .T.)
	...
	If (cAliasVl)->(!EoF())
		...
		cUpdate +=     " WHERE "
		cUpdate +=         "R_E_C_N_O_ = " + ValToSQL((cAliasVl)->REC)
		...
		//----------------------------------------------------
		If cTabela == 'STJ' .And. (cAliasVl)->TJ_PLANO != '000000' .And. (cAliasVl)->TJ_TERMINO == 'S'

			// Atualiza manutenção
			NGATUMANUT( (cAliasVl)->TJ_CODBEM, (cAliasVl)->TJ_SERVICO, (cAliasVl)->TJ_SEQRELA,,,,,,(cAliasVl)->TJ_FILIAL )

			// Atualiza manutenções que foram substituídas, aglutinadas
			dbSelectArea("STJ")
			dbSetOrder(1)
			If dbSeek( (cAliasVl)->TJ_FILIAL + (cAliasVl)->TJ_ORDEM + (cAliasVl)->TJ_PLANO ) .And.;
				( !Empty( (cAliasVl)->TJ_SUBSTIT ) .Or. SuperGetMV("MV_NG1SUBS", .F., "1" ) == "2" )
				NGAgluFim()
			EndIf
...
//----------------------------------------------------------------
/*/{Protheus.doc} fLastMove
Retorna data de ultima movimentação

@param cCodBem, string, código do bem
@param dDataLei, date, data referencia
@param cHora, string, hora referencia
...
/*/
//----------------------------------------------------------------
Static Function fLastMove( cCodBem, dDataLei, cHora )

	Local cDtHrAux := ""
	Local cAliasSTZ := GetNextAlias()

	// Trecho abaixo busca data de movimentação anterior
	BeginSql Alias cAliasSTZ

		SELECT DATAX, HORAX
		FROM
		(
			SELECT STZ.TZ_DATAMOV DATAX, STZ.TZ_HORAENT HORAX
			FROM %Table:STZ% STZ
			WHERE STZ.%notDel%
				AND STZ.TZ_CODBEM = %exp:cCodBem%
				AND STZ.TZ_DATAMOV || STZ.TZ_HORAENT <= %exp:dDataLei+cHora%
				AND STZ.TZ_TIPOMOV = 'E'
				AND STZ.TZ_FILIAL = %xFilial:STZ%

			UNION

			SELECT STZ.TZ_DATASAI DATAX, STZ.TZ_HORASAI HORAX
			FROM %Table:STZ% STZ
			WHERE STZ.%notDel%
				AND STZ.TZ_CODBEM = %exp:cCodBem%
				AND STZ.TZ_DATASAI || STZ.TZ_HORASAI <= %exp:dDataLei+cHora%
				AND STZ.TZ_TIPOMOV = 'S'
				AND STZ.TZ_FILIAL = %xFilial:STZ%
		) TAB_STZ ORDER BY DATAX || HORAX DESC

	EndSql

	If !(cAliasSTZ)->( Eof() )
		cDtHrAux := (cAliasSTZ)->DATAX + (cAliasSTZ)->HORAX
	EndIf
