	...
	//	For nAgrupN := 1 to 2
	    	...
	    	For y := 1 to Len(oGetServ:aCols)
				...
				If i == 0
					...
					@ nLin++,0 PSAY  "Serviço "+".: "+( cAliasVO4 )->VO4_CODSER+If(!Empty(aString[1,2]),Space(7)+Str(aString[1,1],6)+" "+aString[1,2]+" "+aString[1,3],"")+If(!Empty(aString[7,2]),Space(1)+Str(aString[7,1],6)+" "+aString[7,2]+" "+aString[7,3],"") //"Servico "
				...
				Else
					...
					@ nLin++,0 PSAY "Serviço "+" "+"Agrupado"+": "+( cAliasVO4 )->VO4_CODSER+" "+"Descrição"+": "+VO6->VO6_DESSER //"Servico "###"Agrupado"###"Descricao"
					@ nLin++,0 PSAY "Grupo"+": "+( cAliasVO4 )->VO4_GRUSER+"-"+Subs(Posicione("VOS",1,xFilial("VOS")+VV1->VV1_CODMAR+( cAliasVO4 )->VO4_GRUSER,"VOS_DESGRU"),1,15)+Space(2)+"Secção"+": "+VOD->VOD_DESSEC+Space(10)+"Tp Padrão"+": "+Transform(( cAliasVO4 )->VO4_TEMPAD,"@R 99:99") //"Grupo"###"Secao"###"Tp Padrao"
