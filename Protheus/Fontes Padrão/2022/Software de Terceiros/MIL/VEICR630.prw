			...
			If VC3->VC3_TIPO == aRetSX1[17] .and. Empty(VC3->VC3_DATVEN) // Veiculos/Equipamentos
				...
				If lImpCli
					...
					If val(aRetSX1[7]) >= 2
						...
						DbSelectArea("VX5")
						DbSetOrder(1)
						DbSeek( xFilial("VX5") + "033" + VCF->VCF_CODSEG )
						cVendedores := ""
						SA3->(DbSetOrder(1))
						...
						If lA1_IBGE
							...
							@ nLin++,00 psay VCF->VCF_CODCLI+"-"+VCF->VCF_LOJCLI+" "+left(SA1->A1_NOME,25)+" "+left(VX5->VX5_DESCRI,15)+" "+left(VAM->VAM_DESCID,25)+" "+VAM->VAM_ESTADO+" "+cVendedores
						Else
							@ nLin++,00 psay VCF->VCF_CODCLI+"-"+VCF->VCF_LOJCLI+" "+left(SA1->A1_NOME,25)+" "+left(VX5->VX5_DESCRI,15)+" "+left(SA1->A1_MUN,25)+" "+SA1->A1_EST+" "+cVendedores
						EndIf
					EndIf
