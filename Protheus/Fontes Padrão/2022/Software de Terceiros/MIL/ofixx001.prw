...
If !INCLUI // deve-se montar a acols com os registros da base
	...
	While !eof() .and. VS3->VS3_FILIAL == xFilial("VS3").and. VS3->VS3_NUMORC == VS1->VS1_NUMORC
		...
		If SF4->F4_OPEMOV == "05" .and. ALTERA
			...
			aTempPro := OX005PERDES(SBM->BM_CODMAR,VS1->VS1_CENCUS,VS3->VS3_GRUITE,VS3->VS3_CODITE,VS3->VS3_QTDITE,VS3->VS3_PERDES,.f.,VS1->VS1_CLIFAT,VS1->VS1_LOJA,IIF(VS1->VS1_TIPORC == "2","3",VS1->VS1_TIPVEN),VS3->VS3_VALTOT/VS3->VS3_QTDITE,2,VS1->VS1_FORPAG )
			reclock("VS3",.f.)
	...
	for nCntFor = 1 to Len(oGetPecas:aCols)
		...
		if lValMaior
			...
			if !oGetPecas:aCols[nCntFor,Len(oGetPecas:aCols[nCntFor])]
				...
				nValMin := OX005RETMIN(SBM->BM_CODMAR,M->VS1_CENCUS,M->VS3_GRUITE,M->VS3_CODITE,M->VS3_QTDITE,M->VS3_PERDES,.f.,M->VS1_CLIFAT,M->VS1_LOJA,M->VS1_TIPVEN,M->VS3_VALPEC,2,M->VS1_FORPAG,,.f.)
...
===============================================================================
*/
Function OX001FPOK(lMensagem, lLibPv, lRefNF, lRecalc, lMsgLotMin)
...
if  ( oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALPEC","aHeaderP")] == 0 )     .or.;
	...
	If SF4->F4_OPEMOV == "05"
		...
		aTempPro := OX005PERDES(SBM->BM_CODMAR,M->VS1_CENCUS,cGrupoVal,cCodIteVal,M->VS3_QTDITE,M->VS3_PERDES,.f.,M->VS1_CLIFAT,M->VS1_LOJA,IIF(M->VS1_TIPORC == "2","3",M->VS1_TIPVEN),M->VS3_VALTOT/M->VS3_QTDITE,2,M->VS1_FORPAG, GetNewPar("MV_FORMALO","") )
		nValPec := aTempPro[1]
		nMarLuc := aTempPro[2]
	else
		nValPec := 0
...
if oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_QTDITE","aHeaderP")] > 0 
	...
	If lAltValorPeca
		...
		If SF4->F4_OPEMOV == "05"
			...
			Endif
			oGetPecas:aCols[oGetPecas:nAt,FG_POSVAR("VS3_VALPEC","aHeaderP")] := M->VS3_VALPEC
			aTempPro := OX005PERDES(SBM->BM_CODMAR,M->VS1_CENCUS,M->VS3_GRUITE,M->VS3_CODITE,M->VS3_QTDITE,M->VS3_PERDES,.f.,M->VS1_CLIFAT,M->VS1_LOJA,IIF(M->VS1_TIPORC == "2","3",M->VS1_TIPVEN),M->VS3_VALTOT/M->VS3_QTDITE,2,M->VS1_FORPAG, GetNewPar("MV_FORMALO","") )
			nValPec := aTempPro[1]
			nMarLuc := aTempPro[2]
			nDesPro := aTempPro[3]
			If len(aTempPro) > 3
...
@ aPosObj[1,1]+05,10 SAY STR0200  SIZE 150,8 OF oSugest PIXEL COLOR CLR_BLUE // Forma de Pedido
@ aPosObj[1,1]+04,60 COMBOBOX oLimite  VAR cForPed ITEMS aRStatus SIZE 100,8 OF oSugest  PIXEL
	...
	if aSugest[nCntFor,1] // selecionou no listbox
		...
		if DBSeek(xFilial("VEJ")+aSugest[nCntFor,6])
			while xFilial("VEJ")+aSugest[nCntFor,6] == VEJ->VEJ_FILIAL + VEJ->VEJ_CODMAR
				if VEJ->VEJ_PROGRA == cForPed .and. VEJ->VEJ_PEDOSV != "1"   
					If VEJ->VEJ_TIPPED == oPedido:GtTpPedBO() .and. !lRegBO 
						DbSkip()
						Loop						
					Endif
					lAchou := .t.
...
Else
	...
	While !eof() .and. xFilial("VS3") + VS1->VS1_NUMORC == VS3->VS3_FILIAL + VS3->VS3_NUMORC
		...
		If SF4->F4_OPEMOV == "05"
			...
			//lRetDes := OX005PERDES(VS1->VS1_CODMAR,VS1->VS1_CENCUS,VS3->VS3_GRUITE,VS3->VS3_CODITE,VS3->VS3_QTDITE,VS3->VS3_PERDES,.t.,VS1->VS1_CLIFAT,VS1->VS1_LOJA,IIF(VS1->VS1_TIPORC == "2","3",VS1->VS1_TIPVEN),VS3->VS3_VALTOT/VS3->VS3_QTDITE,,VS1->VS1_FORPAG)
			...
			lRetDes := OX005PERDES(SBM->BM_CODMAR, VS1->VS1_CENCUS, VS3->VS3_GRUITE, VS3->VS3_CODITE,;
				...
				IIF(VS1->VS1_TIPORC == "2", "3", VS1->VS1_TIPVEN), VS3->VS3_VALTOT / VS3->VS3_QTDITE,, VS1->VS1_FORPAG)
			...
			While !eof() .and. xFilial("VS3") + VS1->VS1_NUMORC == VS3->VS3_FILIAL + VS3->VS3_NUMORC
				...
				If !(aIDescont[nItVS7]) .or. lVS7SemProb // Cria VS7 de todos os Itens se pelo menos 1 tiver problema de Margem ou Desconto
					...
					aRetDes := OX005PERDES(SBM->BM_CODMAR, VS1->VS1_CENCUS, VS3->VS3_GRUITE ,VS3->VS3_CODITE,;
						VS3->VS3_QTDITE, VS3->VS3_PERDES, .t., VS1->VS1_CLIFAT, VS1->VS1_LOJA,;
						IIF(VS1->VS1_TIPORC == "2", "3", VS1->VS1_TIPVEN), VS3->VS3_VALTOT / VS3->VS3_QTDITE, 3, VS1->VS1_FORPAG)
...
===============================================================================
*/
Static Function OX001OFVS6(cTipGer)
