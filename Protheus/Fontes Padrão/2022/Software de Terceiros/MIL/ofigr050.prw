	  ...
	  For nIte:=0 to 1
		  ...
		  If VG5->(Dbseek(xFilial("VG5")+VGA->VGA_CODMAR+VGA->VGA_NUMOSV ))
	       ...
	       @ FS_QUEBPAG(),011 PSAY If(nIte==0,STR0026,STR0027)  //ITENS NORMAIS","ITENS ADICIONAIS"
            ...
            //Renata
		      If !Empty(VG5->VG5_CAUSA)
			      @ FS_QUEBPAG(),011 PSAY STR0067 //"Causa: "
			      @ nLin,019 PSAY SUBS(Alltrim(VG5->VG5_CAUSA),1,71)
			      for nCntFor = 72 to Len(Alltrim(VG5->VG5_CAUSA)) step 90
			      	nLin++
			      	@ nLin,019 PSAY SUBS(Alltrim(VG5->VG5_CAUSA),nCntFor,90)
			      next
			      nLin ++
		      EndIf       
		          
				//Renata
		      If !Empty(VG5->VG5_REPARO)
			      @ FS_QUEBPAG(),011 PSAY STR0068 //"Reparo: "
			      @ nLin,019 PSAY SUBS(Alltrim(VG5->VG5_REPARO),1,71)
			      for nCntFor = 72 to Len(Alltrim(VG5->VG5_REPARO)) step 90
			      	nLin++
			      	@ nLin,019 PSAY SUBS(Alltrim(VG5->VG5_REPARO),nCntFor,90)
			      next
			      nLin ++
		      EndIf   
		              
				//Renata
		      If !Empty(VG5->VG5_OUTROS)
			      @ FS_QUEBPAG(),011 PSAY STR0069 //"Outros: "
			      @ nLin,019 PSAY SUBS(Alltrim(VG5->VG5_OUTROS),1,71)
			      for nCntFor = 72 to Len(Alltrim(VG5->VG5_OUTROS)) step 90
			      	nLin++
			      	@ nLin,019 PSAY SUBS(Alltrim(VG5->VG5_OUTROS),nCntFor,90)
			      next
			      nLin ++
		      EndIf           
