...
Local cTitCpo := ""
//
oReport := TReport():New("ORDBUSCC","Transferencia de Peças entre OSs",nil,{|oReport| OBUSCC_Impressao(oReport)}) // Transferencia de Peças entre OSs
oSection1 := TRSection():New(oReport,"Item",{}) // Item
oSection2 := TRSection():New(oReport,"Transferência",{}) // Transferência
oSection3 := TRSection():New(oReport,"Usuário",{}) // Usuário
//
...
For nCntFor := 1 to len(aCposReq) // Campos do Destino - Requisicao
	...
	If aCposReq[nCntFor] == "VO3_NUMOSV"
		cTitCpo := "OS Destino" // OS Destino
	EndIf
	TRCell():New(oSection2,"R"+aCposReq[nCntFor],,cTitCpo,aHeadVO3[nPosCpo,3],aHeadVO3[nPosCpo,4],,&("{|| cR"+aCposReq[nCntFor]+" }") )
Next
TRCell():New(oSection2,"Nome",,"Nome Produtivo","@!",25,,&("{|| cRNomProReq }")) // Nome Produtivo
TRFunction():New(oSection2:Cell("DVO3_QTDREQ"),NIL,"SUM") // Totalizador
//
TRCell():New(oSection3,"Usuário",,"Usuário","@!",100,,&("{|| cObsUsuario }")) // Codigo Usuário / Nome Usuario / Data / Hora
//
