...
±±³Funcao    ³ VEIXA026 ³ Autor ³ Thiago                            ³ Data ³ 10/01/11  ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descricao ³ Entrada de Veiculos por Retorno de remessa (fonte duplicado do VEIXA006)³±±
...
Private cBrwCond := 'VVF->VVF_OPEMOV=="7"' // Condicao do Browse, validar ao Incluir/Alterar/Excluir
...
dbSetOrder(1)
//
FilBrowse('VVF',{},'VVF->VVF_OPEMOV=="7"') // Filtra os retornos de remessa
...
±±³Funcao    ³ VXA006A  ³ Autor ³ Andre Luis Almeida / Luis Delorme ³ Data ³ 26/01/09 ³±±
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Function VXA006A(cAlias,nReg,nOpc)
//
// If &cBrwCond // Condicao do Browse, validar ao Incluir/Alterar/Excluir
	...
	Else // VISUALIZACAO E CANCELAMENTO
		DBSelectArea("VVF")
		dbclearFilter()
		VEIXX000(,,,nOpc,"7",,,"C")	// VEIXX000(xAutoCab,xAutoItens,xAutoCP,nOpc,xOpeMov)
		FilBrowse('VVF',{},'VVF->VVF_OPEMOV=="7"')
	EndIf
// EndIf
//
Return .t.
...
±±³Funcao    ³VXA006BRVV0 ³ Autor ³Andre Luis Almeida / Luis Delorme³ Data ³ 26/01/09 ³±±
...
Private cBrwCond2 := 'VV0->VV0_OPEMOV=="3" .AND. VV0->VV0_SITNFI=="1" .AND. !Empty(VV0->VV0_NUMNFI) .AND. xFilial("VV0")==VV0->VV0_FILIAL .AND. VXA006FILA()' // Condicao do Browse, validar ao Incluir/Alterar/Excluir
//
aRotina := {;
{ STR0002 ,"axPesqui", 0 , 1,,.f.},;	// Pesquisar
{ STR0012 ,"VXA006DEVA('"+cFilAnt+"')", 0 , 1,,.f.}}	// Retornar
//
dbSelectArea("VV0")
dbSetOrder(4)
//
FilBrowse('VV0',{},'VV0->VV0_OPEMOV=="3" .AND. VV0->VV0_SITNFI=="1" .AND. !Empty(VV0->VV0_NUMNFI) .AND. xFilial("VV0")==VV0->VV0_FILIAL .AND. VXA006FILA()')
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Funcao    |VXA006FILA³ Autor ³ Andre Luis Almeida / Luis Delorme ³ Data ³ 19/03/09 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Funcao    |VXA006DEVA| Autor ³Andre Luis Almeida / Luis Delorme  ³ Data ³ 26/01/09 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
...
If &cBrwCond2 // Condicao do Browse 2, validar ao Devolver
	...
	aAdd(aParamBox,{1,STR0015,Space(TamSX3("F4_CODIGO")[1]),"","VXA006TES()","SF4","",0,.T.}) // TES
	...
	while !eof() .and. xFilial("VVA")+VV0->VV0_NUMTRA == VVA->VVA_FILIAL + VVA->VVA_NUMTRA
		...
		If ! FGX_VV1SB1("CHAINT", VV1->VV1_CHAINT , /* cMVMIL0010 */ , cGruVei )
			FMX_HELP("VA026E01", STR0038) // Veiculo nao encontrado
			Return .f.
		endif
		DBSelectArea("SD2")
		DBSetOrder(3)
	...
	lMsErroAuto := .f.
	//
	MSExecAuto({|x,y,w,z,k,l| VEIXX000(x,y,w,z,k,l,,"C")},xAutoCab,xAutoItens,{},3,"7",xAutoAux )
	//
	If lMsErroAuto
		DisarmTransaction()
		MostraErro()
...
{ OemtoAnsi(STR0003) ,"VXA006A"     		, 0 , 2},;		// Visualizar
{ OemtoAnsi(STR0004) ,"VXA006A"    		, 0 , 3},;		// Devolver
{ OemtoAnsi(STR0005) ,"VXA006A"    	 	, 0 , 5},;		// Cancelar
{ OemtoAnsi(STR0006) ,"VXA006LEGA" 	 	, 0 , 6},;		// Legenda
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³Funcao    ³VXA006LEGA³ Autor ³ Andre Luis Almeida / Luis Delorme ³ Data ³ 26/01/09 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descricao ³ Legenda - Entrada de Veiculos por Retorno de remessa                   ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Uso       ³ Veiculos                                                               ³±±
