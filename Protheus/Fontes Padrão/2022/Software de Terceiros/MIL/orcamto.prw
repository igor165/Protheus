...
Static Function RptDefOrc()
	...
	TrCell():New(oSection7, "B1_COD",     "SB1", "Código",,     40)
...
Static Function RunRptOrc(oReport)
	...
	If !(PECORC->(EOF()))
		...
		While !(PECORC->(EOF())) .and. PECORC->VS3_NUMORC == cOrc .AND. PECORC->VS3_FILIAL == xFilial("VS3")
			...
			oSection7:Cell("B1_COD"):SetValue(PECORC->VS3_CODITE)
	...
	if VS1->VS1_TIPORC == "2" .AND. !(SERVORC->(EOF()))
		...
		Do While !(SERVORC->(EOF())) .and. SERVORC->VS4_NUMORC == cOrc
			...
			oSection10:Cell("VO6_CODSER"):SetValue(IIf(MV_PAR05 # 1, SERVORC->VS4_CODSER, " "))
	...
	if MV_PAR03 == 2
		oSection15:Cell("TOTAL"):SetValue(/*Transform(*/nTotPec+nTotSer+VS1_DESACE+VS1_VALFRE-nTotDesP/*,"@E 9999,999,999.99")*/)
	Else
		oSection15:Cell("TOTAL"):SetValue(nTotPec+nTotSer+VS1_DESACE+VS1_VALFRE)
	Endif
	If !Empty(VS1->VS1_FORPAG)
