...
While !(cAliasSD2)->(Eof())
   ...
   Endif
	...
	EndIf
	cQuery := "SELECT SF4.F4_DUPLIC, SF4.F4_ESTOQUE, SF4.F4_OPEMOV, SF4.F4_PISCRED, SF4.F4_PISCOF, SF4.F4_INCSOL "
	...
	if !Empty((cAliasSD2)->D2_NFORI) .and. Alltrim((cAliasSF4)->F4_OPEMOV) == "09" .and. Alltrim((cAliasSD2)->D2_TIPO) == "D"
		...
		If ExistBlock("ORVW02E3")
			...
			If (cAliasSBM)->BM_GRUPO $ cGAce  .or. (cAliasSBM)->BM_CODMAR $  cMarcaVW .OR. (cAliasSBM)->BM_CODMAR $  cMarcaMAN
				aPPA[47,8] +=(cAliasSD2)->D2_TOTAL + (cAliasSD2)->D2_DESPESA + (cAliasSD2)->D2_VALFRE + (cAliasSD2)->D2_SEGURO + (cAliasSD2)->D2_VALIPI + IIF((cAliasSF4)->F4_INCSOL = "S",(cAliasSD2)->D2_ICMSRET,0)
			Else
				aPPA[48,8] +=(cAliasSD2)->D2_TOTAL + (cAliasSD2)->D2_DESPESA + (cAliasSD2)->D2_VALFRE + (cAliasSD2)->D2_SEGURO + (cAliasSD2)->D2_VALIPI + IIF((cAliasSF4)->F4_INCSOL = "S",(cAliasSD2)->D2_ICMSRET,0)
			EndIf
		endif	
	endif
	DbSelectArea(cAliasSD2)
