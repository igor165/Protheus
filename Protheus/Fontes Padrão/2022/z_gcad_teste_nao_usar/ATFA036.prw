...
STATIC lAtf030		:= FWIsInCallStack('ATF030_020')
...
Local nVisualiza	:= 2
Local cFilter		:= ""
Local lAF036BRW 	:= ExistBlock("AF036BRW")
...
If !__lRotAuto
	...
	/*
	 * Verifica se será exibido na Browse as Baixas ou os tipos de ativos
	 */
	If nVisualiza == 2
...
oModel:SetRelation('FN7VALOR',{{'FN7_TIPO','FN7_TIPO'},{'FN7_TPSALD','FN7_TPSALD'}}, FN7->(IndexKey(2)) )
...
If mv_par04 == 2 .Or. MV_PAR04 == 3 .Or.lValidCpo
	...
	If __nOper == OPER_BAIXA
		 ...
		 */
		If SN1->N1_TPCTRAT $ "2/3" .AND. !FWIsInCallStack('ATFA320')
...
Local oModelMaster	:= oModel:GetModel(If(FWIsInCallStack("AF036BxLote") .or. FWIsInCallStack('ATF036_020') .or. lAtf030,"FN6ATIVOS", "FN6MASTER"))
...
Local cNFItem		:= oModelMaster:GetValue('FN6_ITEMNF')
	...
	If lRet .And. lGeraNF
		...
		If lRet
			If !Empty(cTESSaida)
				...
				//--------------------------------------------------------------
				// Verifica se a TES esta configura para não movimentar estoque
				//--------------------------------------------------------------
				If lRet .And. GetAdvFVal("SF4","F4_ESTOQUE",XFilial("SF4")+cTESSaida,1,"") <> 'N'
					lRet := .F.
					Help(" ",1, "AF036Pos5",,STR0099,1,0) //"O Tipo de Saida não pode atualizar estoque para a geração da nota fiscal."
				EndIf
...
If FWIsInCallStack("AF036BxLote") .Or. FWIsInCallStack('ATF036_020') .or. lAtf030
 __nOper := 3
EndIf
 ...
 * Gatilhos
 */
oStr2:AddTrigger( 'OK', 'OK', { || .T. } , {|| AF036DtBx()  } )
...
Local lBxLote			:= FWIsInCallStack("AF036BxLote") .or. FWIsInCallStack('ATF036_020') .or. lAtf030
...
If oModel != Nil .AND. oModel:IsActive()
	...
	oModelMaster	:= oModel:GetModel(If(FWIsInCallStack("AF036BxLote") .Or. FWIsInCallStack('ATF036_020') .or. lAtf030,"FN6ATIVOS", "FN6MASTER"))
	oModelValor		:= oModel:GetModel('FN7VALOR')
...
Local oModelFN6		:= oModelPai:GetModel(If(FWIsInCallStack("AF036BxLote") .Or. FWIsInCallStack('ATF036_020') .or. lAtf030,"FN6ATIVOS", "FN6MASTER"))
...
Local cGeraNF			:= oModelMaster:GetValue("FN6_GERANF")	// Codigo Base
...
If __nOper == OPER_BAIXA
	...
	For nX := 1 to nQtdMoedas
		...
		If lLoad
			...
			If oModelValor:Length() > 5
				oModelValor:ClearData(.T.)
			EndIf
...
If __nOper == OPER_BAIXA .Or. __nOper == OPER_BXLOT
	...
	If oModelPrinci:getid() == "ATFA036L"
		...
		cFilOri	:= FWxFilial("SN3")
		SN3->( dbSeek( cFilOri + oModelTipo:GetValue("FN7_CBASE",nLineTp) + oModelTipo:GetValue("FN7_CITEM",nLineTp) + oModelTipo:GetValue("FN7_TIPO",nLineTp) + "0" + oModelTipo:GetValue("FN7_TPSALD",nLineTp) + oModelTipo:GetValue("FN7_SEQ",nLineTp) + oModelTipo:GetValue("FN7_SEQREA",nLineTp)) )
	...
	lPropBx := oModelMaster:GetValue('FN6_DEPREC') == "1"
			...
			For nX := 2 To nQtdMoedas
				...
				If oModelAcum:SeekLine({{"TPATIVO",SN3->N3_TIPO},{"TPSALDO",SN3->N3_TPSALDO},{"MOEDA",PADL(nX, nTamMoeda,"0")}})
					IF &('SM2->M2_MOEDA'+cMoeda) > 0
						oModelAcum:SetValue("TXMEDIA", oModelAcum:GetValue("TXMEDIA") + &('SM2->M2_MOEDA'+cMoeda))
						oModelAcum:SetValue("ADIAS", oModelAcum:GetValue("ADIAS") + 1 )
					EndIf
	
					/*
					* Calcula a Taxa Média
					*/
					If oModelAcum:GetValue("TXMEDIA") == 0 .OR. oModelAcum:GetValue("ADIAS") == 0
						oModelAcum:SetValue("ADIAS", 1)
						oModelAcum:SetValue("TXMEDIA", 1)
					Else
						oModelAcum:SetValue("TXMEDIA",oModelAcum:GetValue("TXMEDIA") / oModelAcum:GetValue("ADIAS")  )
					EndIf
				EndIf
			Next nX
			SM2->(dbSkip())
		...
		If (!lAtfctap .Or. ! (cTipDepr $ '4/5/8/9')) .And. !(SN3->N3_TIPO $ '03/13' )
			...
			If lAtClDepr .OR. EMPTY(SN1->N1_PATRIM)
				...
				*/
				...
				If (Empty(dBloqueio) .Or. Left(Dtos(dBloqueio), 6) <= Left(Dtos(dBaixa036), 6)) .And. !SN1->N1_STATUS $ "2|3"
					...
					If Abs(SN3->N3_VRDACM1 + SN3->N3_VRCDA1) < Abs(SN3->N3_VORIG1+SN3->N3_VRCACM1+SN3->N3_AMPLIA1)
						If aTaxaMes[1] > 0
							...
							If (oModelMaster:GetValue("FN6_DEPREC") != '0') .And. (  dData > SN3->N3_DINDEPR )
								...
								Else
									...
									Else
										For nX := 1 To nQtdMoedas
											oModelAcum:GoLine(nX)
											If &("SN3->N3_TXDEPR" + CVALTOCHAR(nX)) > 0.00
												If nX == 1
					...
					If oModelMaster:GetValue("FN6_DEPREC") != '0' .And. (  dData > SN3->N3_DINDEPR )
						...
						For nX:= 2 to nQtdMoedas
							...
							If &("SN3->N3_TXDEPR" + CVALTOCHAR(nX)) > 0.0000
	...
	For nX := 1 To nQtdMoedas
		...
		*/
		If oModelAcum:SeekLine({{"TPATIVO",oModelTipo:GetValue("FN7_TIPO")},{"TPSALDO",oModelTipo:GetValue("FN7_TPSALD")},{"MOEDA",PADL(nX, nTamMoeda,"0")}})
			...
			oModelValor:SeekLine({{"FN7_TIPO",oModelTipo:GetValue("FN7_TIPO")},{"FN7_TPSALD",oModelTipo:GetValue("FN7_TPSALD")},{"FN7_MOEDA",PADL(nX, nTamMoeda,"0")}})
...
If __nOper == OPER_BAIXA	
	...
	BEGIN TRANSACTION
		...
		//---------------------------------------------------------
		If lOK .And. oModelMaster:GetValue("FN6_GERANF") == "1"
			...
			For nCtnTipo := 1 to oModelTipo:Length()
				...
				*/
				If oModelTipo:GetValue("OK" , nCtnTipo)
					...
					*/
					cSeq := Af036Grava(cAlsAtu, cNota, cSerie,nQuant,nQtdOrig,lCtbBxInt,cIDMOV,lMovCiap,cNFItem,dBaixa036,cMotivo,oModel,lQuant,@nHdlPrv,@nTotal,cLanc)
	...
	Else
		...
		BEGIN TRANSACTION
			...
			If lOk .And. !Empty(FN6->FN6_NUMNF) .And. !Empty(FN6->FN6_SERIE)
				lOk := A036VlNota(FN6->FN6_NUMNF,FN6->FN6_SERIE)
			EndIf
					...
					While lOK .And. FN7->(!Eof()) .AND. cFilFN7 + FN6->FN6_CODBX == FN7->FN7_FILIAL + FN7->FN7_CODBX										
						...
						If oModelTipo:SeekLine(aBusca)
							If oModelTipo:GetValue("OK")
								If FN7->FN7_MOEDA == '01'
									lOk := AF036Cance(FN6->FN6_CBASE,FN6->FN6_CITEM,FN7->FN7_TIPO,FN7->FN7_TPSALD,FN6->FN6_DTBAIX,FN7->FN7_SEQ,FN7->FN7_MOTIVO,@nHdlPrv,@nTotal,FN7->FN7_CODBX)
								EndIf
								FN7->(RecLock("FN7",.F.))
								FN7->FN7_STATUS := '2'
								FN7->(MsUnLock())
...
/*/
//-------------------------------------------------------------------
Function AF036DtBx(lDesmarca,lDtLote)
Local oModel			:= FWModelActive()
...
Local dUltDepr			:= GetMV("MV_ULTDEPR")							// Data Ult Depreciacao
...
If __nOper != OPER_VISUA .AND. __nOper != OPER_CANC .And. __nOper != OPER_CANLT
	...
	cTipoSLD 	:= oModelTipo:GetValue("FN7_TPSALD")
	SN3->(dbSetOrder(11)) //N3_FILIAL+N3_CBASE+N3_ITEM+N3_TIPO+N3_BAIXA+N3_TPSALDO+N3_SEQ+N3_SEQREAV
	If RusCheckRevalFunctions() .Or. SN3->(dbSeek(cFilOri + cBase + cItem + cTipo + '0' + cTipoSLD ))
...
/*/
//-------------------------------------------------------------------
Function Af036Grava(cAlias, cNota, cSerie,nQuant,nQtdOrig,lCtbBxInt,cIDMOV,lMovCiap,cNFItem,dBaixa036,cCodMot,oModel,lQuant,nHdlPrv,nTotal,cLanc)
Local aArea			:= GetArea()
Local oModelValor	:= oModel:GetModel("FN7VALOR")
Local oModelMaster  := oModel:GetModel(If(FWIsInCallStack("AF036BxLote") .Or. FWIsInCallStack('ATF036_020') .or.lAtf030,"FN6ATIVOS", "FN6MASTER"))
Local lPadrao	 	:= .F.
...
dbSelectArea("SF9")
SF9->(dbSetOrder(1))
If SF9->( dbSeek(xFilial("SF9")+SN1->N1_CODCIAP) ) .And. lMovCiap
	...
	While SN3->(!Eof()) .And. cFilSN3 == SN3->N3_FILIAL .And. SN1->N1_CBASE == SN3->N3_CBASE  .And. SN1->N1_ITEM  == SN3->N3_ITEM 
		If ( SN3->N3_TIPO $ ("01*03*10" + cTypes10) )
			nVlOrig += &('SN3->N3_VORIG' + cMoedaAtf)
			aAdd(aN3Tipo,{SN3->N3_TIPO, &('SN3->N3_VORIG' + cMoedaAtf)})	
		EndIf
		SN3->(dbSkip())
	EndDo
	/*
	...
	*/
	...
	//???
	nPropSF9 	:= NoRound(oModelValor:GetValue("FN7_VLBAIX",nMoedaATF) * SN1->N1_ICMSAPR / nVlOrig , 2 ) 
	...
	If Substring(cCodMot,1,2)		== "01"
		cMotSF9 := "2"
	ElseIf Substring(cCodMot,1,2) 	== "10"
		cMotSF9 := "3"
...
IF Str(oModelValor:GetValue("FN7_VLBAIX",1),18,2) == Str(oModelValor:GetValue("FN7_VLATU",1),18,2)	// Baixa Total
	...
	If oModelMaster:GetValue("FN6_DEPREC") == '2'
		SN3->N3_NOVO		:= "1"
	EndIf
...
If nTotDepr # 0
	...
	ATFXMOV(cFilAnt,@cIDMOV,dBaixa036,cOcorr,SN3->N3_CBASE,SN3->N3_ITEM,SN3->N3_TIPO,SN3->N3_BAIXA,SN3->N3_SEQ,cSeqReav,"3",nQuant,cTpSaldo,,aValDepr,aDadosComp,/*nRecnoSN4*/,/*lComple*/,/*lValSN1*/,/*lClassifica*/,lOnOff,cPadrao,"ATFA036")
EndIf
...
If nTotDepr # 0
	...
	ATFXMOV(cFilAnt,@cIDMOV,dBaixa036,cOcorr,SN3->N3_CBASE,SN3->N3_ITEM,SN3->N3_TIPO,SN3->N3_BAIXA,SN3->N3_SEQ,cSeqReav,"4",nQuant,cTpSaldo,,aValDepr,aDadosComp,/*nRecnoSN4*/,/*lComple*/,/*lValSN1*/,/*lClassifica*/,lOnOff,cPadrao,"ATFA036")
EndIf
...
If nValCorr # 0
	...
	ATFXMOV(cFilAnt,@cIDMOV,dBaixa036,cOcorr,SN3->N3_CBASE,SN3->N3_ITEM,SN3->N3_TIPO,SN3->N3_BAIXA,SN3->N3_SEQ,cSeqReav,"2",nQuant,cTpSaldo,,aValores,aDadosComp,/*nRecnoSN4*/,/*lComple*/,/*lValSN1*/,/*lClassifica*/,lOnOff,cPadrao,"ATFA036")
EndIf
...
If nValCorDep # 0
	...
	ATFXMOV(cFilAnt,@cIDMOV,dBaixa036,cOcorr,SN3->N3_CBASE,SN3->N3_ITEM,SN3->N3_TIPO,SN3->N3_BAIXA,SN3->N3_SEQ,cSeqReav,"4",0,cTpSaldo,,aValores,aDadosComp,/*nRecnoSN4*/,/*lComple*/,/*lValSN1*/,/*lClassifica*/,lOnOff,cPadrao,"ATFA036")
EndIf
...
If nValCorDep # 0
	...
	ATFXMOV(cFilAnt,@cIDMOV,dBaixa036,cOcorr,SN3->N3_CBASE,SN3->N3_ITEM,SN3->N3_TIPO,SN3->N3_BAIXA,SN3->N3_SEQ,cSeqReav,"5",0,cTpSaldo,,aValores,aDadosComp,/*nRecnoSN4*/,/*lComple*/,/*lValSN1*/,/*lClassifica*/,lOnOff,cPadrao,"ATFA036")	
End
 ...
 */
If ! SN3->N3_TIPO $ "07,08,09" .And. (nTotBaix # 0)
	...
	ATFXMOV(cFilAnt,@cIDMOV,dBaixa036,cOcorr,SN3->N3_CBASE,SN3->N3_ITEM,SN3->N3_TIPO,SN3->N3_BAIXA,SN3->N3_SEQ,cSeqReav,"1",nQuant,cTpSaldo,,aValBaixa,aDadosComp,/*nRecnoSN4*/,/*lComple*/,/*lValSN1*/,/*lClassifica*/,lOnOff,cPadrao,"ATFA036")
...
If nValCorr # 0
	...
	ATFXMOV(cFilAnt,@cIDMOV,dBaixa036,cOcorr,SN3->N3_CBASE,SN3->N3_ITEM,SN3->N3_TIPO,SN3->N3_BAIXA,SN3->N3_SEQ,cSeqReav,"1",nQuant,cTpSaldo,,aValores,aDadosComp,/*nRecnoSN4*/,/*lComple*/,/*lValSN1*/,/*lClassifica*/,lOnOff,cPadrao,"ATFA036")
Endif
...
If nTotBaix # 0
	...
	ATFXMOV(cFilAnt,@cIDMOV,dBaixa036,cOcorr,SN3->N3_CBASE,SN3->N3_ITEM,SN3->N3_TIPO,SN3->N3_BAIXA,SN3->N3_SEQ,cSeqReav,"4",nQuant,cTpSaldo,,aValBx,aDadosComp,/*nRecnoSN4*/,/*lComple*/,/*lValSN1*/,/*lClassifica*/,lOnOff,cPadrao,"ATFA036")
Endif
...
If VerPadrao(cPadrao) .AND. nHdlPrv > 0
	nTotal += DetProva(nHdlPrv,cPadrao,"ATFA036",cLoteAtf)
EndIf
...
/*/
//-------------------------------------------------------------------
Function AF036Cance(cBase,cItem,cTipo,cTipoSld,dDatBaix,cSeq,cMotivo,nHdlPrv,nTotal,cCodBX,cFilFN7)
...
If lRet 
	...
	If lIsRussia
	    If !Empty(SN1->N1_BAIXA) .And. dUltDepr >= SN1->N1_BAIXA .And. !(dUltDepr == SN1->N1_BAIXA .And. RusCheckRevalFunctions(.F.))
	...
	If lRet .AND. SN4->N4_MOTIVO $ "01/10" .And. !Empty(SN4->N4_NOTA)
		lRet := A036VlNota(SN4->N4_NOTA,SN4->N4_SERIE)
		If !lRet
	...
	*/
	...
	While SN4->N4_FILIAL+SN4->N4_CBASE+SN4->N4_ITEM+SN4->N4_TIPO+DTOS(SN4->N4_DATA)==cChave .And. SN4->(!Eof())
		If cSeq == SN4->N4_SEQ  .And. (SN4->N4_OCORR $ "01/06/07/08/10/20" .Or. (SN4->N4_OCORR $ "03" .And. lCancTrans))// Removida implantacao, caso baixar
			...
			//-------------------------------------------------------------------------------------------------------------------------------
			If lCancTrans .And. (/*cIDSN4Aux > SN4->N4_IDMOV .Or.*/ nRecnoSN4 > SN4->(Recno())) 
				SN4->(DBSkip())
				Loop
			EndIf
...
/*/
//-------------------------------------------------------------------
Function A036VlNota(cNota,cSerie)
Local lRet		:= .T.
Local aArea		:= GetArea()
...
If lVldNF
	...
	If SF2->(dbSeek( xFilial("SF2") + cNota + cSerie ))
		//-------------------------------
		// Exclui a Nota Fiscal de Saida
		//-------------------------------
		lRet := AF036ExcNF(cNota,cSerie)
	EndIf
...
/*/
//-------------------------------------------------------------------
Function AF036ExcNF(cNumNFS,cSerie)
Local aAreaSF2		:= SF2->(GetArea())
Local aRegSD2		:= {}
Local aRegSE1		:= {}
Local aRegSE2		:= {}
...
If oModelFN6 != Nil 
	...
	Case cCampoFN6 == "FN6_NATURE"
		lRet := oModelFN6:GetValue("FN6_GERANF") == '1' .And. GetAdvFVal("SF4","F4_DUPLIC",XFilial("SF4")+oModelFN6:GetValue("FN6_TESSAI"),1,"") == 'S'
...
Local cTipo 	  := SN3->N3_TIPO
Local cTpSld	  := SN3->N3_TPSALDO
Local cSeq      := SN3->N3_SEQ
Local dBaixa030 := SN3->N3_DTBAIXA
Local lAuxAuto :=.F.
Local lAuxHelp := .F.
Local lAuxMSHel:=.f. 
Local lSetAuto := .F.
...
/*/
//-----------------------------------------------------------------------
Static Function RusCheckRevalFunctions(lReval AS LOGICAL, lStorno AS LOGICAL)
Local lRet := .F.
Default lReval	:= .T.
Default lStorno	:= .T.

lRet		:= lIsRussia .And. ((lReval .And. IsInCallStack("RU01T05COM")) .Or. (lStorno .And. IsInCallStack("RU01T04_S2STORNO")))
