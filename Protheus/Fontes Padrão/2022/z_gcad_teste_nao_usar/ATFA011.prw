		...
		If lRet
			...
			If nI <> nLin .and. !oGetRateio:aCols[nI,len(oGetRateio:aHeader)+1]
				If	( Alltrim(oGetRateio:aCols[nI,nPCC]) == Alltrim(oGetRateio:aCols[nLin,nPCC]) );
					...
					lRet := .f.
					Help(" ",1,"FA011LinOkB",, STR0030 + Alltrim(oGetRateio:aCols[nI,nPSeq]) + ".",1,0) //"Centro de Custo, Conta Contábil, item da conta e classe de valor digitado já foi incluído na sequencia "
