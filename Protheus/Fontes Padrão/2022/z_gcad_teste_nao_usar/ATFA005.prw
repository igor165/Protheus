...
If nOper == MODEL_OPERATION_UPDATE .Or. nOper == MODEL_OPERATION_DELETE  
	If FNI->(!EOF())
		...
		If lRet .And. nOper == MODEL_OPERATION_DELETE .And. cRev > "0001"
			Help( ,, 'AF005STAT3',, STR0024, 1, 0 ) //"O indice possui revisão anterior e não poder?ser excluido"
			lRet := .F.
		EndIf 
...
If nOpc == MODEL_OPERATION_DELETE    //se operacao igual a exclusão
	...
	If !lResp    //verifica se h?registro em que o indice est?amarrado a ficha do ativo
		Help( ,, "AF05VERIF1",, STR0001+cDescInd+' '+STR0002+cCODIND+''+STR0003, 1, 0 ) //mensagem: 'Índice de Código:tal não pode ser excluído, pois est?sendo utilizado'
	Endif
	...
	If lResp 
		...
		If FNT->(MsSeek(xFilial("FNT")+cCODIND))    //verifica se h?registro em que o indice est?amarrado a alguma taxa
			lResp := .F.
			Help( ,, "AF05VERIF2",, STR0001+cDescInd+' '+STR0002+cCODIND+''+STR0003, 1, 0 ) //mensagem: 'Índice de Código:tal não pode ser excluído, pois est?sendo utilizado'
		Endif
		RestArea(aAreaFNT)
	Endif
...
Else
	If cTipo == '2'
		...
		If lResp .and. oModelFNI:GetValue('FNI_PERIOD') != '2'
			Help( ,, "AF05VERIF3",, STR0019 , 1, 0 ) // 'Para índices Tipo = 2 - Calculado, s??permitido período mensal'
			lResp:=.F.
		EndIf
		...
		If lResp .And. __nOper == OPER_REVISAR
			If lResp .And. ( Empty(dDataRev) )
				Help( ,, "AF05VERIF8",, STR0030 , 1, 0 ) // "Na operação de revisão, a data de revisão ?obrigatória"
				lResp:=.F.
			EndIf
...
±±ºPrograma  ³AFA430CPYºAutor  ³Alvaro Camillo Neto ?Data ? 12/12/11   º±?
