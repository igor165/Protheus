...
//?rogramador ?ata    ?ssue     ?otivo da Alteracao                   ?
//ÃÄÄÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
//?.Rodriguez ?4/04/18?MINA-2123?F012ATDOK() Si A2_TIPDOC = 13 (tipo  ?
//?           ?       ?         ?oc identificaci? = C?ula ciudadan??
//?           ?       ?         ? valida A2_PFISICA. COL              ?
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
//-------------------------------------------------------------------
...
/*
 * Verificação do Par?etro da Contabilização On/Off Line
 */
pergunte("AFA012",.F.)
AF012PerAut()
...
aRotina := Nil
oBrowse:SetCacheView(.F.)// N? realiza o cache da viewdef
...
If __lClassifica
	oView:AddUserButton( STR0079,'' , {|oView| Af012Avp()}    ,,,{OPC_INC,OPC_ALTER}) //C?culo AVP
Else
	oView:AddUserButton( STR0079,'' , {|oView| Af012Avp()}    ,,,{OPC_INC}) //C?culo AVP
EndIf
If __lIncluMan
	oView:AddUserButton( STR0080,'' , {|oView| Af12CrAuto()} ) //M?tiplos
EndIf
...
//-----------------------------------------------
// Ajustes de dicion?io especificos por release
//-----------------------------------------------
...
//----------------------------------------------------------------------
// Os objetos de estrutura s? carregados ap? a correção do dicion?io
//----------------------------------------------------------------------
...
// Ajuste da validação pois o X3_VALID esta incorreto na base do cliente (o espa? existe e funcionava na 11.80, mas parou de funcionar na 12),
// n? subiremos para a MAIN pois este ser?um ajuste pontual e tempor?io para o cliente n? precisar rodar o UPDDISTR.
oSN1:SetProperty('N1_STATUS'	,MODEL_FIELD_VALID, FWBuildFeature( STRUCT_FEATURE_VALID, 'Pertence("0|1|2|3|4")' ))

//-- JRJ Ponto de entrada para permitir a configuracao de campos mantendo a integrida com o PE AT010SN1 na vers? P11.80 
If __lAltera
...
oModel:GetModel( 'SNDDETAIL' ):SetLoadFilter( { { 'ND_STATUS', "'1'" } } )// S?os responsaveis ativos
...
Local lMostra     := If( lPE .and. (Execblock('ATMOSCNT')),.T., .F.)  // PE para mostrar a tela de  lactos na exclus? em lote de ativos
	...
	If Pergunte("AFA012EXLT",.T.)
		...
		Begin Transaction
			...
			Else
				Help(" ",1,"RECNO")
			EndIf
			// Deixar na transac? a contabilização - n? ha como contabilizar posteriormente 
			// pois as linhas s? exlcuidas e n? ha contabilização off
...
//Deleta tabela tempor?ia criada no banco de dados
If _oATFA0122 <> Nil
	_oATFA0122:Delete()
	_oATFA0122 := Nil
Endif
...
If SN1->(MsRLock()) .AND. (cAliasTRB)->(MsRLock())
	...
	//Caso a exclusao n? seja permitida, desmarca registro
...
Possibilitar a convers? entre m?odos de depreciação.
@author William Matos Gundim Junior
@since 19/01/2014
@version 1.0
/*/
...
If lRet  .and. SN1->N1_PATRIM = 'V' .and. Alltrim(SN1->N1_ORIGEM) == 'ATFA460'
	Help(" ",1,"AF012460CV")  //'Este ativo foi gerado a partir do processo de constituição de provis?. Este tipo de ativo n? poder?sofrer revis? de metodo de depreciação.'
	lRet := .F.
Endif   
...
If lRet
	...
	DEFINE WIZARD oWizard TITLE If(nOpcRot == 4 , STR0057, STR0058) ;//" Convers? de M?odo de Depreciação "##" Cancelamento da convers? de M?odo de Depreciação "
	        HEADER " " ; 			
	        MESSAGE STR0005;//" Apresentação "
	        TEXT If(nOpcRot == 4,' ',' ') + CRLF  + CRLF +;	//" Esse processo ir?realizar a convers? dos m?odos de depreciação do bem"##" Esse processo ir?cancelar a convers? dos m?odos de depreciação do bem"
	       		STR0006 + SN1->N1_CBASE   + CRLF +;	//"C?igo Base: "
...
Seleção dos registros da tabela SN3 para convers? do m?odo de depreciação.
@author William Matos Gundim Junior
@since 19/01/2014
@version 1.0
/*/
...
If nOpcRot == OPC_CONVERSAO //Convers?
	// Faz o Seek dos tipo de bens n? baixados
...
Tela para seleção dos registros para convers? do m?odo de depreciação.
@author William Matos Gundim Junior
@since 19/01/2014
@version 1.0
/*/
...
If Len(aBens) > 0
	...
	oMarkBem:bLine := {|| {If(aBens[oMarkBem:nAt,1], oCheck , oNoCheck ),;
	                   ...
	                   aBens[oMarkBem:nAt,09]}} //  cDescMet  - Descrição do m?odo de depreciação 
...
Valida o cancelamento da convers? verificando se foi realizada alguma operação ap? a convers?
@author William Matos Gundim Junior
@since  09/01/2014
@version 12
/*/
...
If nOpcRot != OPC_CONVERSAO
	If SN4->(dbSeek( xFilial("SN4") + cBase + cItem + cTipo + DTOS(dDataConv)  ) )
		While SN4->(!EOF()) .And. SN4->(N4_FILIAL + N4_CBASE + N4_ITEM + N4_TIPO ) == xFilial("SN4") + cBase + cItem + cTipo .And. SN4->N4_DATA >= dDataConv
			// Se tiver um movimento com IDMOV diferente da convers?, n? ser?possivel cancelar a convers?
			If Alltrim(SN4->N4_IDMOV) > Alltrim(cIdMov)  
				lRet := .F.
				Help( " ", 1, "AF012CANCV")//Help "N? ?poss?el cancelar a convers?, pois existe movimentação ap? a operação"
				Exit
			EndIf
			SN4->(dbSkip())
		EndDo
	...
	If  cTipo == '10'
		...
		If FNF->(MsSeek(xFilial("FNF")+ cBase + cItem + "1"))
			lRet := .F.
			Help( " ", 1, "AF012CANCV")  //"N? ?poss?el cancelar a convers?, pois existe movimentação de AVP para este bem"##"Para desfazer a operação para este tipo de bem, recomenda-se a realização de nova convers?"
...
Mostra a tela de seleção dos bens  para processamento da convers? 
@author William Matos Gundim Junior
@since  09/01/2014
@version 12
/*/
...
//?ariaveis para a MsAdvSize e MsObjSize?
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Local lEnchBar   		:= .F. // Se a janela de di?ogo possuir?enchoicebar (.T.)
Local lPadrao    		:= .F. // Se a janela deve respeitar as medidas padr?s do Protheus (.T.) ou usar o m?imo dispon?el (.F.)
Local nMinY	    	:= 400 // Altura m?ima da janela
...
cCadastro := STR0011 //"Convers? de M?odos de depreciação"

//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ??
//?efinição das posições dos objetos na tela?
...
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
//?ontagem do acols da get de modificadores?
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
...
If nOpcA == 1
	If nOpcRot == OPC_CONVERSAO
		MsgRun( STR0012,, {||	lRet := AF012CvGrv(oGetSN3,aRecNo) } )//"Processando Convers?..."
	Else
		MsgRun( STR0013,, {||	lRet := AF012CvCan(aRecNo) } )	//"Processando Cancelamento da Convers?..."
	EndIf
EndIf
...
Realiza a validação de linha da get de convers? de bens
@author William Matos Gundim Junior
@since  09/01/2014
@version 12
/*/
...
If lRet .And. nPosTpDepr > 0 .And. nPosViUtil > 0
	If !aCols[nLinha][Len(aHeader)+1] .And. aCols[nLinha][nPosTpDepr] == "6" .And. aCols[nLinha][nPosViUtil] == 0
		Help("",1,"AF010VDUTI")  //"O campo de vida util ?obrigat?io para esse tipo de depreciação "
		lRet := .F.
	EndIf
EndIf
...
If lRet .And. nPosTpDepr > 0 .And. nPosPrdMes > 0 .And. nPosPrdAno > 0
	If !aCols[nLinha][Len(aHeader)+1] .And. aCols[nLinha][nPosTpDepr] $ "4/5" .And. (aCols[nLinha][nPosPrdMes] == 0 .OR.  aCols[nLinha][nPosPrdAno] == 0)
		Help("",1,"AF010FPROD") //HELP Para calculo pelo m?odo de produção de ativos ?necess?io informar o fator de produção.
		lRet := .F.
	EndIf  
	If !aCols[nLinha][Len(aHeader)+1] .And. aCols[nLinha][nPosTpDepr] $ "4/5" .And. aCols[nLinha][nPosPrdMes] > aCols[nLinha][nPosPrdAno]
	  	Help(" ", 1, "AF012FPRA")//Help "Para calculo pelo m?odo de produção de ativos a produção do periodo deve ser menor ou igual a produção prevista"
		lRet := .F.
	EndIf  	
EndIf
...
If lRet .And. nPosTpDepr > 0 .And. nPosValMax > 0
	If !aCols[nLinha][Len(aHeader)+1] .And. aCols[nLinha][nPosTpDepr] == "7" .And. aCols[nLinha][nPosValMax] == 0
		Help("",1,"AF010VMXD") //Para o calculo pelo m?odo Linear com valor maximo de depreciacao ?obrigatorio o valor maximo de depreciacao
		lRet := .F.
	EndIf
EndIf
...
If lRet .And. nPosTpDepr > 0 .And. nPosValMax > 0 .And. nPosVlDpMax > 0
	If !aCols[nLinha][Len(aHeader)+1] .And. aCols[nLinha][nPosTpDepr] == "7" .And.  aCols[nLinha][nPosValMax] < aCols[nLinha][nPosVlDpMax] 
		Help( " ", 1, "AF012CVVMX" )  //"Para o calculo desse m?odo de depreciação o valor m?imo n? pode ser menor que o valor depreciação acumulada atual "
		lRet := .F.
	EndIf
EndIf
...
If lRet .And. nPosTpDepr > 0 .And. nPosSalv > 0 .And. nPosVlDepSav > 0 .And. nPosVlOriSav > 0
	If !aCols[nLinha][Len(aHeader)+1] .And. aCols[nLinha][nPosTpDepr] == "2" .And. aCols[nLinha][nPosSalv] > aCols[nLinha][nPosVlOriSav] - aCols[nLinha][nPosVlDepSav]  
		Help( " ", 1, "AF012CVSALV") // "Para o calculo desse m?odo de depreciação, o valor de salvamento n? pode ser maior que o valor cont?il do bem "
		lRet := .F.
	EndIf
EndIf
...
If lRet .And. nPosTipo > 0 .And. nPosTpDepr > 0 .And. nPosCodInd > 0
	If !aCols[nLinha][Len(aHeader)+1] .And. aCols[nLinha][nPosTipo] == "10" .And. aCols[nLinha][nPosTpDepr] == "A" .And. Empty(aCols[nLinha][nPosCodInd])
		HELP("",1,"AF010CODIN") // "C?igo do ?dice n? informado."##"Informe o c?igo do ?dice para bens com tipo de depreciação c?culo por ?dice."
		lRet := .F.
	EndIf
EndIf
...
Monta o aHeader da getdados de convers? 
@author William Matos Gundim Junior
@since  09/01/2014
@version 12
/*/
...
Monta o aCols da getdados de convers? 
@author William Matos Gundim Junior
@since  09/01/2014
@version 12
/*/
...
Realiza a convers?, realizando a baixa de um tipo e criando o novo 
@author William Matos Gundim Junior
@since  09/01/2014
@version 12
/*/
...
Realiza o cancelamento da convers?, realizando o cancelamento da baixa e excluindo o tipo atual
@author William Matos Gundim Junior
@since  27/01/2014
@version 12
/*/ 
...
If SN1->N1_PATRIM = 'V' .AND. Alltrim(SN1->N1_ORIGEM) == 'ATFA460'
	Help(" ",1,"AF012460BQ") //Help 'Este ativo foi gerado a partir do processo de constituição de provis?. Este tipo de ativo n? poder?ser alterado'
	lRet  := .F.
Endif
...
If lRet
	...
	SaveInter()
	cCadastro := STR0014 //"Par?etros"
	If !lAutomato
		If ParamBox( {		{ 1, STR0015, 	cBemDe  	,"@!",".T." ,"SN1",".T.",55,.F.	},; //"Do C?igo do Bem"
			{ 1, STR0016, 	cBemAte	,"@!",".T.","SN1",".T.",55,.T.	},; //"At?o C?igo do Bem"
			{ 1, STR0017, 	cItemDe	,"",".T.","",".T.",20,.F.		},; //"Do Item"
			{ 1, STR0018, 	cItemAte	,"",".T.","",".T.",20,.T.		},; //"At?o Item"
			{ 1, STR0019,	cGrupoDe	,"@!",".T.","SNG",".T.",25,.F.	},; //"Do Grupo"
			{ 1, STR0020,	cGrupoAte	,"@!",".T.","SNG",".T.",25,.T.	},; //"At?o Grupo"
			{ 1, STR0021,	dAquisDe	,"",".T.","",".T.",55,.F.		},; //"Da Data de Aquisição"
			{ 1, STR0022,	dAquisAte	,"",".T.","",".T.",55,.T.		},; //"At?a Data de Aquisição"
...
//Monta o cabe?lho com os nomes dos campos chave da tabela SN1
dbSelectArea("SX3")
SX3->(dbSetOrder(2))		//X3_CAMPO
...
If Empty(cAtivo)
	cAtivo += STR0028 + " " + If(MV_PAR09 == 1, STR0029, STR0030) + CRLF		//"N? foram encontrados bens de acordo com os par?etros informados"##"ou os bens j?encontram-se bloqueados/desbloqueados."
...
If !Empty(cEmInvent)
	cTexto += STR0036 + CRLF //" Os seguintes ativos n? foram efetivados porque est? em processo de invent?io. "
	cTexto += cEmInvent + CRLF   		
EndIf

If !Empty(cLocBloq)
	cTexto += STR0037  + CRLF //" Os seguintes ativos n? foram efetivados porque os locais cadastrados est? bloqueados "
...
If lRet
	...
	//?rray a ser passado para ParamBox?
	...
	aAdd(aPerg,{2,STR0041,STR0044,{STR0043, STR0044 },60,"",.T.})//"Exclui classificação anterior"##"N?"##"Sim"##"N?"

	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
	//?aso confirme a tela de parametros processa a rotina de exportacao ?
...
/*/
//-------------------------------------------------------------------
Function AF012IClas(xAutoCab) //recebe o array j?montado pela CTBIMPCSV
Local aArea			:= GetArea()
Local aAreaSN3	   	:= SN3->(GetArea())
...
//--------------------------------------------------------
// Trata as informações que ser? utilizadas em pesquisas
//--------------------------------------------------------
...
If !Empty(cCodAtv) .And. !Empty(cItemAtv) .And. !Empty(cTipoAtv) .And. !Empty(cTpSldAtv)
	...
	//?reparacao do array dos itens que ja EXISTAM.?
	...
	If SN1->(DbSeek(xFilial("SN1")+cCodAtv+cItemAtv))   //FILIAL+CBASE+ITEM
		...
		If lRet
			...
			//?ara utilizacao da rotina automatica e necessario a inclusao do campo N3_SEQ.  ?
			//? Campo N3_SEQ serve para enumerar o tanto de linhas da grid(aCols)            ?
			//?rava a sequencia de aquisição do bem.Cada vez que um novo tipo ?gerado no SN3?
			//? sequencia e acresida de um. Este nro e gerado qdo um tipo e incluido no SN3. ?
			...
			//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
			//?egras de importacao                     ?
			//?xecucao da rotina automatica 4 Alteracao?
			//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
		...
		EndIf
	Else
		Help( " ", 1, "AF012NOREG") //'N? encontrou registros'
	Endif
Else
	Help( " ", 1, "AF012OBRIG")  //"H?campos obrigat?ios utilizados para identificação do ativo n? preenchidos."
EndIf
...
// Busca se a classificacao a ser importada j?existe na ficha do bem
If SN3->(MsSeek( xFilial("SN3") + cCodBase + cItem + cTipo + "0" + cTpSld ))
	
	If lExcImp
		If !AF12DEPRAC(cCodBase,cItem,cTipo,cTpSld) //Se o bem j?possui movimentação faz a baixa do tipo
		...
		EndIf
	Else
		Help(" ",1, "AF012CLASIMP") //Help "J?existe essa classificação para o bem. Verifique o tipo do bem e o tipo de saldo"
		lRet := .F.
	EndIf
...
Função para geração m?tipla de bens.
@author William Matos Gundim Junior
@since  27/01/2014
@version 12
/*/
...
If oModel:GetValue('SN1MASTER','N1_TPCTRAT') $ "2|3"
	Help(" ",1,"NOMULTERC")// "Opção de Multiplo n? dispon?el para bens em controle de terceiros"
	lRet := .F.
EndIf
...
If __LinOk
	...
	If !oModAtv:IsCopy() .And. !oAux:IsInserted() .And. !__lClassifica
		If oModAtv:GetOperation() == MODEL_OPERATION_UPDATE
			If cAcao == "DELETE"
				If ATFV012RAC(oModAtv:GetValue('SN1MASTER','N1_CBASE'),oModAtv:GetValue('SN1MASTER','N1_ITEM'),oModAtv:GetValue('SN3DETAIL','N3_TIPO'),oModAtv:GetValue('SN3DETAIL','N3_TPSALDO')) .And. ! RusCheckRevalFunctions() // Verifica se existe movimentos do ativo exceto a inclus?
					Help(" ",1, "AF010DEL")
					lRet := .F.
				Endif
			Else
					...
					If Left(cCampo,9) == "N3_TXDEPR"
						If lIsRussia
							...
							IF !(Empty(cStatus) .or. cStatus=="0") // allow edit when status == "0-Scheduled"
								lRet := !ATFV012RAC(oModAtv:GetValue('SN1MASTER','N1_CBASE'),oModAtv:GetValue('SN1MASTER','N1_ITEM'),oModAtv:GetValue('SN3DETAIL','N3_TIPO'),oModAtv:GetValue('SN3DETAIL','N3_TPSALDO')) // Verifica se existe movimentos do ativo exceto a inclus?							
							Endif
						Else
							lRet := !ATFV012RAC(oModAtv:GetValue('SN1MASTER','N1_CBASE'),oModAtv:GetValue('SN1MASTER','N1_ITEM'),oModAtv:GetValue('SN3DETAIL','N3_TIPO'),oModAtv:GetValue('SN3DETAIL','N3_TPSALDO')) // Verifica se existe movimentos do ativo exceto a inclus?
						Endif
...
//?orcar posicionamento no SX1              ?
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
pergunte("AFA012",.F.)
AF012PerAut()

//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//?„o deixar excluir o tipo 10 caso exista o tipo 14 nem o tipo 14
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
If __LinOk
	...
	If oAux:IsDeleted() .AND. oAux:GetValue('N3_TIPO') $ ('10|14' + cTypes10) // aCols[n][nUsado+1]  // CAZARINI - 13/03/2017 - If is Russia, add new valuations models - main models
		If Ascan(oAux:aCols,{|x| AllTrim(x[4]) == '14'}) > 0 //existe um tipo 14, n? permitir
			Help(" ",1,"AFDELT14",,STR0131+" " +STR0132+ " " +STR0133,1,0)//"Este item n? pode ser exclu?o.""Itens de Tipo 10 s?podem ser exclu?os se n? existirem itens de Tipo 14,""e itens de Tipo 14 n? podem ser exclu?os manualmente."                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   
			lRet := .F.
		Endif
	EndIf
	...
	//?alidação das regras de informações por tipo de ativo?
	...
	//?alidação para localizações POR / EUA            ?
	...
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
	//?alidação do preenchimento do campo N3_CODIND    ?
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
	If lRet .AND. !oAux:IsDeleted() .AND. oAux:GetValue('N3_TPDEPR') == "A" .AND. Empty(oAux:GetValue('N3_CODIND'))
		Help( ,1, "AF010CODIN")		//"Campo Ind.calculo (N3_CODIND) n? preenchido."##"Para o tipo de depreciação c?culo por ?dice ?necess?io informar o c?igo do ?dice (N3_CODIND)."
		lRet := .F.
	EndIf
	
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
	//?alidação para localização Colombia              ?
	//? tipo de redução de saldos deve ter cadastrados ?
	//? valor de salvamento e a vida util do bem       ?
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
	If cPaisLoc == "COL"
		...
		If lRet .AND. !oAux:IsDeleted() .AND. oAux:GetValue('N3_TPDEPR') $ "4|5" .AND. oAux:GetValue('N3_PRODMES') > oAux:GetValue('N3_PRODANO')
			Help( " ", 1, "AF012FPRA") //"Para calculo pelo m?odo de produção de ativos a produção do periodo deve ser menor ou igual a produção prevista"
			lRet := .F.
		EndIf
	...
	If cPaisLoc $ "BRA|DOM|ARG|COS"
		...
		If cPaisLoc $ "ARG|BRA|COS"
			...
			If lRet .AND.!oAux:IsDeleted() .AND. oAux:GetValue('N3_TPDEPR') $ "4|5|8|9" .AND. oAux:GetValue('N3_PRODANO') == 0
				Help("",1,"AF010FPROD")		//Para calculo pelo m?odo de produção de ativos ?necess?io informar o fator de produção.
				lRet := .F.
			EndIf
			...
			If lRet .AND. !oAux:IsDeleted() .AND. oAux:GetValue('N3_PRODACM') > 0 .AND. oAux:GetValue('N3_VRDACM1') == 0 .AND. oAux:GetValue('N3_TPDEPR') $ "4|5|8|9"
				lRet := .F.
				Help("",1,"AF010PRACM")		//"O valor da depreciação acumulada n? foi informado."##"Para bens com produção acumulada ?necess?io informar o valor da depreciação acumulada."
			EndIf
				
			If lRet .AND. !oAux:IsDeleted() .AND. oAux:GetValue('N3_PRODACM') == 0 .AND. oAux:GetValue('N3_VRDACM1') > 0 .AND. oAux:GetValue('N3_TPDEPR') $ "4|5|8|9"
				lRet := .F.
				Help("",1,"AF010VRACM")		//"A produção acumulada n? foi informada."##"Para bens com depreciação acumulada ?necess?io informar a produção acumulada."
			EndIf
		Else
	...
	If lRet
		...
		If lRet
			...
			If lRet
				...
				ElseIf nOper == 4 .And. oAux:IsDeleted() .AND. ATFV012RAC(cBase,cItem,cTipo) .And. ! RusCheckRevalFunctions() // Verifica se existe movimentos do ativo exceto a inclus?
					Help(" ",1, "AF010DEL")
					lRet := .F.
				...
				ElseIf lRet
					...
					//?N? aceita deprecia‡? acumulada maior que ?
							...
							If ExistBlock("AF12VLR0")
								If !ExecBlock("AF12VLR0",.F.,.F.,{nValOri,nVorigAtf}) // Controlar Bens Valor Zerado - Inventario
									If ExistBlock("A30EMBRA")
										//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
										//?A30EMBRA- Espec?ico para EMBRAER ?
										//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
					...
					//?Se a data de in?io da deprecia‡„o for menor que a data         ?
					//?do ?timo c lculo, exigir a digita‡„o da deprecia‡„o acumulada. ?
					...
					//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
					//?Se a data de in?io da deprecia‡„o for menor que a data         ?
					//?do ?timo c lculo, quando o parametro MV_TIPDEPR for igual a 2, ?
					...
					cTipoDepr $ "01|10" .AND. (nOper == 3 .Or. FwIsInCallStack("ATFA240"))//Adicionado a verificação do FwIsCallStack para verificar se foi chamado apartir da classifição o que n? estava sendo validado 
						Help( " ", 1, "AF012DT10",, STR0050 + DTOC(GetMV("MV_ULTDEPR")), 1, 0 ) //"A Data Inicial de Depreciação n? pode ser menor que a data do ?timo calculo = "
						lRet := .F.
					Endif
					If lRet
	...
	If lRet .AND. Alltrim(oAux:GetValue('N3_CRIDEPR')) $ "03|04"
		If !oAux:GetValue('N3_TIPO') $ ("10|12" + cTypesNM)
			Help( " ", 1, "AF012DEP") //"Crit?io de depreciação n? ?valido para o tipo de ativo em quest?"
			lRet:= .F.
		EndIf
	EndIf
	...
	If lRet .And. !oAux:IsDeleted() .And. !Empty( cGrupo )
		...
		For nX := 1 To Len(aCposSN3)
			...
			If aCposSN3[nX][10] .And. Empty(oAux:GetValue(aCposSN3[nX][3]))
				Help(" ",1,"AF012ALNOK",,STR0145 + AllTrim(aCposSN3[nX][1]) + " (" + AllTrim(aCposSN3[nX][3])+ ") " + STR0146,1,0) //"O campo "###" n? foi preenchido."
				lRet := .F.
				Exit
			EndIf
...
If nOper == 3 .OR. nOper == 4
	...
	If oModel:GetValue('SN1MASTER','N1_STATUS') $ '3' .AND. Empty(oModel:GetValue('SN1MASTER','N1_LOCAL'))
		lRet := .F.
		If nOper == 4
			Help(" ",1,"AF010BLOQ") //Este bem est?bloqueado. Bens bloqueados n? podem ser alterados/excluidos.
		ElseIf nOper == 3
			Help(" ",1,"AF012ATDOK",,STR0157,1,0) //'N? ?possivel definir o ativo como "Bloqueado por Local" (N1_STATUS), sem informar um local bloqueado (N1_LOCAL).'
		EndIf
	EndIf
	...
	If lRet .And. !Empty(dDataBloq) .AND. (oModel:GetValue('SN1MASTER','N1_AQUISIC') <= dDataBloq)
		Help(" ",1,"AF012ABLQM") //"A data de aquisição do bem ?igual ou menor que a data de bloqueio de movimentação : "
		lRet := .F.
	EndIf
	
	//Validacao para o bloquei do proceco
	If lRet .And. !CtbValiDt(,oModel:GetValue('SN1MASTER','N1_AQUISIC') ,.F.,,,{"ATF001"},)
		Help(" ",1,"CTBBLOQ",,STR0135,1,0)//CTBBLOQ - "Calend?io Cont?il Bloqueado. Verfique o processo."
		lRet := .F.
	EndIf

	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
	//?alidação da obrigatoriedade do preenchimento do campo N3_CODIND
	...
	If lRet .AND. cPaisLoc == "BRA" 
		...
		If M->N1_CALCPIS $ '1|2' .And. AllTrim(STR(oModel:GetValue('SN1MASTER','N1_MESCPIS'))) <> "0"
			Help(" ",1,"AF012MESPIS",,STR0167,1,0)// "O campo [Meses Cl.Pis] deve estar zerado para a condição de c?culo de PIS igual a (SIM ou N?)"
			lRet := .F.
		ElseIf M->N1_CALCPIS == '3' .And. !lExist
			Help(" ",1,"AF012MESPISV",,STR0168,1,0) // "  obrigat?io o preenchimento do campo [Meses Cl.Pis] com um valor v?ido, verifique o par?etro MV_ATFMAPR."
			lRet := .F.
		EndIf		
	EndIf
	...
	If lRet .AND. lMargem .AND. !(Empty(oModel:GetValue('SN1MASTER','N1_MARGEM')))
		...
		If !lTp10ou13
			Help(" ", 1, "AF012MARG")  //"Para aplicação de Margem Gerencial na ficha de imobilizado ?necess?io que seja cadastrado um bem do tipo gerencial (Tipo 10 ou 13)"
			lRet := .F.
		Endif
	Endif
		...
		Else
			...
			If SA2->(DbSeek(xFilial("SA2")+ oModel:GetValue('SN1MASTER','N1_FORNEC') + oModel:GetValue('SN1MASTER','N1_LOJA')))
				...
				ElseIf SA2->A2_TIPDOC == "13" .And. Empty(SA2->A2_PFISICA)
					Help("",1,"AF010NONIT","",STR0161,1,0) // Debe informar el Tipo de Documento en el cat?ogo de proveedores.
					lRet := .F.
				EndIf
			Else
	...
	If lRet	
		if oModel:GetValue('SN1MASTER','N1_PATRIM') == 'E'
			...
			If Ascan(oAux:aCols,{|bematf|!(bematf[nPosTipo] $ ("03|13|10" + cTypes10) ) .And. !bematf[nPosDel] }) > 0  // CAZARINI - 13/03/2017 - If is Russia, add new valuations models - main models
				Help( " ", 1, "AF012EMP") //""S?s? permitidos os tipos 03 ou 13 quando o Bem for classificado como Custo de Empr?timo"
				lRet := .F.
			Endif
		Endif
	Endif
	...
	//?alidação das regras de tipo de ativo               ?
	...
	If lRet .AND. lAtfCusPrv .AND. !(Empty(oModel:GetValue('SN1MASTER','N1_PROVIS')))
		...
		For nX := 1 To oAux:Length()
			If !oAux:IsDeleted(nX) .AND. oAux:GetValue('N3_TIPO', nX) $ ("10|13" + cTypes10) .AND. oAux:GetValue('N3_ATFCPR') == '2' // CAZARINI - 13/03/2017 - If is Russia, add new valuations models - main models
				lRet := MsgYesNo (STR0001  + CRLF + STR0002, STR0003)  //'Existem ativos gerenciais de Custo/Provis? que n? est? sendo considerados para os saldos de Custo/Provis?.'###'Deseja continuar assim mesmo?'###'Ativos de Custo/Provis?'  
				If !lRet
					Exit
				Endif
			Endif
	...
	If lRet
		If oModel:GetValue('SN1MASTER','N1_TPCTRAT') == "2"
			If !SNO->(DBSeek(xFilial("SNO") + oModel:GetValue('SN1MASTER','N1_CBASE') + oModel:GetValue('SN1MASTER','N1_ITEM')))
				Help( " ", 1, "AF012TERC",, STR0125, 1, 0 )		//"  Necess?io cadastrar dados de terceiro!"
				lRet := .F.
			Else
		...
		If oModel:GetValue('SN1MASTER','N1_TPCTRAT') == "3"
			If !SNP->(DBSeek(xFilial("SNP") + oModel:GetValue('SN1MASTER','N1_CBASE') + oModel:GetValue('SN1MASTER','N1_ITEM')))
				Help( " ", 1, "AF012TERC",, STR0126, 1, 0 )		//"  Necess?io cadastrar dados de bens em terceiro!" 
				lRet := .F.
			Else
				If Empty(oModel:GetValue('SN1MASTER','N1_FORNEC')) .And. Empty(oModel:GetValue('SN1MASTER','N1_LOJA'))  // Incluir
			...
			If !oAux:IsDeleted(nX)
				For nCpo := 1 To Len(aAux)
					If aAux[nCpo][10] .And. Empty(oAux:GetValue(aAux[nCpo][3],nX)) 
						Help(" ",1,"AF012ATDOK",,STR0145 + AllTrim(aAux[nCpo][1]) + " (" + AllTrim(aAux[nCpo][3])+ ") " + STR0146,1,0) //"O campo "###" n? foi preenchido."
						lRet := .F.
						Exit
					EndIf
				Next nCpo
...
ElseIf nOper == 5
	...
	//N? exclui bens relacionados a Planejamento de Aquisição.
	dbSelectArea("SNN") 
	...
	If AllTrim(cChaveSNN) == "NN_FILIAL+NN_CODEFTV+NN_ITMEFTV+NN_CODIGO+NN_ITEM"
		...
		If dbSeek(xFilial("SNN")+SN1->N1_CBASE+SN1->N1_ITEM)
			Help(" ",1,"AF012PLAT")	//"Este Ativo esta associado a um Planejamento de Aquisição e n? pode ser excluido."
			lRet := .F.
		EndIf               
	EndIf
	
	//N? exclui bens relacionados a controle de provis?
	If lProvis .AND. !Empty(SN1->N1_PROVIS) .AND. !lAtfa460
		Help(" ",1,"AF012PRV")	//'Este Ativo esta associado a controle de provisao e n? pode ser excluido.'
		lRet := .F.
	EndIf
	...
	If lRet
		...
		If FNF->(dbSeek(xFilial("FNF")+SN1->(N1_CBASE+N1_ITEM)+'2'+'1'))
			Help(" ",1,"AF010EXAVP")//'Este Ativo possui movimentos de aproppriação AVP e n? poder?ser excluido.'
			lRet := .F.
		Endif
	Endif
	
	//Verifica se o bem foi gerado por constituição de provis?
	If Alltrim(SN1->N1_ORIGEM) == 'ATFA460' .AND. !lAtfa460
		Help(" ",1,"AF012460E") //'Este ativo foi gerado a partir do processo de constituição de provis?. Este tipo de ativo somente poder?ser cancelado a partir do processo que o gerou'
		lRet  := .F.
	EndIf
	...
	If lRet
		...
		If dbSeek(xFilial("SN1") + SN1->N1_CBASE + SN1->N1_ITEM)
			...
			If !Empty(dDataBloq) .AND. (SN1->N1_AQUISIC <= dDataBloq)
				Help(" ",1,"AF012ABLQM",,STR0115 + DTOC(dDataBloq) ,1,0) //"A data de aquisição do bem ?igual ou menor que a data de bloqueio de movimentação : "
				lRet := .F.
			EndIf
	...
	If lRet .AND. SN1->N1_STATUS = '0'
		If Alltrim(SN1->N1_ORIGEM) == 'ATFA310' 
	 	   If !IsInCallStack("A310GRVATF") 
				Help(" ",1,"AF012EPA") //'Bem planejado somente poder?ser excluido atrav? do estorno da efetivação na rotina de planejamento de ativo (ATFA310)'
				lRet := .F.
			Endif
		EndIf
	EndIf				
	...
	//-------------------------------------------------------------
	// N? permite a exclus? de bens originados de transfer?cia
	...
	If lRet .And. GetMV("MV_ATFCATR",.F.,'2') == '1'
		...
		If TRBSN4->(!Eof()) .And. !FWIsInCallStack("ATFA060")
			lRet := .F.
			Help(" ",1,"AF012ATDOK",,STR0134,1,0) //"Bem originado do processo de transfer?cia. Na rotina de transfer?cia, utilize a opção de cancelamento de transfer?cia."
		EndIf
		TRBSN4->(DbCloseArea())
	EndIf
...
Realiza a validação da convers? de bens
@author William Matos Gundim Junior
@since  21/01/2014
@version 12
/*/
...
//--------------------------------------------------------------------------------------
// Tratamento para identificar se uma das rotinas do ATFA012 est?executando o processo
	...
	If oModel:GetOperation() == 3 .Or. __lClassifica//Inclusao
		...
		If !Empty(oModel:GetValue('SN3DETAIL','N3_TIPO'))  .And. !Empty(cGrupo)
			...
			If __cGrupoFNG != cGrupo
				...
				If MsgYesNo(STR0004) //"Essa alteração implicar?na perda dos dados digitados, deseja continuar?"
					...
					For nX := 1 To nQtdx
						oAux:GoLine( nX )							
				 		If !oAux:IsDeleted()
							...
							//Esse ajuste foi necess?io devido ao DeleteLine que ?utilizado abaixo
		...
		If lCont		
			...
			//---------------------------------------
			// Preenchimento do cabe?lho (SNG x SN1)
			//---------------------------------------
			SNG->(DBSetOrder(1))
			If SNG->(DBSeek(XFilial("SNG")+cGrupo))
				...
				If cPaisLoc == "BRA"
					//For? a atualização do valor de aquisição
			...
			If FNG->(DbSeek(xFilial("FNG")+ cGrupo))
				...
				//--------------------------------------------------------------------------------
				// Tira temporariamente a obrigatoriedade dos campos para funcionamento do m?odo
				...
				While FNG->(!EOF()) .and. cGrupo == FNG->FNG_GRUPO .AND. FNG->FNG_FILIAL == xFilial("FNG")
					...
					If lClaLote 
						...
						Else 
							//Atribui o valor da primeira linha nas demais que n? possuem valor (Uso  Grupo de bens)
							oAux:LoadValue('N3_VORIG1',nVeorig1) 
			...
			ElseIf SNG->(MsSeek(xFilial("SNG")+cGrupo)) //Necessario para gatilhar os dados em cadastros de grupo de bens provenientes da rotina ATFA270(LEGADO) pois nesse caso terei apenas SNG e n? terei FNG
				...
				For nX:= 1 to oAux:Length()
					...
					If !oAux:IsDeleted()
						...
						//-----------------------------------------------
						If SNG->(ColumnPos("NG_TPSALDO")) > 0 .And. SNG->(ColumnPos("NG_TPDEPR")) > 0 //Tratamento necess?io at?inclus? dos campos para o M?ico no ATUSX (No Brasil j?est? ok)
						...
						If SNG->(ColumnPos("NG_TPDEPR")) > 0 //Tratamento necess?io at?inclus? dos campos para o M?ico no ATUSX (No Brasil j?est? ok)
...
//Posiciona na primeira linha n? deletada
oAux:GoLine(nLinha)
...
If !oAux:IsInserted(oAux:GetLine()) .AND. !__lClassifica .AND. !__lCopia
	Help(" ",1,"AF012VCTA", ,STR0163,1,0,,,,,,{STR0164} ) //Este campo n? pode ser alterado####Se esta operação for inevit?el, exclua e inclua novamente o registro ou utilize a rotina de transfer?cia.	 
	lRet := .F.
Endif
...
//Bens ligados a projeto nao podem ser relacionados a um controle de provis?
//Bens que possuam AVP nao podem ser relacionados a um controle de provis?
//Bens que possuam margem gerencial nao podem ser relacionados a um controle de provis?
...
//Se a ficha de imobilizado ja estiver relacionada a um controle de provis?, nao posso alterar o campo N1_PROVIS
...
Else
	If !lRepChapa
		If Empty(nChapa)
			If !(cStatus == "0") 
				Help(" ",1,"AF012ACHAP",,cDescCpo + STR0152,1,0,,,,,,{STR0153 + cNomeCpo + "." }) //" n? informado."###"Preencha o campo "
				lRet := .F.
			EndIf
		Else
			DBSelectArea("SN1")
			...
			If DBSeek(XFilial("SN1") + nChapa) .And. cBase+cItem != SN1->(N1_CBASE+N1_ITEM)
				Help(" ",1,"AF012ACHAP",,cDescCpo + STR0154,1,0,,,,,,{STR0155 + cNomeCpo + "."}) //" j?existente no sistema."###"Defina um novo c?igo no campo "
				lRet := .F.
			EndIf
		EndIf
	EndIf
...
Validar o c?igo CIAP informado. 
@author William Matos Gundim Junior
@since  10/01/2014
@version 12
/*/
...
Local nOper    := oModel:GetOperation() // 3 - Inclus? | 4 - Alteração | 5 - Exclus?
Local lRet     := .T.
...
If (dbSeek(xFilial("SF9")+ cCodCiap))
	If ( SF9->F9_VLESTOR != 0 .AND. SF9->F9_VALICMS - SF9->F9_ICMIMOB == 0 )
		Help(" ",1,"AF010CIAP1")// “Cr?ito de ICMS estornado n? pode ser apropriado.”.
		lRet := .F.
	EndIf
EndIf
If ( "N1_ICMSAPR" $ AllTrim(cCampo) .AND. SF9->(Found()) )
	...
	If ( nOper == 4 .And. lRet )
		If ( !Empty(SN1->N1_CODCIAP) .AND. SN1->N1_ICMSAPR != cICMS )
			Help(" ",1,"AF010CIAP3") //“N? ?permitido ?alteração do valor correspondente ao Cr?ito de ICMS do bem.”.
			lRet := .F.
		EndIf	
	EndIf
EndIf
...
//Executar quando n? forem os processos de C?ia ou Classificação.
If !lRet
	If xFilial("SN1")+ cBase + nItem == SN1->N1_FILIAL + SN1->N1_CBASE + SN1->N1_ITEM
		Help(" ",1,"AFA010NAOP") //“Este campo n? pode ser alterado.”
		lRet := .F.
	Else
		lRet := .T.
	Endif
...
//Para classificação patrimonial 'Or?mento de Provis? de despesas' que possua Tipo de AVP por parcela
//o c?culo do AVP ser?realizado posteriormente atrav? da rotina de apuração de provis?s.
...
@param cMargem -> C?igo da regra de margem gerencial      
@author William Matos Gundim Junior
@since  10/01/2014
@version 12
/*/
...
Else
	...
	If !(FNU->(MsSeek(xFilial("FNU")+cProvis+cRevPrv)))
		Help("  ",1,"AF12NOPROVIS") //Help "Controle de Provis? n? cadastrado"
		lRet := .F.
	ElseIf FNU->FNU_MSBLQL == "1"  //Bloqueado
		Help("  ",1,"AF12PROVISBLQ")//Help "Controle de provis? bloqueado para uso. Utilize outro controle de provis?"
		lRet := .F.
	ElseIf (FNU->FNU_STATUS $ "0/3")  //1 = Ativo; A = Pendente Realizacao
		Help("  ",1,"AF12PROVISATV") //"Controle de provis? possui status que n? permite relacionamento de bens de execução. Apenas controles de provis? com status Ativo ou Pendente Realização podem ser relacionadas a um imobilizado."
		lRet := .F.
	Else	
		lRet := .T.
	Endif
...
If !lDtVazia
	...
	If oModel:GetValue('SN3DETAIL','N3_VRDBAL' + cMoedEx) > 0 
		Help(" ",1, "AFA010EXA1" ) 	//"Quando for informada a data de exaust? n? ?permitido digitar a depreciação acumulada."
		lRet := .F.
	Endif
	If lRet
		If dIniDepr >= dExaust
			Help(" ",1,"AFA010EXA2") //"Data de exaust? inv?ida. Verifique se esta data est?menor ou igual ?data base ou menor ou igual ?data de in?io de depreciação, que s? condições inv?idas.”.
			lRet := .F.
		Endif	
	EndIf
Endif
...
Valida o crit?io de depreciação  
@author William Matos Gundim Junior
@since  22/01/2014
@version 12
/*/
...
Validação do valor m?imo da depreciação.
@author William Matos Gundim Junior
@since  10/01/2014
@version 12
/*/
...
If FWIsInCallStack("ATFA251")
	...
	If nVlMaxDepr > aCols[n][nPosVMOrig]
		Help( " ", 1, "AF010VMXDEPR",,STR0027, 1, 0 ) // "Valor M?imo de Deprecição n? pode ser maior que o valor original na moeda configurada no parametro MV_ATFMDMX "
		lRet := .F.
	EndIf
Else
	...
	If nVlMaxDepr > oModel:GetValue('SN3DETAIL', cCpoOrig) 
		Help(" ", 1, "AF012VMXDEPR")  //"Valor M?imo de Deprecição n? pode ser maior que o valor original na moeda configurada no parametro MV_ATFMDMX "
		lRet := .F.
	EndIf
	oModel := Nil
EndIf 
...
If FunName() == "ATFA012" // Validacao valida apenas para a rotina ATFA012
	...
	oAux   := oModel:GetModel('SN3DETAIL')
	If !oAux:IsInserted()
		Help(" ",1,"AFA010NAOP") //"N? permitir a edição do campo para linhas j?existentes".
		lRet := .F.
	EndIf 
...
Validação de preenchimento do campo de valor m?imo de depreciação.
@author William Matos Gundim Junior
@since  10/01/2014
@version 12
/*/  
...
//Tratamento para uso das rotinas relacionadas que n? foram convertidas 
//para MVC. Ap? adequacao este trecho nao sera necessario
If FwIsInCallStack("ATFA251")
...
dExaust := &(ReadVar())
lDtVazia := Empty( dExaust )  // Se n? houver data de exaust?, retorna.
...
Preenchimento da depreciação do ?timo balan? nas demais moedas.
@author William Matos Gundim Junior
@since  10/01/2014
@version 12
/*/ 
...
If !lAquiTrf //A rotina ATFA251 n? esta adaptada para o modelo MVC
	...
	Else
		...
		If !lRet
			...
			Else
				Help( " ", 1, "AF012TIPDEP") // "Esse m?odo de depreciação n? ?v?ido para esse tipo de ativo."
			EndIf
		EndIf
...
Else
	...
	If nPosN3TpDp > 0
		nLinha := n //validar o campo N3_TPDEPR na função AF010AVTIP, pois o ATFA251 n? est?preparado para o MVC
...
If (!oAux:GetValue('N3_TIPO') $ "02|05" .OR. !GetNewPar("MV_ZRADEPR",.F.))
	...
	//Converte apenas com base na moeda 1, pois o valor do ativo na outra moeda n? necessita ter base na cotação do dia.
	If "N3_VORIG1" $ cCampo 
		For nX := 2 to Len(aPosVOrigX)	// Checa se os valores convertidos v? ultrapassar o tamanho de casas Inteiras
			...
			If aPosVOrigX[nX]>0 .And. Len(AllTrim(Str(int(nValorConv)))) > TamSX3(aAux[aPosVOrigX[nX]][3])[1] - (TamSX3(aAux[aPosVOrigX[nX]][3])[2]+1)
				...
				lRet := .F.
				Help( " ", 1, "AF010ESTVAL",, STR0117+cMoedaErr, 1, 0 ) // "A convers? do valor Original na Moeda 1 para as seguintes moedas, ultrapassara o tamanho maximo permitido: "
			EndIf
...
//Validação para verificar se o grupo contabil ?igual da tabela SN3 quando for alteração
//N? se aplica quando for Classificação de Compras
	...
	For nY := 1 To Len(aEntidade)
		If SN3->(FieldPos("N3_" + Subs(aEntidade[nY], 4))) <> 0 .AND. aEntidade[nY] != "NG_FILIAL"
			...
			If SN3->&("N3_" + Subs(aEntidade[nY], 4)) != &("SNG->NG_" + Subs(aEntidade[nY], 4)) .AND. lRet
				Help(" ",1,"AF010GRP") //“As Entidades Cont?eis do Grupo s? diferentes das Entidades Cont?eis do Bem.”.
				Return .F.
			EndIf
		Endif
	Next
...
Preenchimento da ?tima depreciação nas moedas.
@author William Matos Gundim Junior
@since  10/01/2014
@version 12
/*/ 
...
Validação do c?igo da margem gerencial.
@author William Matos Gundim Junior
@since  10/01/2014
@version 12
/*/ 
...
Else
	...
	If !(FNQ->(MsSeek(xFilial("FNQ")+cMargem+cRevMrg)))
		Help("  ",1,"AF012NOMARGEM ")    //"Regra de Margem Gerencial n? cadastrada"
		lRet := .F.
	ElseIf FNQ->FNQ_STATUS == "2"  //Revisado 
		Help("  ",1,"AF12MARGEMREV")	//"Regra de Margem Gerencial possui revis? posterior. Utilize a ultima revis? ativa da regra de margem gerencial"
		lRet := .F.
	...
	If !dbSeek( xFilial("SN3") + cBase + cItem + "01" ) .AND. !dbSeek( xFilial("SN3") + cBase + cItem + "10" )
		...
		For nX := 1 To oAux:Length() 
			If nX != n1
				Do Case
				//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
				//?erificar se eh a linha de edicao atual e aplicar validacao do que ?
				//?oi digitado atraves variavel do buffer.                           ?
				//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
					Case nX == oAux:GetLine() //Verifico se ?a linha atual e aplico a validacao na memoria
...
If !FwIsInCallStack("ATFA155")
	If lRet .And. cTipo $ "11" .AND. ( oModel:GetOperation() != 3 )  
		Help(" ",1,"AF012TP11")		//"Tipo de ativo valido somente na inclus? de um ativo"
		lRet := .F.
	EndIf
EndIf
...
Local aTipos 		:= {}
// ***********************************************************
// Tipos de ativo que podem possuir multiplas c?ias ativas  *
// ***********************************************************
...
//Express? removida da validação direta do campo para contemplar os novos tipos incluido no SX5->TABELA G1
...
If cPatrim == "T" .AND. !oAuxBusca:GetValue('N3_TIPO',nLinha) $ ('01|10' + cTypes10) .AND. !oAuxBusca:IsDeleted(nLinha)
	Help( " ",1,"AF012PERM" ) // "N? ?permitido usar o tipo nesta operação!"
	lRet := .F.
EndIf
...
If lRet .And. !lLinOk .and. (!__lCopia) .and. cTipo == "14" .And. !oAuxBusca:IsDeleted() //Linha nova
	Help( " ", 1, "AF012NOT14")	//"N? ?permitida a inclus? manual de Tipo 14. Utilize o processo de AVP padr?."
	lRet := .F.
EndIf

//MRG
If lRet .And. !lLinOk .and. ( !__lCopia) .and. cTipo == "15" .And. !oAuxBusca:IsDeleted() //Linha nova
	Help( " ", 1, "AF012NOT15",,)	//"N? ?permitida a inclus? manual de Tipo 15. Utilize o processo de Margem Gerencial padr?."
	lRet := .F.
EndIf

// *********************************************************************
// Valida os tipos de ativo que podem possuir multiplas c?ias ativas  *
// *********************************************************************
...
If lRet .AND. aScan(aTiposBase,{|x| cTipo == x}) == 0 .and. !Empty(cTipo) .AND. AllTrim(FunName()) <> 'ATFA175' 
	...
	If Empty(nTipoUnico)
		Help( " ", 1, "AF012TIP",, STR0113, 1, 0 ) //"Tipo de ativo n? ?um tipo base"
		lRet := .F.
	EndIf
	...
	If lRet .AND. cTipo $ "50|51|52|53|54"
		...
		For nX := 1 to oAuxBusca:Length()
			If !oAuxBusca:IsDeleted(nX) .AND. nX != oAuxBusca:GetLine() .AND. oAuxBusca:GetValue('N3_TIPO', nX) $ "50|51|52|53|54"
				Help( " ", 1, "AF012CADATF") //“Este item j?se encontra digitado no Cadastro de Ativos.”
				lRet := .F.
				Exit
			EndIf
		Next nX
...
If lRet .and. cPatrim = 'E' .and. !(cTipo $ "03/13/10"+cTypes10) .And. !oAuxBusca:IsDeleted()
	Help( " ", 1, "AF012EMP") //""S?permitidos os tipos 03 ou 13 quando o Bem for classificado como Custo de Emprestimo"
	lRet := .F.
Endif
...
If FwIsInCallStack("ATFA251")
	...
	If nPosTpDepr>0 .And. !Empty(&(ReadVar()))
		...
		EndCase
		If !lRet .And. !IsBlind()
			Help( " ", 1, "AF010FDNTUS",, STR0144, 1, 0 ) // "Esse campo n? ?utilizado pelo m?odo de depreciação selecionado, o valor informado n? ser?considerado."
		EndIf
	EndIf
...
Else
	...
	If !Empty(&(ReadVar()))
		Do Case
			...
			EndCase
		If !lRet .And. !IsBlind()
			Help( " ", 1, "AF012FDNTUS",, STR0144) // "Esse campo n? ?utilizado pelo m?odo de depreciação selecionado, o valor informado n? ser?considerado."
		EndIf
	EndIf
...
Local lEnchBar		:= .T.									// Se a janela de di?ogo possuir?enchoicebar (.T.)
Local lPadrao		:= .F.									// Se a janela deve respeitar as medidas padr?s do Protheus (.T.) ou usar o m?imo dispon?el (.F.)
Local nMinY		:= 400									// Altura m?ima da janela
Local aSize		:= MsAdvSize(lEnchBar, lPadrao, nMinY)
...
For nTypes10 := 1 to len(aTypes10)
	...
	//Verifica se h?algum tipo 10 informado na Grid para que seja poss?el fazer o c?culo AVP
	//If oSN3:SeekLine( { {"N3_TIPO", "10" } } )
	If oSN3:SeekLine( { {"N3_TIPO", cTypeAux } } )
		lType10 := .T.
		If AllTrim( oSN3:GetValue("N3_CCONTAB") ) <> ""
			//Se n? h?um valor informado na depreciação gerencia
			If oSN3:GetValue("N3_VORIG1") == 0 		
				lRet := .F.
				Help( " ", 1, "AF012AVP01",, OemToAnsi(STR0121), 1, 0 ) //"O valor do bem na moeda legal n? foi digitado."
			Endif
		Else //N? h?uma conta cont?il informada na depreciação gerencial
			lRet := .F.
			Help( " ", 1, "AF012AVP02",, OemToAnsi(STR0122), 1, 0 ) //"O campo Conta (N3_CCONTAB) n? foi preenchido." 
		Endif
	//Else //N? h?nenhum tipo '10' informado (Depreciação Gerencial/Contabil) 
	//	lRet := .F.
	//	Help( " ", 1, "AF012AVP03",, OemToAnsi(STR0123), 1, 0 ) //"Informe ao menos uma valor do tipo 10 (Depreciação Gerencial/Contabil) para realizar o c?culo AVP."
	Endif
	...
	If AF012TPAVP() //Valida se o tipo de AVP e valido			
		If FA012VldAvp( oSN1:GetValue('N1_DTAVP'), oSN1:GetValue('N1_INDAVP') ) 
			If oSN1:GetValue('N1_INIAVP') < oSN1:GetValue('N1_DTAVP')
				...
				Else
					Help(" ", 1, "AF012NOAVP")  //"Para classificação patrimonial 'Or?mento de Provis? de despesas' que possua Tipo de AVP por parcela, o c?culo do AVP ser?realizado posteriormente atrav? da rotina de apuração de provis?s."
				Endif	
			Else
...
Rotina de inclus? e alteração do rateio.
@author William Matos Gundim Junior
@since  10/01/2014
@version 12
/*/ 
...
If AFXVerRat() 
	...
	Else
		AF011FRat(@__aRateio,nLin,nOper,bFuncPanel)
		//Atualiza grid SN3 (rateio e c?igo rateio)
...
Validar o ?dice AVP e a data prevista de execução.
@author William Matos Gundim Junior
@since  10/01/2014
@version 12
/*/ 
...
//Se for a validação do campo "N1_DTAVP" e a data for menor ou igual a data de inicio, apresenta um help e n? valida
	...
	Else
		If cVar == "M->N1_INDAVP" .And. !Empty(cIndAVP)
			...
			If !(FIT->(MsSeek(xFilial("FIT")+ oModel:GetValue('SN1MASTER','N1_INDAVP'))))
				Help("  ",1,"NOINDAVP")  //"C?igo de indice financeiro n? cadastrado."
			Else
				lRet := .T.
			Endif
...
If nValor < 0
	If !(cTipo $ cN3TipoNeg + "|05")
		Help(" ",1,"AF012TPNEG")//Valor deste item n? pode ser negativo.###Informe um valor maior que zero ou zero.
		lRet := .F.
	Endif
EndIf
...
Local cDac		  := ''	
Local cN3TipoNeg  	:= Alltrim(SuperGetMv("MV_N3TPNEG",.F.,"")) // Tipos de N3_TIPO que aceitam Valor originais negativos
Local lAtfClas  := IsInCallStack("AF240Class") //Por legado da vers? 11, permitir a alteração do valor do bem na classificação de ativos (ATFA240)
...
If !oModel:IsCopy() .AND. !oAux:IsInserted() .AND. !lAtfClas .AND. !(lIsRussia .AND. (Empty(cStatus) .OR. cStatus=="0")) // Se for c?ia de registro, autorizar esta alteração
	Help(" ",1,"AFA010NAOP")
	lRet := .F.
Else
...
If Empty(cBase) .OR. Empty(cItem)
	lContinua := .F.
	Help(" ",1,"AF012CDBAS") //"Campo de c?igo base ou Item n? preenchido, por favor verifique."
EndIf
...
If lOk
	...
	For nX := 2 to nQuantas
		...
		EndIf
		cTaxa := aAux[aPosTaxa[nX],3]
		If alMoeda[nX]	/// Se n? tem a moeda informada no par?etro (Moeda em branco)
			If nX > 9
...
Valida se houve depreciação ap? a inclus?
@author William Matos Gundim Junior
@since  27/01/2014
@version 12
/*/
...
If SN1->(dbSeek(xFilial("SN1")+cCodBase+cItem ))   //FILIAL+CBASE+ITEM
	...
	If SN3->(MsSeek(xFilial("SN3") + cCodBase + cItem + cTipo +"0"+cTpSld ))
		...
		/*ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
		?ocalizar a ultima reavalicao para exclusao.             |
		ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?/
...
Gera a constituição do AVP para inclus? de m?tiplos bens
@author William Matos Gundim Junior
@since  27/01/2014
@version 12
/*/
...
Função que verifica se o N4_IDMOV da depreciação ?o mesmo que o da inclus?, ou se houve depreciação ap? a inclus?. 
...
Local aRecAtf   := {}
Local lDigReav := .F.
Local nRECSN3Exc := -1 // Recno da linha do SN3 se estiver exclu?a , para excluir registros no MSdocument.
Local cTpSaldo	:= ""
Local cOcorr      := ""
Local aDadosComp :={}
Local aValores   := {}
...
// Durante a inclusao, uma linha pode ser exclusa, e n? deve ser considerada para o N3_SEQ
...
If nOper != MODEL_OPERATION_DELETE
	For nX := 1 To oSN3:Length()
		...
		//--------------------------------------------------------------------------------------------------------------
		// Na inclusao incrementa o N3_SEQ, tratamento necess?io pois ao deletar uma linha, a sequencia fica incorreta
		//--------------------------------------------------------------------------------------------------------------
		If nOper == MODEL_OPERATION_INSERT
			nSeq++

		//-------------------------------------------------------------------------------------------------------------
		// Na alteracao avalia se j?era linha existente para n? mudar o N3_SEQ, n? perdendo a referencia com a SN4.
		// Caso seja linha nova, obt? a ?tima sequencia para incrementar
		//-------------------------------------------------------------------------------------------------------------
		ElseIf nOper == MODEL_OPERATION_UPDATE
			...
			If !SN3->(MSSeek(XFilial("SN3")+oSN1:GetValue("N1_CBASE")+oSN1:GetValue("N1_ITEM")+oSN3:GetValue("N3_TIPO")+oSN3:GetValue("N3_BAIXA")+oSN3:GetValue("N3_SEQ")))
				...
				//--------------------------------------------------------------------------------------
				// Tratamento necessario para classificacao de ativo com mudanca do c?igo base ou item
				//--------------------------------------------------------------------------------------
				If Empty(nSeqAux)
					nSeq++
				Else
			...
			If Len(aCols14) > 0
				...
				For nTam := 1 To Len(aCols14)
					...
					For nInc := 1 to Len(aCols14[nTam])
						...
						Else //Exceto os dados editaveis os demais s? copias exatadas do tipo 10
...
//?Verifica o n?ero de itens de Ativo Imobilizado                           ?
//?Dever  conter 1 item (SN1) e no m?imo mais 1 item (SN3)                  ?
...
If lRet
	/*
	* Geração de Lan?mentos Cont?eis conforme o Tipo de Ativo
	*/
		...
		If nOper != 5
			...
			For nX := 1 To oSN3:Length()
					...
					If FNA->(MsSeek(xFilial("FNA")+SN3->(N3_CBASE+N3_ITEM+N3_TIPO+N3_SEQ+N3_SEQREAV+N3_TPSALDO)))
						While FNA->(!EoF()) .And.;
							...
							Case FNA->FNA_OCORR == "P1"
								cPadrao := "876"		//Estorno de apontamento de revis? de estimativa de produção
				...
				If oSN3:IsDeleted(nX) .And. lDeleta
					...
					('SN3')->(dbDelete())
					('SN3')->(FkCommit())
					nRECSN3Exc := SN3->(RECNO()) // Recno da linha do SN3 se estiver exclu?a , para excluir registros no MSdocument.
					nCntDel++
	...
	If lAFA012 //Ponto de entrada ap? commit do model
...
If __lAtfAuto .And. Type('__aAutoItens')== "A"
	...
	If nValRlz > 0
		...
		//a taxa ser?zero, pois o valor foi informado
...
Função para determinar qual o c?ido do lan?mento padr?
@author William Matos Gundim Junior
@since  09/01/2014
@version 12
/*/
...
Local nDecimal		:= 0				// Auxiliar n?ero de decimais
...
//?apeamento do conteudo de aParam					  ?
//?Param[1] - N3_VORIG1							  ?
//?Param[2] - N3_VRDACM1							  ?
//?Param[3] - N3_TPDEPR							  ?
//?Param[4] - N3_VMXDEPR							  ?
...
If dInDepr <= dDataBase
	...
	For nI := 1 to Len( aCotaM )
		...
		If aTaxaRef[ nI ] # 0
			...
			// Tratamento da correção Monet?ia
			If nI == 1
...
Local nAnosDecor	:= 0							//Tempo Decorrido em anos desde o in?io dos c?culos.
Local nAnosRest	:= 0		  					//Tempo restante em anos para o t?mino dos c?culos.
...
For nI := 1 To nQuantas
	...
	If nI == 1
		...
		nDeprAcum += aVlrCda[1] //Este campo ?tratado apenas na moeda 1
	Endif
	...
	If nI == 1
		nVlAtivo += aVlrCAcm[1]	//Este campo ?tratado apenas na moeda 1
	Endif
...
//N? exclui bens relacionados a Base instalada (SIGATEC)
If lRet .And. AliasInDic("AA3")
	...
	If TRBAA3->RECAA3 > 0
		lRet := .F.  
		If lHelp
			Help(" ",1,"AF010BASE") // Este Ativo esta associado a base instalada, e n? pode ser excluido.   
		Endif
	Endif
	TRBAA3->(dbCloseArea())							
EndIf        
...
//Nao permite a exclus? de bens com CIAP associado.
dbSelectArea("SF9")
dbSetOrder(1)
If lRet .AND. (!Empty(SN1->N1_CODCIAP) ) .AND. ( MsSeek(xFilial("SF9")+SN1->N1_CODCIAP) )
	If !IsBlind()
		lRet := MsgYesNo(STR0111 + SN1->N1_CODCIAP,STR0112)//"O Bem possui um CIAP associado deseja Continuar a exclus? ?"##"Atenção"
	Else
		lRet := .F.
	Endif
Endif
...
//Neste caso, o bem n? podera ser excluido, mas apenas baixado
If lRet
	FNF->(dbSetOrder(4)) //FNF_FILIAL+FNF_CBASE+FNF_ITEM+FNF_TPMOV+FNF_STATUS
	IF FNF->(dbSeek(xFilial("FNF")+SN1->(N1_CBASE+N1_ITEM)+'2'+'1'))
		Help(" ",1,"AF012EXAVP") //'Este Ativo possui movimentos de aproppriação AVP e n? poder?ser excluido.'
		lRet  := .F.
		xRet := .T.
	Endif
Endif
...
If lRet .AND. xCab <> Nil
	...
	EndIf
	__oModelAut:SetOperation(nOpcAuto)  // 3 - Inclus? | 4 - Alteração | 5 - Exclus?
	...
	If nOpcAuto == MODEL_OPERATION_INSERT .Or. nOpcAuto == MODEL_OPERATION_UPDATE .Or. lClaLote
		For nX := 1 To Len(xCab)
			...
			//Se for classificação em lote for? salvar data de aquisição
			...
			Else
				...
				If xItens[nX,Len(xItens[nX]),1] == "AUTDELETA"
					...
					For nY := 1 To Len(xItens[nX])
						...
						//Se for classificação em lote for? salvar data de aquisição e data de inicio da depreciação
...
While !EOF() .AND. SN3->N3_FILIAL+SN3->N3_CBASE+SN3->N3_ITEM == xFilial(cAlias1)+SN1->N1_CBASE+SN1->N1_ITEM
	...
	If lAtClDepr .OR. Empty(SN1->N1_PATRIM)
		// Pelo calculo mensal
		//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
		//?Se for conta de Capital social, n? lan‡a contra-partida da corre‡? monet ria. ?
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ?
		// *******************************
		// Controle de multiplas moedas  *
		// *******************************
	...
	If  SN3->N3_TIPO == '14'
		...
		If FNF->(DbSeek(XFilial("FNF")+SN1->(N1_CBASE+N1_ITEM)+'1'))
			...
			While !(FNF->(EOF())) .AND. FNF->(FNF_FILIAL+FNF_CBASE+FNF_ITEM) == ;
				...
				ElseIf FNF->FNF_TPMOV $ '9|A' //Exclusao de Ajuste de AVP por Revis?
					RecLock("FNF",.F.)
					FNF->(DbDelete())
					FNF->(MsUnlock())
				EndIf
...
// Exclui os registros da tabela de respons?el SND.
...
//Chamada da função FwIntegDef para realizar a exclus? em casos que exista integração EAI.
If lIntATFA12
	FwIntegDef("ATFA012")
EndIf  
...
If !lAutomato
	...
	If Empty(cAtivo)
		cTexto += AllTrim(STR0028) + " " + IIF(MV_PAR09 == 2, STR0030, STR0029) + CRLF		//"N? foram encontrados bens de acordo com os par?etros informados"##"ou os bens j?encontram-se bloqueados/desbloqueados."
	...
	If !Empty(cEmInvent)
		cTexto += STR0052 + CRLF //" Os seguintes ativos n? foram efetivados porque est? em processo de invent?io. "
		cTexto += cEmInvent + CRLF   		
	EndIf
	
	If !Empty(cLocBloq)
		cTexto += STR0051 + CRLF //" Os seguintes ativos n? foram efetivados porque os locais cadastrados est? bloqueados "
		cTexto += cLocBloq + CRLF   		
	EndIf
...
// Esta alteração n? deve ocorrer, pois podem 
...
If __lAltera
	...
	Else	
		For i:= 1 to __nQuantas
			...
			oStruct:SetProperty(Iif(i<10,'N3_VRDBAL','N3_VRDBA') + cMoed,MODEL_FIELD_WHEN, {|| Af12Bloq(oStruct) }) //Valor da depreciação referente ao ?timo balan? na Moeda.
			oStruct:SetProperty(Iif(i<10,'N3_VRDMES','N3_VRDME') + cMoed,MODEL_FIELD_WHEN, {|| Af12Bloq(oStruct) }) //Valor da ?tima depreciaçâo na Moeda
		...
		oStruct:SetProperty('N3_VRCBAL1',MODEL_FIELD_WHEN, {|| Af12Bloq(oStruct) }) //Valor da correção referente ao ?timo balan?.
		oStruct:SetProperty('N3_VRCMES1',MODEL_FIELD_WHEN, {|| Af12Bloq(oStruct) }) //Valor da ?tima correção.
		oStruct:SetProperty('N3_VRCDM1' ,MODEL_FIELD_WHEN, {|| Af12Bloq(oStruct) }) //Valor da ?tima correção.
		oStruct:SetProperty('N3_VRCDB1' ,MODEL_FIELD_WHEN, {|| Af12Bloq(oStruct) }) //Valor da ?tima correção.
		oStruct:SetProperty('N3_VRCDA1' ,MODEL_FIELD_WHEN, {|| Af12Bloq(oStruct) }) //Valor da ?tima correção.
		If lIsRussia
...
±±?rograma  ?F12aExcId ?utor ?icrosiga           ?Data ? 02/29/12   º±?
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±?
±±?esc.     ?ealiza a exclusao dos movimentos do SN4 do bem quando      º±?
±±?         ?om o mesmo ID                                              º±?
...
If SN1->(MsSeek(xFilial("SN1") + cBase + cItem ))
	...
	__lIncluMan	:= .T.
	__aRateio		:= {}
	cTitulo      := STR0065 //"C?ia"
	cPrograma    := 'ATFA012'
	nOperation   := MODEL_OPERATION_INSERT
	
	oModel       := FWLoadModel( cPrograma )
	oModel:SetOperation( nOperation ) // Inclus?
	...
	pergunte("AFA012",.F.)
	AF012PerAut()
	If MV_PAR03 == 2 // C?ia Sem Acumulados
...
If ExistBlock("F010VldDel")
	/*
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ??
	//?ste PE sera executado em substituicao a rotina de validacao?
	//?a exclusao do ativo. Devera retornar .T. para prosseguir a ?
	//?xclusao, ou .F. caso contrario.                            ?
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ??
	*/
...
Else
	...
	If lRet
		...
		If dbSeek(xFilial("SN1")+SN1->N1_CBASE+SN1->N1_ITEM)
			...
			lTemDeprec := !Empty( SN1->N1_BAIXA) .Or. IIf(SN3->N3_VRDACM1 != 0,ATFV012RAC(SN3->N3_CBASE, SN3->N3_ITEM),.F.)
			//---------------------------------------------------------------------------------------------------------------
			// Caso seja a rotina de cancelamento de transfer?cia, permite, pois a query avalia se o bem j?veio depreciado
			// e nao sofreu depreciações na filial destino, permitindo a exclus? e retorno pra filial origem
			//----------------------------------------------------------------------------------------------------------------
			...
			EndIf
			If !Empty(dDataBloq) .AND. (SN1->N1_AQUISIC <= dDataBloq)
				If (lHelp,Help(" ",1,"AF010ABLQM",,STR0118 + DTOC(dDataBloq) ,1,0),) //"A data de aquisição do bem ?igual ou menor que a data de bloqueio de movimentação : "
				lRet := .F.
			EndIf
			//Validacao para o bloquei do proceco
			If lRet .And. !CtbValiDt(,SN1->N1_AQUISIC ,.F.,,,{"ATF001"},)
	...
	EndIf
EndIf
//N? Excluir Bem Efetivado pela Rotina ATFA310 se o campo N1_ORIGEM igual "ATFA310" e N1_STATUS = '0' e Rotina que chamou se for diferente de "A310GRVATF"
If lRet .And. SN1->(FieldPos("N1_STATUS")) > 0 .And.  SN1->N1_STATUS = '0'
	If Alltrim(SN1->N1_ORIGEM) == 'ATFA310' 
	    If !IsInCallStack("A310GRVATF") 
			If (lHelp, Help(" ",1,"AF010EPA",,STR0119,1,0),)	//'Bem planejado somente poder?ser excluido atrav? do estorno da efetivação na rotina de planejamento de ativo (ATFA310)'
			lRet := .F.
		Endif
	EndIf
EndIf
...
±±?un‡„o    ?tfVldCpoCfg?Autor ?Claudio Donizete Souza?Data ?29.07.10 ³±?
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±?
±±?escri‡„o ?Valida se o campo eh obrigatorio, conforme configuracao do   ³±?
±±?         ?grupo de campos                                              ³±?
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±?
±± ÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±?rograma  ?jRetParam?utor  ?lvaro Camillo Neto ?Data ? 17/12/08   º±?
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±?
±±?esc.     ?justa as repostas do aParambox                             º±?
±±?         ?                                                           º±?
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±?
±±?so       ?		                                                      º±?
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±?
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±?
±± ÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±?rograma  ?arregStru?utor  ?amon Neves         ?Data ? 18/11/11   º±?
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±?
±±?esc.     ?Monta o a Header especifico para a rotina automatica.      º±?
±±?         ?                                                           º±?
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±?
±±?so       ?AF010Exec                                                  º±?
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±?
...
Exclui o Bem de terceiro caso o usu?io cancele a operação ou altere o controle
...
If nPosN3Tipo>0 .And. nPosN3TpDp>0
	...
	Else
		...
		If !lRet
			...
			Else
				Help( " ", 1, "AF010TIPDEP",, STR0024, 1, 0 ) // "Esse m?odo de depreciação n? ?v?ido para esse tipo de ativo."
			EndIf
		EndIf
	EndIf
EndIf
...
Validação do valor m?imo da depreciação    para conversao de metodo de depreciação 
...
If nVlMaxDepr > aCols[n][nPosVMOrig]
	Help( " ", 1, "AF010VMXDEPR",,STR0136, 1, 0 ) // "Valor M?imo de Deprecição n? pode ser maior que o valor original na moeda configurada no parametro MV_ATFMDMX "
	lRet := .F.
EndIf
...
Validação dos campos dos m?odos de depreciacao gerencial para conversao de metodo 
...
If nPosTpDepr>0 .And. !Empty(&(ReadVar()))
	...
	EndCase
	If !lRet .And. !IsBlind()
		Help( " ", 1, "AF012FDNTUS") // "Esse campo n? ?utilizado pelo m?odo de depreciação selecionado, o valor informado n? ser?considerado."
	EndIf
EndIf
...
Verifica se o tipo do ativo ?novo ou ?alteração
...
±±?un‡…o    ?AtvIsBaixd ?Autor ?aniel Mendes ?       Data ?29/06/15 ³±?
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±?
±±?escricao ?Verifica se o ativo esta baixado sem consistir a data, com ³±?
±±?         ?base na SN1 posicionada ou par?etro de chave              ³±?
...
1-N? permitir alterar a data de inicio da depreciação
apos o bem j?ser depreciado
2-N? permitir que a depreciação inicie antes da data de aquisição 
...
If lRet .And. !Empty(M->N3_DINDEPR) .And.( M->N3_DINDEPR < oModelSN1:GetValue("N1_AQUISIC"))
	MsgAlert(STR0156) // "Data a Depreciação n? pode ser superior a data da aquisição do ativo, digite uma data menor ou igual a data da aquisição"
	lRet := .F.
EndIf
...
Validação de respons?el pelo bem
...
If !lAlt
	...
	If !Empty(cResp)
		...
		If lHelp
			lRet := MsgYesNo(STR0137 + cNome + STR0138 )//"O ativo est?com o seguinte participante como respons?el: "##" Procede com a operação?"
		Else
			lRet := .F.
		EndIf
	EndIf
...
@param   Caracter, cVersion, Vers? da mensagem.
...
@obs
O m?odo ir?retornar um objeto do tipo TOTVSBusinessEvent caso
...
//- N? quando incluido novo tipo de ativo
...
Exclui a vinculação do bem com o responsavel no momento da exclus? do bem
...
Static Function A012SetEnt(oLinha,cField,cEnt,lApaga)
	...
	//Ajuste para que os valores que estejam em branco no cadastro do grupo de bens n? sobreponha o que j?esta preenchido
	If Empty(Alltrim(cRet))
		lApaga	:= .F.
	Endif
	
	If (lApaga .OR.(Empty(oLinha:GetValue(cEntN3)))) // Verifica se deve sobre-escrever o campo, caso esteja em branco a informação ?colocada de qualquer maneira 
...
±±?unção    ?PrzTotPrev  ?Autor ?Norberto M Melo       ?Data ?1/06/08  ³±?
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±?escri‡…o ?Tempo total para a depreciação em anos                        ³±?
±±?         ?Se n? informada atrav? de N1_PRZDEPR efetua um c?culo utili³±?
±±?         ?zando a taxa anual cadastrada no N3                           ³±?
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±?etorno   ?Valor num?ico representado a quantidade em anos.             ³±?
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±?ATA      ?Programador   ?Manutencao Efetuada                           ³±?
±±?         ?              ?                                              ³±?
±±?         ?              ?                                              ³±?
±±?         ?              ?                                              ³±?
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±?unção    ?MaxTxDegr   ?Autor ?Norberto M Melo       ?Data ?1/06/08  ³±?
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±?escri‡…o ?Taxa M?ima de Depreciação que via de regra dever?ser infor  ³±?
±±?         ?mada no cadastro de grupos, por? na falta deste cadastro     ³±?
±±?         ?ser?considerado o valor informado no N3                      ³±?
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±?arametros?ExprL1 - Aplicar o coeficiente sobre a taxa                   ³±?
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±?etorno   ?Array com cinco elementos contendo a taxa respectiva de cada  ³±?
±±?         ?moeda.                                                        ³±?
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±?ATA      ?Programador   ?Manutencao Efetuada                           ³±?
±±?         ?              ?                                              ³±?
...
If lRet
	...
	If SN1->(dbSeek( xFilial("SN1") + cBase + nItem))
		Help(" ",1,"JAGRAVADO")		//"J?existe registro com esta informação.###Troque a chave principal deste registro."
		lRet := .F.
	EndIf
EndIf
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±?
±± ÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±?rograma  ?F012ACHCC?utor  ?OTVS SA            ?Data ? 10/01/2018 º±?
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±?
±±?esc.     ?Validação do campo N3_CCUSTO, de forma que o Centro de     º±?
±±?         ?de despesa de depreciacao tenha o mesmo valor.             º±?
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±?
±±?so       ?ATF012                                                     º±?
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±?
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±?
...
Return .T.
//Validacao changSets antes de tudo.