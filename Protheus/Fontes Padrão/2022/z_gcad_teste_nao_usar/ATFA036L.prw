					...
					For nCtnTipo := 1 to oModelTipo:Length()
						...
						*/
						If oModelTipo:GetValue("OK")
							...
							*/
							cSeq := Af036Grava(cAlsAtu, cNota, cSerie,nQuant,nQtdOrig,lCtbBxInt,cIDMOV,lMovCiap,cNFItem,dBaixa036,cCodMot,oModel,lQuant,@nHdlPrv,@nTotal)
							If Empty(cSeq)
								lOk := .F.
								Exit
							EndIf
				...
				While (cAlsBens)->(!Eof())
					...
					If cChavAux <> (cAlsBens)->FN7_CBASE +(cAlsBens)->FN7_CITEM + (cAlsBens)->FN7_TIPO + (cAlsBens)->FN7_TPSALD
						...
						If (cAlsBens)->FN7_MOEDA == '01'
							...
							lOk := AF036Cance((cAlsBens)->FN7_CBASE,(cAlsBens)->FN7_CITEM,(cAlsBens)->FN7_TIPO,(cAlsBens)->FN7_TPSALD,(cAlsBens)->FN7_DTBAIX,(cAlsBens)->FN7_SEQ,(cAlsBens)->FN7_MOTIVO,@nHdlPrv,@nTotal,(cAlsBens)->FN7_CODBX,cFilFN8)
						EndIf
...
If __nOper == OPER_BXLOT
	...
	*/
	...
	If lIsRussia
		cQryBens	:= AF036RUQRY(lRusDepBonus, cRusTypeDepBonus, oModelPar)
	EndIf
...
ElseIf __nOper == OPER_CANLT
	...
	cQryBens += " INNER JOIN " + RetSqlName("SN1") + " SN1 ON SN1.N1_FILIAL = FN6.FN6_FILORI AND SN1.N1_CBASE = FN6.FN6_CBASE AND SN1.N1_ITEM = FN6.FN6_CITEM "
	cQryBens += " WHERE "
...
dbSelectArea( "SN1" )
SN1->(dbSetOrder( 1 ))
SN1->(dbSeek( cFilOri + cBase + cItem ))

If __nOper == OPER_BXLOT
	/*
	* Verifica todos os tipos de ativos existentes para o bem selecionado
	*/
	SN3->(dbSeek( cFilOri + cBase + cItem ))
	
	/*
	* Carrega array para exibição no grid
	*/
	While SN3->(!Eof()) .And. ( cFilOri + SN1->N1_CBASE + SN1->N1_ITEM == SN3->N3_FILIAL + SN3->N3_CBASE + SN3->N3_ITEM )
		aCgTipo := {}
...
ElseIf __nOper == OPER_CANLT
	...
	*/
	While FN7->(!Eof()) .And. ( cFilFN7 + cIdBaixa == FN7->FN7_FILIAL + FN7->FN7_CODBX)
		If SN3->(dbSeek( FN7->FN7_FILORI + FN7->FN7_CBASE + FN7->FN7_CITEM + FN7->FN7_TIPO ))
...
Local cFilOri		:= oModelMaster:GetValue("FN6ATIVOS","FN6_FILORI")
Local cCodBX		:= oModelMaster:GetValue("FN6ATIVOS","FN6_CODBX" )
...
If __nOper == OPER_BXLOT
	...
	If Val(GetAdvFVal("SN3","N3_BAIXA",cFilOri+oModelTipo:GetValue("FN7TIPO","FN7_CBASE")+oModelTipo:GetValue("FN7TIPO","FN7_CITEM"),1,"",.T.)) == 0
				...
				If lRet
					If SF4->(dbSeek(xFilial("SF4") + cTESSaida) )
						...
						If lRet .And. SF4->F4_ESTOQUE <> 'N'
							lRet := .F.
							Help(" ",1, "AF036LPo5",,STR0040+ " " + STR0049 + cFilAnt + " "+STR0050 + cBase + " " + STR0051+ cItem,1,0) //"O Tipo de Saída não pode atualizar estoque para a geração da nota fiscal."##"Filial de Origem: "##"Código Base: "##"Item: "
						EndIf
...
If oView != Nil
	MsgRun( STR0020,, {|| AF036LGVal(oModelFN6,cCampo,xValCpo) } )//"Processando..."
Else
	AF036LGVal(oModelFN6,cCampo,xValCpo)
EndIf
...
@version 12
/*/
Function AF036LGVal(oModelFN6,cCampo,xValCpo)
Local nY 	:= 1
Local lRet	:= .F.
...
/*/
//-----------------------------------------------------------------------
Static Function AF036RUQRY(lRusDepBonus AS LOGICAL, cRusTypeDepBonus AS CHARACTER, oModelPar AS OBJECT)
