...
Else
	...
	// ADICIONA AO MODELO ESTRUTURA DE FORMULÁRIO DE EDIÇÃO POR CAMPOS
	...
	// Define a inserção de um item no grid como .F.
	oModel:GetModel( 'G3KDETAIL' ):SetOptional( .T. )
	
	// ALTERA PROPRIEDADE DO CAMPO PARA OBRIGATÓRIO
	...
	// ADICIONA A DESCRIÇÃO DO MODELO DE DADO
...
If nModulo <> 89 .AND. !(ValidEAI() .AND. SupergetMV("MV_INTTUR",,.F.))	
	...
	Else
		...
		// RELACIONA O ID DA VIEW COM BOX DE EXIBIÇÃO
...
//Validar se está em modo sincronismo de carga inicial do EAI e se a versão da mensagem é = a 3 (mensagem especifica para o modulo turismo)
//Validar na proxima realease se a função FWEAIInSinc existir, remover o IsInCallStack
