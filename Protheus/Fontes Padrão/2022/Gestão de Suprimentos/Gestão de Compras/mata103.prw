...
/*/
Function MATA103(xAutoCab,xAutoItens,nOpcAuto,lWhenGet,xAutoImp,xAutoAFN,xParamAuto,xRateioCC,lGravaAuto,xCodRSef,xCodRet,xAposEsp,xNatRend,xAutoPFS,xCompDKD)
Local nPos      	:= 0 
Local bBlock    	:= {|| Nil}
Local nX	   		:= 0
Local nAutoPC		:= 0
...
If CheckCols(n,aCols)
	...
	If !aCols[n][Len(aCols[n])]
		...
		((nPosNfOri>0 .Or. nPosItmOri>0 .OR. nPosSerOri>0) .and. !lGspInUseM) .And. !Len(aCompFutur) > 0
	...
	For nX := 1 To nMaxFor
	    If aColsSE2[nX][nPValor] > 0
			...
			If FindFunction("A103IdGen")
				A103IdGen(cIdsTrGen,@lPccMR,@lIrfMR,@lInsMR,@lIssMR,@lCidMR,@lSestMR)
			Endif

            FaAvalSE2(1, "MATA100",(nX==1),MaFisRet(,"NF_VALIRR"),MaFisRet(,"NF_VALINS"),lRetParc,MaFisRet(,"NF_VALISS"),MaFisRet(,"NF_BASEISS"),lRatImp,cRecIss,,aCodR,aImpos,lPccMR,lIrfMR,lInsMR,lIssMR,lCidMR,lSestMR,,If(lRateio,oRatIRF:aRatIRF,NIL)) 
            If cPaisLoc == "BRA"
				...
				Else
					If Empty(SF1->F1_NUMRA)
						If lCtbOnLine
							...
							//Rotina de integracao com o modulo financeiro
							A103AtuSE2(1,aRecSE2,aHeadSE2,aColsSE2,aHeadSEV,aColsSEV,cFornIss,cLojaIss,cDirf,cCodRet,cModRetPIS,nIndexSE2,aSEZ,dVencIss,cMdRtISS,SF1->F1_TXMOEDA,lTxNeg,aRecGerSE2,cA2FRETISS,cB1FRETISS,aMultas,lRatLiq,lRatImp,aCodR,cRecIss,lPisCofImp,aTitImp,lIssImp,lTemDocs,,aParcTrGen,aRecSEV,cIdsTrGen)
