...
// Projeto: BSC
// Modulo : Database
// Fonte  : BSC023C_Doc.prw
// ---------+-------------------+--------------------------------------------------------
// Data     | Autor             | Descricao
// ---------+-------------------+--------------------------------------------------------
// 25.06.04 | 1645 Leandro Marcelino Santos
// --------------------------------------------------------------------------------------

#include "BIDefs.ch"
#include "BSCDefs.ch"
#include "BSC023C_Doc.ch"
...
@table BSC023C
--------------------------------------------------------------------------------------*/
#define TAG_ENTITY "FCSDOC"
#define TAG_GROUP  "FCSDOCS"
#define TEXT_ENTITY STR0001/*//"Documento"*/
#define TEXT_GROUP  STR0002/*//"Documentos"*/

class TBSC023C from TBITable
...
method New() class TBSC023C
	::NewBSC023C()
return
method NewBSC023C() class TBSC023C
	local oField
	...
	// Indexes
	::addIndex(TBIIndex():New("BSC023CI01",	{"ID"},		.t.))
	::addIndex(TBIIndex():New("BSC023CI02",	{"NOME", "CONTEXTID"},	.t.))
	::addIndex(TBIIndex():New("BSC023CI03",	{"PARENTID", "ID"},	.t.))
return

// Arvore
method oArvore(nParentID) class TBSC023C
...
method oToXMLList(nParentID) class TBSC023C
...
// Carregar
method oToXMLNode(nParentID) class TBSC023C
...
method nInsFromXML(oXML, cPath) class TBSC023C
...
method nUpdFromXML(oXML, cPath) class TBSC023C
...
method nDelFromXML(nID) class TBSC023C
...
method nDuplicate(nParentID, nNewParentID, nNewContextID) class TBSC023C
